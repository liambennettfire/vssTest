...
BEGIN
  ...
  IF @v_usageclasscode is null OR @v_usageclasscode <= 0 BEGIN
    print 'Could not find usageclasscode for qsicode 56'
    return
  END
 ...
 SELECT @v_count = count(*) 
  ...
  VALUES (@v_new_qsiwindowviewkey, 'Publicity Project','Publicity Project',
  ...
  SELECT @v_configobjectkey = configobjectkey 
   ...
   WHERE lower(defaultlabeldesc) = lower('Event Details') 
  ...
  IF @v_configobjectkey <> 0 BEGIN
    ...
    VALUES (@v_new_configdetailkey,@v_configobjectkey,@v_usageclasscode,'Event Details',
         1,0,'Firebrand',getdate(),3,
         null,null,null,@v_new_qsiwindowviewkey,
         null)
  END
  ELSE  BEGIN
      print 'Section not found on this database: Event Details'
  END
  ...
  IF @v_configobjectkey <> 0 BEGIN
    ...
    INSERT INTO qsiconfigdetail (configdetailkey,configobjectkey,usageclasscode,labeldesc,visibleind,minimizedind, 
                               lastuserid,lastmaintdate,position,path,viewcontrolname,editcontrolname, 
                               qsiwindowviewkey,sectioncontrolname) 
    VALUES (@v_new_configdetailkey,@v_configobjectkey,@v_usageclasscode,'Publicity Project',
