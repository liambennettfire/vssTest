...
--    END
  ...
  DECLARE taqprojectlementpartner_cur CURSOR FOR
	SELECT pep.partnercontactkey
	  FROM taqprojectelementpartner pep
	  JOIN cscustomerpartner cp on pep.partnercontactkey = cp.partnercontactkey
	 WHERE 
		assetkey = @i_taqelementkey
		and (@i_partnertag = 0 or cp.partnertag = @i_partnertag) -- If the partner tag is 0, select all partners
      ...
      -- this asset was processed after the most recent change
       ...
       END -- Changed to Latest Actual Processed Date from Latest Actual Distribute Date found KB 7/2/2013 Case24535 
			ELSE IF @v_pendingdistribution = 1 BEGIN
			   UPDATE taqprojectelementpartner
            SET cspartnerstatuscode = 5,        --Feb 2018 - changed from 3 (scheduled) to 5 - (not up to date)
                lastuserid = @i_userid,
                lastmaintdate = getdate()
          WHERE assetkey = @i_taqelementkey
            AND partnercontactkey = @v_partnercontactkey
