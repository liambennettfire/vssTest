-- Enable Input AND Approval Currency fields on selected Project Detail sections
...
-- Enable 'Allow Alternate P&L Currencies' option
UPDATE clientoptions SET optionvalue = 1 WHERE optionid = 115
...
-- Scales
SELECT @v_scales_itemtype = datacode
FROM gentables
...
DECLARE cur_projecttypes CURSOR FOR
  ...
  WHERE 
    ...
    AND (s.qsicode IN (@v_posummary_qsicode, @v_poproforma_qsicode, @v_pofinal_qsicode)
      OR s.datacode = @v_scales_itemtype)
...
BEGIN
  ...
  INSERT INTO @InsertTable
    (datacode, datasubcode, numericdesc1, sortorder, itemtype, usageclass)
  VALUES
    (@v_sectioncode, 24, 2, 0, @v_itemtype, @v_usageclass) -- approval currency is hidden
