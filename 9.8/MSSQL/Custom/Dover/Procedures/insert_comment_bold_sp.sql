...
if exists (select * from dbo.sysobjects where id = Object_id('dbo.insert_comment_bold_sp') and (type = 'P' or type = 'RF'))
begin
 drop proc dbo.insert_comment_bold_sp
end

GO
create proc dbo.insert_comment_bold_sp
AS
...
while (@i_status<>-1 )  /* sttus 1*/
  begin
	...
	select @searchtest1 = charindex('<b>',@online_commenttext)
	select @searchtest3 = charindex('</b>',@online_commenttext)
		...
		if @charfind2 > 0 
		  begin
			...
			/* insert the paragraph by itself do not circle the entire paragraph*/
				select @online_commenttext = STUFF(@online_commenttext,@searchtest1,3,'{\b\f1\fs20\cf0\up0\dn0 ')
