  ...
  DECLARE c_ecf_products CURSOR fast_forward FOR
	...
	select  @i_productfetchstatus  = @@FETCH_STATUS
	 ...
	 while (@i_productfetchstatus >-1 )
		...
		--  QSI  -- '\\mcdonald\mediachase\uap_images\AuthorPics\'
		--  GO LIVE -- '\\qsiweb002\AUthorPics\' Update lenght to 24 from 45
		

		Select @v_file = @v_filepath + Substring(commenttext,45,datalength(commenttext)) 
		from uap..qsicomments 
...
--print cast(@i_current_metakey as varchar)
	  ...
	  where objectid = @i_product_id
		...
		--  QSI  -- '\\mcdonald\mediachase\barb_images\AuthorPics\'
