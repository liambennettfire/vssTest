...
BEGIN
			...
			select @i_pubtoweb_count = count (*) from UAP..bookdetail bd
			where publishtowebind=1
			and bookkey in (select bookkey from UAP..book where workkey=@i_workkey)
			...
			begin
				...
				Select @v_title = Substring(UAP.dbo.qweb_get_Title(@i_bookkey,'T'),1,50)
