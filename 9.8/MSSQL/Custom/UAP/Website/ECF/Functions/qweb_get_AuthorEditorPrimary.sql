...
BEGIN
		...
		select @fullauthordisplayname = fullauthordisplayname FROM uap..bookdetail
		...
		IF NOT EXISTS (Select * FROM uap..bookauthor 
						WHERE bookkey = @bookkey 
						--and primaryind = 1 
						and authortypecode in (12, 16)
					  )
			BEGIN
				IF EXISTS (Select * FROM uap..bookauthor WHERE bookkey = @bookkey)
					BEGIN
						...
						@lastname = a.lastname, @suffix = gc.suffix FROM uap..bookauthor ba
						JOIN uap..author a
						ON ba.authorkey = a.authorkey
						JOIN uap..globalcontact gc
		...
		DECLARE c_authors CURSOR FOR
			...
			gc.suffix FROM uap..bookauthor ba
			JOIN uap..author a
			ON ba.authorkey = a.authorkey
			JOIN uap..globalcontact gc
			ON a.authorkey = gc.globalcontactkey
