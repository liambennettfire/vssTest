/****** Object:  StoredProcedure [dbo].[qweb_ecf_SkuEx_Journal_By_Price]    Script Date: 04/09/2009 14:53:02 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

drop procedure [qweb_ecf_SkuEx_Journal_By_Price] 
go
...
------------------------------------------------------
	...
	SET @Journals_Single_Issues_Available = (CASE Upper(UNL.dbo.[get_Tab_Journals_Single_Issues_Available?](@i_bookkey))
					WHEN 'YES' Then 1
					ELSE 0 END)
	...
	select  @i_titlefetchstatus  = @@FETCH_STATUS
	 ...
	 while (@i_titlefetchstatus >-1 )
		...
		begin
			...
			if not exists (Select * from unl..bookdetail where bookkey = @i_bookkey and mediatypecode = 6 and mediatypesubcode = 1)
				begin
				...
				Select @v_Journals_Brief_Description = commenttext FROM  UNL..bookcomments WHERE commenttypecode = 3 AND commenttypesubcode = 7 AND bookkey = @i_bookkey
				...
				Select @v_Journal_ISSN = itemnumber from UNL..isbn i where i.bookkey = @i_bookkey
				...
				Select @v_journalsubscriptionlocation = alternatedesc1 from UNL..gentables where datacode = @i_pricetypecode and tableid = 306
				Select @v_journalsubsrcriptionlength = alternatedesc2 from UNL..gentables where datacode = @i_pricetypecode and tableid = 306
			

				--if journal special issue use journal bookkey for the following fields
				If @Journal_IssueInd = 1
					Begin
							Select @v_fullauthordisplayname = UNL.dbo.[qweb_get_Author](@masterJournal_bookkey,0,0,'F') + ' ' + 
								  UNL.dbo.[qweb_get_Author](@masterJournal_bookkey,0,0,'M') + ' ' + 
								  UNL.dbo.[qweb_get_Author](@masterJournal_bookkey,0,0,'L')
							...
							Select @v_Journal_ISSN = itemnumber from UNL..isbn i where i.bookkey = @masterJournal_bookkey
							Select @v_authorbylineprepro = commenthtmllite from UNL..bookcomments where commenttypecode = 3 and commenttypesubcode = 73 and bookkey = @masterJournal_bookkey
