IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.printing_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.printing_trackertrigger'
	DROP TRIGGER dbo.printing_trackertrigger
	PRINT 'Creating TRIGGER dbo.printing_trackertrigger...'
END

GO

CREATE TRIGGER printing_trackertrigger
ON printing
FOR INSERT, UPDATE
AS
