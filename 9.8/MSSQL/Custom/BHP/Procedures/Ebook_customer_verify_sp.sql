...
/****** Object:  StoredProcedure [dbo].[Ebook_customer_verify_sp]    Script Date: 04/26/2016 10:07:25 ******/
SET ANSI_NULLS ON
GO
...
if @v_languagecode is null  begin
	...
	insert into bookverificationmessage
	      (messagekey, bookkey, verificationtypecode, messagetypecode, message, lastmaintuser, lastmaintdate)
	values(@v_nextkey, @i_bookkey, @i_verificationtypecode, @v_Error, 'Missing Primary Language'+ @v_bsg_msg,@i_username, getdate() )
	set @v_failed = 1
end 
end
...
if @v_languagecode is NOT null and @v_languagecode2 is NOT  null begin 
	...
	if @v_cnt = 0 begin
		...
		insert into bookverificationmessage
		      (messagekey, bookkey, verificationtypecode, messagetypecode, message, lastmaintuser, lastmaintdate)
		values(@v_nextkey, @i_bookkey, @i_verificationtypecode, @v_Error, 'Missing Eloquence Field tag or export to eloquence indicator for Primary Langauge'+ @v_bsg_msg,@i_username, getdate() )
		set @v_failed = 1
	end 
end
end
...
if @v_territoriescode is null or  @v_territoriescode = 0  begin
	...
	insert into bookverificationmessage
	      (messagekey, bookkey, verificationtypecode, messagetypecode, message, lastmaintuser, lastmaintdate)
	values(@v_nextkey, @i_bookkey, @i_verificationtypecode, @v_Error, 'Missing or Inactive Sales Territories information' + @v_bsg_msg,@i_username, getdate() )
	set @v_failed = 1
end 
end
...
if @v_territoriescode is null or  @v_territoriescode = 0  begin
	...
	insert into bookverificationmessage
	      (messagekey, bookkey, verificationtypecode, messagetypecode, message, lastmaintuser, lastmaintdate)
	values(@v_nextkey, @i_bookkey, @i_verificationtypecode, @v_Error, 'Missing Eloquence Field tag or export to eloquence indicator for Sales Territories' + @v_bsg_msg,@i_username, getdate() )
	set @v_failed = 1
end 
end
...
if @v_mediatypecode is not null begin
	...
	insert into bookverificationmessage
	      (messagekey, bookkey, verificationtypecode, messagetypecode, message, lastmaintuser, lastmaintdate)
	values(@v_nextkey, @i_bookkey, @i_verificationtypecode, @v_Error, 'Missing Eloquence Field tag or export to eloquence indicator for Media'+ @v_bsg_msg,@i_username, getdate() )
	set @v_failed = 1
	end 
end
end
...
if @v_mediatypesubcode is not null and @v_mediatypecode is not null begin
	...
	if @v_cnt = 0 begin
		...
		insert into bookverificationmessage
		      (messagekey, bookkey, verificationtypecode, messagetypecode, message, lastmaintuser, lastmaintdate)
		values(@v_nextkey, @i_bookkey, @i_verificationtypecode, @v_Error, 'Missing Eloquence Field tag or export to eloquence indicator for Format'+ @v_bsg_msg,@i_username, getdate() )
		set @v_failed = 1
	end 
end
end
...
if @i_write_msg = 1 begin
	if @v_cnt > 0 begin
		...
		insert into bookverificationmessage
		      (messagekey, bookkey, verificationtypecode, messagetypecode, message, lastmaintuser, lastmaintdate)
		values(@v_nextkey, @i_bookkey, @i_verificationtypecode, @v_Error, 'Missing Eloquence Field tag or export to eloquence indicator for Author'+ @v_bsg_msg,@i_username, getdate() )
		set @v_failed = 1
		end 
	end
end	
...
if @v_bestdate <getdate() and @v_bisac_status_code = 4 begin
	...
	insert into bookverificationmessage
	      (messagekey, bookkey, verificationtypecode, messagetypecode, message, lastmaintuser, lastmaintdate)
	values(@v_nextkey, @i_bookkey, @i_verificationtypecode, @v_error, 'PUB Date passed, status still Forthcoming (NYP)'+ @v_bsg_msg,@i_username, getdate() )
	set @v_failed = 1
end 
...
if @v_bestdate >getdate()+10 and @v_bisac_status_code = 1 begin
	exec get_next_key @i_username, @v_nextkey out
	insert into bookverificationmessage
	      (messagekey, bookkey, verificationtypecode, messagetypecode, message, lastmaintuser, lastmaintdate)
	values(@v_nextkey, @i_bookkey, @i_verificationtypecode, @v_error, 'PUB Date in future, status is Active'+ @v_bsg_msg,@i_username, getdate() )
	set @v_failed = 1
end 
...
--price 
	...
	BEGIN  --4
		...
		insert into bookverificationmessage
		      (messagekey, bookkey, verificationtypecode, messagetypecode, message, lastmaintuser, lastmaintdate)
		values(@v_nextkey, @i_bookkey, @i_verificationtypecode, @v_Error, 'Missing '+ @c_pricedesc + @v_bsg_msg,@i_username, getdate() )
		set @v_failed = 1
	 END --5
	END --4
...
if @i_verificationtypecode <>0 begin
	if @v_cnt = 0 begin
		...
		insert into bookverificationmessage
		      (messagekey, bookkey, verificationtypecode, messagetypecode, message, lastmaintuser, lastmaintdate)
		values(@v_nextkey, @i_bookkey, @i_verificationtypecode, @v_Error, 'Long Description is missing or Release to Eloquence not checked'+ @v_bsg_msg,@i_username, getdate() )
		set @v_failed = 1
	end
end
end
...
if @i_verificationtypecode <> 0  begin
	if @v_cnt = 0 begin
		...
		insert into bookverificationmessage
		      (messagekey, bookkey, verificationtypecode, messagetypecode, message, lastmaintuser, lastmaintdate)
		values(@v_nextkey, @i_bookkey, @i_verificationtypecode, @v_Error, 'Short Description is missing or Release to Eloquence not checked'+ @v_bsg_msg,@i_username, getdate() )
		set @v_failed = 1
	end
end
end
...
if @i_verificationtypecode <>0  begin
	if @v_cnt = 0 begin
		...
		insert into bookverificationmessage
		      (messagekey, bookkey, verificationtypecode, messagetypecode, message, lastmaintuser, lastmaintdate)
		values(@v_nextkey, @i_bookkey, @i_verificationtypecode, @v_Error, 'Missing Supply Chain data for Print ISBN'+ @v_bsg_msg,@i_username, getdate() )
		set @v_failed = 1
	end
end
end
...
GRANT EXECUTE ON dbo.Ebook_customer_verify_sp TO PUBLIC
GO
