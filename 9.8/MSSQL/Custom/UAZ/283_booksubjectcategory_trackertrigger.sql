IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.booksubjectcategory_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.booksubjectcategory_trackertrigger'
	DROP TRIGGER dbo.booksubjectcategory_trackertrigger
	PRINT 'Creating TRIGGER dbo.booksubjectcategory_trackertrigger...'
END

GO

CREATE TRIGGER booksubjectcategory_trackertrigger
ON booksubjectcategory
FOR INSERT, UPDATE
AS
