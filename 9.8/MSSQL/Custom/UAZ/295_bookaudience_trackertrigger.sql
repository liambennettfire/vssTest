IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.bookaudience_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.bookaudience_trackertrigger'
	DROP TRIGGER dbo.bookaudience_trackertrigger
	PRINT 'Creating TRIGGER dbo.bookaudience_trackertrigger...'
END

GO

CREATE TRIGGER bookaudience_trackertrigger
ON bookaudience
FOR INSERT, UPDATE
AS
