IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.bookcomments_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.bookcomments_trackertrigger'
	DROP TRIGGER dbo.bookcomments_trackertrigger
	PRINT 'Creating TRIGGER dbo.bookcomments_trackertrigger...'
END

GO

CREATE TRIGGER bookcomments_trackertrigger
ON bookcomments
FOR INSERT, UPDATE
AS
