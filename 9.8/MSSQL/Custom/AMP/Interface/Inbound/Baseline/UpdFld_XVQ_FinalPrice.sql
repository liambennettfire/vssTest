...
IF OBJECT_ID('dbo.UpdFld_XVQ_FinalPrice') IS NOT NULL DROP PROCEDURE dbo.UpdFld_XVQ_FinalPrice
GO

CREATE PROCEDURE dbo.UpdFld_XVQ_FinalPrice
@bookkey        int,
@pricecode      int,
@currencycode   int,
@activeind      int,
...
@validation_sproc_name varchar(100), -- null, 'isnumeric', or name of sproc that takes appropriate parameters (see template)
...
begin
	if @transform_sproc_name is not null AND OBJECT_ID(@transform_sproc_name) is not null
		EXEC @transform_sproc_name 1, @pricecode, @currencycode, @effectivedate, @extract output, @length output, @bookkey
	...
	if @is_valid is not null begin
		...
		if OBJECT_ID(@validation_sproc_name) is null begin
			...
			EXEC @validation_sproc_name @bookkey, @pricecode, @currencycode, @extract, @vldn_result_code output, @vldn_result_desc output
...
begin
	if @transform_sproc_name is not null AND OBJECT_ID(@transform_sproc_name) is not null
		EXEC @transform_sproc_name 2, @pricecode, @currencycode, @effectivedate, @extract output, @length output, @bookkey

	declare @uf_error_code int
	declare @uf_error_desc varchar(2000)

	-- Do insert/update and (if applicable) titlehistory
	EXEC dbo.UpdFld_FinalPrice @bookkey, @pricecode, @currencycode, @activeind, @effectivedate, @extract, 1, @length, @userid, @uf_error_code output, @uf_error_desc output
...
begin  -- write to the qsijobmessage table
	...
	-- Code specific to FinalPrice data
	declare @datadesc varchar(255)
	...
	declare @qsijob_short_msg    varchar(30)    -- used to get/pass an "UpdFld_XVQ_XXXX standardized" qsijobmessages messageshortdesc value
