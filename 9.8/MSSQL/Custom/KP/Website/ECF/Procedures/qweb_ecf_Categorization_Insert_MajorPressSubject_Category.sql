if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[qweb_ecf_Categorization_Insert_MajorPressSubject_Category]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[qweb_ecf_Categorization_Insert_MajorPressSubject_Category]

GO


CREATE procedure [dbo].[qweb_ecf_Categorization_Insert_MajorPressSubject_Category] (@i_bookkey int) as
...
BEGIN
	SET @tableid = 413 --Major Press Subjects
	...
	(select categoryid from category where parentcategoryid=1141)) --1141 major press subjects category

	delete from categorization where objectid in 
	(select objectid from productex_journals where pss_product_bookkey = @i_bookkey)
	and objecttypeid=1 and categoryid in (select categoryid from category where parentcategoryid in
	(select categoryid from category where parentcategoryid=1141))

	DECLARE c_pss_mps_category INSENSITIVE CURSOR
	FOR

	Select bookkey, categorycode, categorysubcode
	from cbd..booksubjectcategory
	...
	OPEN c_pss_mps_category

	FETCH NEXT FROM c_pss_mps_category
	...
	select  @i_titlefetchstatus  = @@FETCH_STATUS
	 ...
	 while (@i_titlefetchstatus >-1 )
		...
		begin
			...
			--ONLY ONE LEVEL OF MAJOR PRESS SUBJECTS, IGNORE SUBSUBJECTS
...
--			end
--			else begin
--			  Select @subject_object_id = objectid from CategoryEx_Title_MajorPressSubject 
--			  where pss_subject_categorytableid = @tableid 
--			    and pss_subject_datacode = @i_categorycode
--			end
...
close c_pss_mps_category
deallocate c_pss_mps_category


END
GO
Grant execute on dbo.qweb_ecf_Categorization_Insert_MajorPressSubject_Category to Public
GO
