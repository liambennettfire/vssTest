if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[qweb_ecf_Insert_Products]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
...
BEGIN
			...
			select @i_pubtoweb_count = count (*) from cbd..bookdetail bd
			where publishtowebind=1
			and bookkey in (select bookkey from cbd..book where workkey=@i_workkey)
			...
			begin
				...
				Select @v_title = Substring(cbd.dbo.qweb_get_Title(@i_bookkey,'T'),1,50)
...
GO
Grant execute on dbo.qweb_ecf_Insert_Products to Public
GO