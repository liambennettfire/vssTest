...
BEGIN
	...
	/*and code in (Select cast(bookkey as varchar) from cbd..bookdetail where publishtowebind = 1)*/
	...
	select  @i_productfetchstatus  = @@FETCH_STATUS
	 ...
	 while (@i_productfetchstatus >-1 )
		...
		Select @v_mediatype = cbd.dbo.qweb_get_Media(@i_c_bookkey, 'D')
		Select @d_datetime = getdate()
		select @i_maxsubbookkey=cbd.dbo.qweb_get_subordinate_max_web(@i_c_bookkey)

		
		--  QSI  -- '\\mcdonald\mediachase\UAP_images\'
		--  UNP  -- '\\unp-muskie\Images\ '  *** NOTE EXTRA SPACE AT END OF PATH AT UNP
		-- KP  --> '\\merv\mediachase\kp_images\covers\

		Select @v_file = @v_filepath + REVERSE(SUBSTRING(reverse(pathname),1, CHARINDEX('\', reverse(pathname))-1))
		from cbd..filelocation 
