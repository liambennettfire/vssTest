...
--		and mediatypecode = 6 and mediatypesubcode = 1
		...
		@Journals_Single_Issues_Available = (CASE Upper(cbd.dbo.[get_Tab_Journals_Single_Issues_Available?](@i_bookkey))
				WHEN 'YES' Then 1
				ELSE 0 END)
		from cbd..book b, cbd..bookdetail bd
		where b.bookkey = bd.bookkey
		...
		begin
			...
			print @i_workkey
				...
				Select @masterJournal_bookkey = parentbookkey FROM cbd..bookfamily where childbookkey = @i_bookkey and relationcode = 20005
				...
				Select @v_fullauthordisplayname = cbd.dbo.[qweb_get_Author](@i_bookkey,1,0,'F') + ' ' + 
												  cbd.dbo.[qweb_get_Author](@i_bookkey,1,0,'M') + ' ' + 
				...
				Select @v_jstore_ind = cbd.dbo.get_Tab_Journals_JSTOR(@i_bookkey)
				Select @v_IntructionsToContrib = commenthtmllite from cbd..bookcomments where commenttypecode = 3 and commenttypesubcode = 50 and bookkey = @i_bookkey 
--				Select @v_SingleIssueAvail = CASE cbd.dbo.[get_Tab_Journals_Single_Issues_Available?](@i_bookkey)
--															 WHEN 'Y' Then 1
--															 ELSE 0 END
			
				Select @v_MostRecentIssue_bookkey = bookkey 
					from cbd..book where bookkey in (Select top 1 d.bookkey
												from cbd..bookdates d, cbd..bookdetail bd
				...
				Select @v_MostRecent_VolIssue = commenttext from cbd..bookcomments where commenttypecode = 3 and commenttypesubcode = 53 and bookkey = @v_MostRecentIssue_bookkey 
				IF @Issueind = 0
					begin
						Select @v_toc = commenthtmllite from cbd..bookcomments where commenttypecode = 3 and commenttypesubcode = 52 and bookkey = @v_MostRecentIssue_bookkey 
					end
				IF @issueind = 1
					begin
						Select @v_toc = commenthtmllite from cbd..bookcomments where commenttypecode = 3 and commenttypesubcode = 52 and bookkey = @i_bookkey 
					end
				Select @v_Journal_Advertising = commenthtml from cbd..bookcomments where commenttypecode = 3 and commenttypesubcode = 54 and bookkey = @i_bookkey 
				Select @v_Journal_Website = pathname from cbd..filelocation where filetypecode = 8 and bookkey = @i_bookkey 
				Select @v_Online_Journal_Link = pathname from cbd..filelocation where filetypecode = 10 and bookkey = @i_bookkey 
				--Select @i_Journals_Single_Issues = cbd.dbo.[get_Tab_Journals_Single_Issues_Available?] (@i_bookkey)
				Select @v_Journal_interval = cbd.dbo.get_Tab_Journals_Journal_Interval (@i_bookkey)
				Select @v_publisher = cbd.dbo.qweb_get_GroupLevel3(@i_bookkey,'F')
				select @v_price_text = commenthtmllite from cbd..bookcomments where commenttypecode = 1 and commenttypesubcode = 49 and bookkey = @i_bookkey
				...
				Select @Journal_Brief_description = commentstring from cbd..bookcomments where commenttypecode = 3 and commenttypesubcode = 7 and bookkey = @i_bookkey

				select @i_journal_muse_ind = longvalue from cbd..bookmisc where misckey=31 and bookkey= @i_bookkey
				...
				Select @Journal_ISSN = itemnumber from cbd..isbn i where i.bookkey = @i_bookkey
						...
						If 	@masterJournal_bookkey is not null or @masterJournal_bookkey = ''
							Begin
								If @v_fullauthordisplayname is null or rtrim(@v_fullauthordisplayname) = ''
									begin
												Select @v_fullauthordisplayname = cbd.dbo.[qweb_get_Author](@masterJournal_bookkey,1,0,'F') + ' ' + 
												  cbd.dbo.[qweb_get_Author](@masterJournal_bookkey,1,0,'M') + ' ' + 
												  cbd.dbo.[qweb_get_Author](@masterJournal_bookkey,1,0,'L')
													select @v_unformat_fullauthordisplayname = cbd.dbo.replace_xchars(@v_fullauthordisplayname)
												select @v_authormetakeywords = @v_fullauthordisplayname + ', ' + @v_unformat_fullauthordisplayname
									end 
								if @v_Journal_Advertising is null or @v_Journal_Advertising = ''
									begin
										Select @v_Journal_Advertising = commenthtml from cbd..bookcomments where commenttypecode = 3 and commenttypesubcode = 54 and bookkey = @masterJournal_bookkey 
									end 
								If @v_Journal_Website is null or @v_Journal_Website = ''
									begin
										Select @v_Journal_Website = pathname from cbd..filelocation where filetypecode = 8 and bookkey = @masterJournal_bookkey 
									end 
								if @v_Journal_interval is null or @v_Journal_interval = ''
									begin
										Select @v_Journal_interval = cbd.dbo.get_Tab_Journals_Journal_Interval (@masterJournal_bookkey)
									end
...
--									end
								...
								Select @v_MostRecent_VolIssue = commenttext from cbd..bookcomments where commenttypecode = 3 and commenttypesubcode = 53 and bookkey = @i_bookkey
								

								if @v_IntructionsToContrib is null or @v_IntructionsToContrib = ''
									begin
										Select @v_IntructionsToContrib = commenthtmllite from cbd..bookcomments where commenttypecode = 3 and commenttypesubcode = 50 and bookkey = @masterJournal_bookkey 
									end 
								if @Journal_ISSN is null or @Journal_ISSN = ''
									begin
										Select @Journal_ISSN = itemnumber from cbd..isbn i where i.bookkey = @masterJournal_bookkey
									end
								if @n_Description_Comment is null or @n_Description_Comment = ''
									begin
										Select @n_Description_Comment = commenthtmllite from cbd..bookcomments where commenttypecode = 3 and commenttypesubcode = 8 and bookkey = @masterJournal_bookkey
									end
								if @i_journal_muse_ind is null or @i_journal_muse_ind = ''
									begin
										select @i_journal_muse_ind = longvalue from cbd..bookmisc where misckey=31 and bookkey= @masterJournal_bookkey
									end
							End				
					END
