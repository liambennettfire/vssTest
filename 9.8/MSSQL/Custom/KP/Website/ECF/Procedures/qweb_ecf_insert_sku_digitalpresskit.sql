if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[qweb_ecf_insert_sku_digitalpresskit]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
...
BEGIN
	...
	and f.pss_sku_bookkey in (Select bookkey from cbd..bookdetail where publishtowebind = 1)
...
GO
Grant execute on dbo.qweb_ecf_insert_sku_digitalpresskit to Public
GO