if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qcs_get_product') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
...
BEGIN
	...
	--	SELECT DISTINCT @bgpRetStatus+=ReturnMsgDesc
	--	FROM backgroundprocess
	--	WHERE key1=@bookkey
	--	AND reqforgetprodind=1
	...
	IF (@bgpFailProduct=1 AND @bgpRetStatus='') BEGIN 
		...
		IF(@bgpCount>0) SET @bgpRetStatus = ' This title has failed Get Product processes and cannot be sent to the cloud until these have been resolved.'
	END
	...
	IF (@bgpFailProduct=1) BEGIN
		SET @bgpRetVal = 'The title for bookkey: '+ CAST(@bookkey AS VARCHAR(50)) + ' failed background processing'
		IF (@bgpRetStatus!='') SET @bgpRetVal = @bgpRetVal + ' with:' + @bgpRetStatus
		RETURN @bgpRetVal
	END
  ...
  SELECT @SendRightsToCloud = optionvalue
   ...
   WHERE optionid = 114
    ...
    SELECT TOP 1
		...
		NULL as Type -- added by TT on 03-07-16
    FROM
        book AS b,
        bookdetail AS bd,
        isbn AS i,
