PRINT 'STORED PROCEDURE : dbo.datawarehouse_base_full_uncond'
GO

if exists (select * from dbo.sysobjects where id = Object_id('dbo.datawarehouse_base_full_uncond') and (type = 'P' or type = 'RF'))
begin
 drop proc dbo.datawarehouse_base_full_uncond
end

GO

CREATE  proc dbo.datawarehouse_base_full_uncond 
AS 
...
VALUES (@ware_newhousekey, getdate(),null,'FULL',1)
commit tran

 SELECT @ware_total = count(*)
	    FROM book
   		WHERE   (templatetypecode is null ) OR
         		( templatetypecode = 0 )
...
BEGIN tran
	...
	truncate table whprintingpersonnel
