...
*   --------     --------    -------------------------------------------
**  02/05/2016   UK		     Case 36206
**  02/24/2016	 UK			 Case 36206 - Task 003
*******************************************************************************/
...
IF UPDATE (titleprefix) OR 
	...
	UPDATE (gradelow) OR
	UPDATE (gradehigh) OR
	UPDATE(csapprovalcode) OR
	UPDATE (fullauthordisplayname)
...
BEGIN
   ...
   ELSE IF @v_agelow <> 0 AND @v_agehigh = 0 
	...
	ELSE BEGIN
		IF @v_gradeinfo IS NOT NULL SET @v_agegradeinfo = '/ ' + @v_gradeinfo 
	    ELSE IF @v_ageinfo IS NOT NULL SET @v_agegradeinfo = @v_ageinfo 
	END
