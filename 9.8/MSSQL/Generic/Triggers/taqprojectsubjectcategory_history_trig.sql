IF EXISTS (SELECT * FROM sysobjects WHERE type = 'TR' AND name = 'taqprojectsubjectcategory_history')
	BEGIN
		DROP  Trigger dbo.taqprojectsubjectcategory_history
	END
GO

CREATE TRIGGER [dbo].taqprojectsubjectcategory_history ON [dbo].taqprojectsubjectcategory
  ...
  while @v_tranacation_row_total>=@v_tranacation_row_pointer
    begin
      ...
      SELECT * into #inserted FROM  
        (SELECT Row_Number() OVER (ORDER BY taqprojectkey) as transaction_rowid,*  
        FROM inserted) as a  
