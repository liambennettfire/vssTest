...
BEGIN
  ...
  IF @SearchItem = 1 		    -- Titles
    BEGIN
      IF @i_selectall = 1 BEGIN
        ...
        SELECT key1, COALESCE(key2, 1) key2, key3, @indicator1 as indicator1, @indicator2 as indicator2, @quantity1 as quantity1, @quantity2 as quantity2,
		 ...
		 (SELECT datadesc FROM gentables WHERE tableid = 312 AND datacode = c.mediatypecode) + ' / ' + c.formatname formatdesc,
          CASE 
			 ...
			 (SELECT COUNT(*)
			  ...
			  WHERE tpt.projectrolecode = (select datacode from gentables where tableid = 604 and qsicode = 1) AND
			  tpt.titlerolecode = (select datacode from gentables where tableid = 605 and qsicode = 1) 
			  AND tpt.taqprojectkey IN(
			  select tp.taqprojectkey from taqprojecttitle tp 
			  INNER JOIN taqproject t ON t.taqprojectkey = tp.taqprojectkey
			  where tp.bookkey = r.key1 AND
			  tp.projectrolecode = (select datacode from gentables where tableid = 604 and qsicode = 1) AND
			  tp.titlerolecode = (select datacode from gentables where tableid = 605 and qsicode = 1) 
			  AND t.searchitemcode = (select datacode from gentables where tableid = 550 and qsicode = 9))) = 1 
			  THEN 
			    (SELECT tpt.taqprojectkey
						  FROM taqprojecttitle tpt
						  WHERE tpt.projectrolecode = (select datacode from gentables where tableid = 604 and qsicode = 1) AND
						  tpt.titlerolecode = (select datacode from gentables where tableid = 605 and qsicode = 1) 
						  AND tpt.taqprojectkey IN(
						  ...
						  tp.projectrolecode = (select datacode from gentables where tableid = 604 and qsicode = 1) AND
						  tp.titlerolecode = (select datacode from gentables where tableid = 605 and qsicode = 1) 
						  AND t.searchitemcode = (select datacode from gentables where tableid = 550 and qsicode = 9)))			  
			ELSE 0 
			END AS WorkProjectKeyWithOneFormat		 
  ...
  ELSE  --all project type items
    BEGIN
      IF @i_selectall = 1 BEGIN
        -- Return all items in list
        SELECT key1, key2, key3, c.*, @indicator1 as indicator1, @indicator2 as indicator2, @quantity1 as quantity1, @quantity2 as quantity2
