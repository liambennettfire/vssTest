IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[history_triggers_enable]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[history_triggers_enable]
...
CREATE PROCEDURE [dbo].[history_triggers_enable] 
...
BEGIN 
  ...
  set @v_sqlbase='ENABLE TRIGGER $$Trigger ON $$Table;'
