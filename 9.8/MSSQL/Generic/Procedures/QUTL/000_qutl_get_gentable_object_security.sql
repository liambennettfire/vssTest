  ...
  IF @i_accesscode >= 0 BEGIN
    SELECT @i_accesscode accesscode, g.*
     ...
     WHERE g.tableid = @i_tableid
       AND dbo.qutl_check_gentable_value_security(@i_userkey,@i_windowname,@i_tableid,g.datacode) = @i_accesscode
  END
  ELSE BEGIN
    SELECT dbo.qutl_check_gentable_value_security(@i_userkey,@i_windowname,@i_tableid,g.datacode) accesscode, g.*
      FROM gentables g
     WHERE g.tableid = @i_tableid
  END
