...
**    --------      --------       --------------------------------------------------------
**    04/08/2016    UK		    	   Case 36951 - Task 003
**    05/26/2016    Colman         Case 38209 - Translate certain @i_miscsectionlabel values to known configobjectid
**    11/22/2016    Alan           When Translate certain @i_miscsectionlabel values to known configobjectid make sure title summary is used for titles
********************************************************************************************/
...
BEGIN
  ...
  IF LOWER(@i_miscsectionlabel) = 'project details'
    SELECT @o_configobjectkey = configobjectkey
    FROM qsiconfigobjects
    WHERE LOWER(configobjectid) = 'projectdetails' AND itemtypecode = @v_itemtypecode
  ELSE IF LOWER(@i_miscsectionlabel) = 'title information'
    SELECT @o_configobjectkey = configobjectkey
    FROM qsiconfigobjects
    WHERE LOWER(configobjectid) = 'shtitleinfo' AND itemtypecode = @v_itemtypecode
    AND windowid in (select windowid from qsiwindows where windowname = 'titlesummary')
  ELSE
    SELECT @o_configobjectkey = configobjectkey
    FROM qsiconfigobjects
  ...
  IF @o_configobjectkey = 0 BEGIN
   ...
   -- Determine Section Control Name and WindowID based on Item Type     
	...
	ELSE IF @v_itemtypecode = 1  BEGIN
	    ...
	    SET @v_windowid = (SELECT windowid FROM qsiwindows WHERE LTRIM(RTRIM(LOWER(windowname))) = 'titlesummary')
	    END
