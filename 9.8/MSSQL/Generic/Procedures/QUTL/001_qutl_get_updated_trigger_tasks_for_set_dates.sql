...
IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'qutl_get_updated_trigger_tasks_for_set_dates')
BEGIN
  DROP  Procedure  qutl_get_updated_trigger_tasks_for_set_dates
END
GO

CREATE PROCEDURE dbo.qutl_get_updated_trigger_tasks_for_set_dates
  @i_listkey				INT,	
  @i_datetypecode			INT,
...
/**********************************************************************************************
**  Name: qutl_get_updated_trigger_tasks_for_set_dates
...
BEGIN
  ...
  IF @i_searchitemtypecode = 1 BEGIN  -- Title Search
    IF @i_updatefirstprintingkey = 1 BEGIN
		...
		WHERE tpt.printingkey = (SELECT printingkey FROM printing WHERE bookkey=r.key1 AND printingnum=1) 
	END
	ELSE BEGIN
		INSERT INTO #triggertasks
		SELECT 'original' as rowstatus, tpt.taqtaskkey, COALESCE(tpt.taqprojectkey, 0) as taqprojectkey, tpt.datetypecode, 
	...
	END
  END
  ELSE IF @i_searchitemtypecode IN (3, 5, 6, 11, 10, 15, 9, 14) BEGIN  -- Project Search
    INSERT INTO #triggertasks
	SELECT 'original' as rowstatus, tpt.taqtaskkey, COALESCE(tpt.taqprojectkey, 0) as taqprojectkey, tpt.datetypecode, 
	@i_activedate as activedate, NULL as origactivedate, dt.datelabel, COALESCE(tpt.bookkey, 0) as bookkey, COALESCE(tpt.printingkey, 0) as printingkey,  
	LTRIM(RTRIM(COALESCE(ct.productnumber, '') + ' ' + ct.title)) as title, cp.projecttitle projectname, dt.sortorder, NULL as TaskViewTriggerKeyTitle, NULL as TaskViewTriggerKeyProject, 
  ...
  END
  ELSE IF @i_searchitemtypecode = 2 BEGIN  -- Contacts
    INSERT INTO #triggertasks
	SELECT 'original' as rowstatus, tpt.taqtaskkey, COALESCE(tpt.taqprojectkey, 0) as taqprojectkey, tpt.datetypecode, 
	@i_activedate as activedate, NULL as origactivedate, dt.datelabel, COALESCE(tpt.bookkey, 0) as bookkey, COALESCE(tpt.printingkey, 0) as printingkey,  
	LTRIM(RTRIM(COALESCE(ct.productnumber, '') + ' ' + ct.title)) as title, cp.projecttitle projectname, dt.sortorder, NULL as TaskViewTriggerKeyTitle, NULL as TaskViewTriggerKeyProject, 
	...
	FROM taqprojecttask tpt 
	INNER JOIN qse_searchresults r
	ON r.key1 = tpt.globalcontactkey AND
	   r.listkey = @i_listkey	
	INNER JOIN datetype dt 
	ON tpt.datetypecode = dt.datetypecode AND dt.triggerdateind = 1 and dt.datetypecode = @i_datetypecode
	LEFT OUTER JOIN coreprojectinfo cp 
	...
	FROM taqprojecttask tpt 
	INNER JOIN qse_searchresults r
	ON r.key1 = tpt.globalcontactkey2 AND
	   r.listkey = @i_listkey	
	INNER JOIN datetype dt 
	ON tpt.datetypecode = dt.datetypecode AND dt.triggerdateind = 1 and dt.datetypecode = @i_datetypecode
	LEFT OUTER JOIN coreprojectinfo cp 
...
grant execute on qutl_get_updated_trigger_tasks_for_set_dates  to public
go
