...
BEGIN
  ...
  BEGIN
    ...
    SET @TempIndex = CHARINDEX('taqprojecttype=', @SQLUpdate)
    IF @TempIndex > 0
    BEGIN
      SET @TempName = SUBSTRING(@SQLUpdate, @TempIndex + 15, 100)
      SET @TempIndex = CHARINDEX(', ', @TempName)
      SET @NewProjectType = CONVERT(int, LTRIM(RTRIM(SUBSTRING(@TempName, 0, @TempIndex))))    

      SELECT @Count = COUNT(*)
      FROM gentablesitemtype
      WHERE tableid = 521 AND datacode = @NewProjectType
        AND itemtypecode = @SearchItemcode
        AND COALESCE(itemtypesubcode,0) IN (0,@UsageClasscode)
      ...
      BEGIN
        ...
        INSERT INTO qse_updatefeedback (userkey, searchitemcode, key1, key2, itemdesc, runtime, message)
        VALUES (@UserKey, @SearchItem, @ProjectKey, 0, @Title, getdate(), 'New Type is not allowed for ' + @Title + '.')
