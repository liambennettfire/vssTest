...
DECLARE
 ...
 BEGIN
  ...
  END
	...
	SELECT
		t.taqversionformatkey, t.relatedprojectkey
	INTO
		#tmp_relatedprojects
	FROM
		taqversionrelatedcomponents_view t
	WHERE 
		t.taqprojectkey = @i_po_projectkey
	...
	FROM #tmp_relatedprojects t
	INNER JOIN taqProject p 
		ON t.relatedprojectkey = p.taqprojectkey 
	INNER JOIN taqversionformat f
		ON t.taqversionformatkey = f.taqprojectformatkey
    ...
    BEGIN
		...
		BEGIN
		  ...
		  INSERT INTO gposection
			(gpokey, sectionkey, sectiontype, key1, key2, key3, quantity, description, taqversionformatkey, bookkey, printingkey, lastuserid, lastmaintdate)
		  VALUES
			(@i_gpokey, @v_new_sectionkey, @v_sectiontype, @v_key1, @v_key2, @v_key3, @v_quantity, @v_sectiondesc, @v_po_formatkey, @v_bookkey, @v_printingkey, @v_lastuserid, getdate())
