...
DECLARE
  ...
  @v_rowcount INT,
  @v_class_proforma_poreport INT,
  @v_class_final_poreport INT,    
  @v_itemtypecode INT,
...
BEGIN
  ...
  SELECT @v_class_proforma_poreport = datasubcode 
  FROM subgentables 
  where tableid = 550 AND qsicode = 42      
  
  SELECT @v_class_final_poreport = datasubcode 
  FROM subgentables 
  ...
  IF @v_vendor_rolecode = @i_rolecode BEGIN  
    ...
    IF @v_usageclasscode = @v_class_proforma_poreport OR @v_usageclasscode = @v_class_final_poreport BEGIN --Proforma or Final PO Report
		select @v_generated_title = dbo.get_generated_poreporttitle(@i_projectkey)			
    END
    IF @v_itemtypecode = 15 BEGIN	--other Purchase Order classes
