...
--  @v_tablename  varchar(50),
  ...
  if @i_TriggerActionID='delete'
    ...
    begin
      ...
      select @v_keyname=sc.name
        ...
        where so.id=object_id('tempdb..#deleted')
            ...
            and sc.xtype=st.xtype
            and st.name not like 'sys%'
            and sc.name='paymentkey'
      if @v_keyname is not null
        begin  
            set @v_sql = N'select @o_keyvalue=paymentkey from #deleted' 
            exec sp_executesql @v_sql, @v_sql_parmdef,
              @o_keyvalue = @v_keyvalue output
            set @v_paymentkey=@v_keyvalue
        end
      else
        begin  
          set @v_paymentkey=null
        END
    ...
    begin
      ...
      if @v_keyname is not null
        begin  
            set @v_sql = N'select @o_keyvalue=paymentkey from #inserted' 
            exec sp_executesql @v_sql, @v_sql_parmdef,
              @o_keyvalue = @v_keyvalue output
            set @v_paymentkey=@v_keyvalue
        end
          ...
          if @v_beforevalue<>@v_aftervalue
            ...
            begin
              insert into historychanges
               (columnkey,bookkey,printingkey,taqprojectkey,elementkey,misckey,beforevalue,aftervalue,changetype,lastmaintdate,lastuserid,paymentkey)
               values
               (@v_columnkey,@v_bookkey,@v_printingkey,@v_taqprojectkey,@v_elementkey,@v_misckey,@v_beforevalue,@v_aftervalue,@i_TriggerActionID,@v_lastmaintdate,@v_lastuserid,@v_paymentkey)
            end
