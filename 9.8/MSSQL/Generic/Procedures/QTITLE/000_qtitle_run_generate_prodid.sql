if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[qtitle_run_generate_prodid]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure [dbo].[qtitle_run_generate_prodid]
GO
...
**	--------	------	-----------
**  10/13/17  Colman  Case 47730 - handle case where @i_orglevelkey and @i_orgentrykey are both 0.
**************************************************************************************************/

CREATE PROCEDURE dbo.qtitle_run_generate_prodid
  @i_bookkey      INT,
  @i_printingkey  INT,
  @i_orglevelkey  INT,
  @i_orgentrykey  INT,
...
BEGIN
  ...
  ELSE
  BEGIN
    SELECT @v_orgentrykey = ISNULL(orgentrykey, 0)
    FROM bookorgentry
    WHERE bookkey = @i_bookkey AND orglevelkey = @i_orglevelkey
  END
...
grant execute on qtitle_run_generate_prodid to public
go