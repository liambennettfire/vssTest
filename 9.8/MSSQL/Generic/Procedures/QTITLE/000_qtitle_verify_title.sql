...
BEGIN
  ...
  SELECT @v_qsicode = qsicode, @v_StoredProcName = COALESCE(alternatedesc1,'')
    FROM gentables
   WHERE tableid = 556
  ...
  FROM gentables_ext
  WHERE tableid = 556 AND datacode=@i_verifytypecode
  
  IF @v_erroroutputind = 1 OR @v_qsicode = 3 BEGIN
