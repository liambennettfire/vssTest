...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
	    SELECT @v_paymenttype = datacode FROM gentables WHERE tableid = 635 AND qsicode = 1

        EXEC get_next_key @i_userid, @v_newpaymentkey OUTPUT

        INSERT INTO taqprojectpayments
          (paymentkey, taqprojectkey, paymenttype, datetypecode, dateoffsetcode, paymentamount, lastuserid, lastmaintdate )
        VALUES
          (@v_newpaymentkey, @i_new_projectkey, @v_paymenttype, @v_datetypecode, @v_dateoffsetcode, @v_amount, @i_userid, getdate())
    ...
    BEGIN
      ...
      DECLARE payments_cur CURSOR FOR
        SELECT paymentkey, taqprojectkey, paymenttype, datetypecode, dateoffsetcode, paymentamount, taqtaskkey, date, originaldate, 
		       reviseddate, note, payeecontactkey, pmtstatuscode, invoicenumber, invoicesent, checknumber, paymentmethodcode
        FROM taqprojectpayments
        WHERE taqprojectkey = @i_from_projectkey
      ...
      INTO @v_paymentkey, @v_taqprojectkey, @v_paymenttype, @v_datetypecode, @v_dateoffsetcode, @v_paymentamount, @v_taqtaskkey, @v_date, 
	       @v_originaldate, @v_reviseddate, @v_note, @v_payeecontactkey, @v_pmtstatuscode, @v_invoicenumber, @v_invoicesent, @v_checknumber, @v_paymentmethodcode
      ...
      BEGIN
        ...
        INSERT INTO taqprojectpayments
          (paymentkey, taqprojectkey, paymenttype, datetypecode, dateoffsetcode, paymentamount, taqtaskkey, date, originaldate, 
		   reviseddate, note, payeecontactkey, pmtstatuscode, invoicenumber, invoicesent, checknumber, lastuserid, lastmaintdate, paymentmethodcode )
        VALUES
          (@v_newpaymentkey, @i_new_projectkey, @v_paymenttype, @v_datetypecode, @v_dateoffsetcode, @v_paymentamount, @v_taqtaskkey, @v_date, 
	       @v_originaldate, @v_reviseddate, @v_note, @v_payeecontactkey, @v_pmtstatuscode, @v_invoicenumber, @v_invoicesent, @v_checknumber,  @i_userid, getdate(), @v_paymentmethodcode)
