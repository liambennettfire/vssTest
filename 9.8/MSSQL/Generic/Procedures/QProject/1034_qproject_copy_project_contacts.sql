...
END
	...
	begin
        ...
        FETCH NEXT FROM taqprojectcontactrole_iteration_cur INTO @v_rolecode, @v_activeind, @v_authortypecode, @v_primaryind, @v_ratetypecode, @v_workrate, @v_quantity, @v_shippingmethodcode, @v_globalcontactrelationshipkey, @v_indicator, @v_taqversionformatkey
    ...
    BEGIN                 
				...
				values(@newkey2, @newkey, @i_new_projectkey, @v_rolecode, @v_activeind,
					   ...
					   @v_shippingmethodcode, @v_globalcontactrelationshipkey, @v_indicator, @v_taqversionformatkey)
...
--			IF @v_indicator1 <> 1 OR NOT EXISTS(SELECT * FROM taqprojectcontactrole WHERE taqprojectkey = @i_new_projectkey AND rolecode = @v_rolecode) BEGIN 		 	                
			IF NOT EXISTS(SELECT * FROM taqprojectcontactrole WHERE taqprojectkey = @i_new_projectkey AND rolecode = @v_rolecode AND taqprojectcontactkey = @newkey AND COALESCE(taqversionformatkey, 0) = COALESCE(@v_taqversionformatkey, 0) AND COALESCE(shippingmethodcode, 0) = COALESCE(@v_shippingmethodcode, 0)) BEGIN 		 	                
				exec get_next_key @i_userid, @newkey2 output
				insert into taqprojectcontactrole
						(taqprojectcontactrolekey, taqprojectcontactkey, taqprojectkey, rolecode, activeind,
						authortypecode, primaryind, lastuserid, lastmaintdate, ratetypecode, workrate, quantity, 
