...
********************************************************************************************************************/
  ...
  DECLARE project_format_cur CURSOR FOR 
   SELECT COALESCE(f.taqprojectformatkey,0),f.seasoncode,f.seasonfirmind,f.mediatypecode,
        ...
        p.agehighupind, p.agehigh, p.gradelowupind, p.gradelow, p.gradehighupind, p.gradehigh, CASE WHEN @i_approvedbymasterind = 1 THEN f.propagatefromprimaryind ELSE @i_propagateind END 
     FROM taqproject p 
  ...
  INTO @taqprojectformatkey_var,@seasoncode_var,@seasonfirmind_var,@mediatypecode_var,
    ...
    @v_agehighupind, @v_agehigh, @v_gradelowupind, @v_gradelow, @v_gradehighupind, @v_gradehigh, @v_propagateind
  ...
  END
    ...
    IF @process_contracts_var = 1 OR @process_titles_var = 1 BEGIN
      ...
      ELSE BEGIN
        ...
        INSERT INTO book (bookkey,title,subtitle,shorttitle,standardind,workkey,linklevelcode,
                ...
                propagatefrombookkey,creationdate, titletypecode)
    ...
    IF @setuprequiredforsendtotmm_var = 2 BEGIN
      ...
      BEGIN
        ...
        EXEC qspec_apply_specificationtemplate @v_projectkey, @i_projectkey,
          @v_TaqProjectFormatKey, @v_itemtypecode_printing, @v_usageclasscode_printing, @lastuserid_var, 2, 0, @o_error_code OUTPUT, @o_error_desc OUTPUT    
    ...
    -- Case 10799 - If the client option 75, ԓetup required for Send to TMM processԮ is 0 (false), apply the default template 
