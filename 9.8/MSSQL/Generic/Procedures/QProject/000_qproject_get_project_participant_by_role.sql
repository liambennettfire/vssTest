...
*******************************************************************************/
  ...
  CREATE TABLE #participantcontactroleinfo (
   ...
   -- exec qutl_trace 'qproject_get_project_participant_by_role', '@i_projectkey', @i_projectkey, NULL,
...
ELSE
  ...
  t.taqversionformatkey,
   CASE WHEN COALESCE(t.taqversionformatkey, 0) > 0 THEN
        (SELECT f.description FROM taqversionformat f WHERE f.taqprojectformatkey = t.taqversionformatkey)
      ELSE ''
