...
** 04/27/2016 - Uday - Case 37340                
** 01/29/2018   Colman  Case 49071     Performance
*******************************************************************************/

  DECLARE
    @v_quote       VARCHAR(2),
    @v_bookkeylist VARCHAR(max),
    @v_sqlwith     VARCHAR(max),
    ...
    @v_sqlapply1   VARCHAR(2000),
    @v_sqlapply2   VARCHAR(2000),
  ...
  SET @o_error_desc = ''
  SET @v_bookkeylist = '0'
  IF LTRIM(RTRIM(ISNULL(@i_bookkeylist, ''))) <> ''
    SET @v_bookkeylist = @v_bookkeylist + ',' + @i_bookkeylist
  ...
  -- DECLARE @StartTime AS DATETIME, @ElapsedTime AS VARCHAR(100)
  -- SET @StartTime = GETDATE()

  -- ***** Build the dynamic SQL which will be used to retrieve tasks ***** --
      SET @v_sqlwith = '
;WITH CTE_Access (objectkey, accesscode, datacode)
AS
(
  SELECT objectkey, accesscode, datacode
  FROM dbo.qutl_get_gentables_access_table_multi(' + cast(@i_userkey as varchar) + ', ' + @v_quote + 'tasktracking' + @v_quote + ',323, ' + @v_quote + @v_bookkeylist + @v_quote + ', ' + cast(@i_printingnum as varchar) + ', 0)
)'      
  IF @i_dropdownuse = 1
    BEGIN
      -- For drop-downs, get distinct dates existing on this project/title
      SET @v_sqlselect1 = '
SELECT DISTINCT d.datetypecode,COALESCE(d.usedexclusivelybycsind,0) usedexclusivelybycsind,
CASE 
...
FROM taqprojecttask t
  ...
  LEFT OUTER JOIN CTE_Access a ON t.datetypecode = a.datacode AND ISNULL(t.bookkey,0) = a.objectkey'
      SET @v_sqlwhere1 = '
WHERE 1=1'
    END
  ELSE  
...
'SELECT CASE
WHEN d.datelabel IS NULL OR LTRIM(RTRIM(d.datelabel))=' + @v_quote + @v_quote + ' THEN d.description
ELSE d.datelabel END datelabel,
a.accesscode,
CASE WHEN t.bookkey > 0 THEN dbo.qutl_get_user_orgsecurity(' + cast(@i_userkey as varchar) + ',
  (SELECT TOP(1) b.orgentrykey
   FROM orglevel o
   LEFT OUTER JOIN bookorgentry b ON o.orglevelkey = b.orglevelkey AND b.bookkey = t.bookkey
   ORDER BY o.orglevelkey))
END userorgaccesscode,
...
CASE WHEN LEN(taqtasknote) > 6 THEN
  CAST(t.taqtasknote AS VARCHAR(6)) + ' + @v_quote + '...' + @v_quote + ' ELSE t.taqtasknote END AS note, 
t.activedate as origactivedate, 
t.originaldate as origorigdate,
...
ELSE ltrim(rtrim(COALESCE(dbo.get_gentables_desc(287,e.taqelementtypecode,null),'' ''))) 
END AS elementtypedesc,
...
dbo.qproject_taskoverride_exists_for_task(t.taqtaskkey) taskoverrideexistsfortask, d.triggerdateind '
...
a.accesscode,
CASE 
  WHEN t.bookkey > 0 THEN dbo.qutl_get_user_orgsecurity(' + cast(@i_userkey as varchar) + ', (
    SELECT TOP(1) b.orgentrykey
    FROM orglevel o 
    LEFT OUTER JOIN bookorgentry b ON o.orglevelkey = b.orglevelkey AND b.bookkey = t.bookkey 
    ORDER BY o.orglevelkey))
...
0 taskoverrideexistsfortask, d.triggerdateind '      
...
FROM taqprojecttask t
  ...
  LEFT OUTER JOIN (select distinct globalcontactkey, lastname from globalcontact) AS gc2 ON gc2.globalcontactkey = t.globalcontactkey2 '
      SET @v_sqlwhere2 = '
WHERE t.taqelementkey IS NULL AND tok.taqtaskkey IS NULL'
  ...
  BEGIN
    ...
    IF @v_alldatetypesind = 1 BEGIN
      ...
      SET @v_sqlselect1 = @v_sqlselect1 + ',COALESCE(d.sortorder, 999) AS sortorder, 0 as taskviewtriggerkey '
      SET @v_sqlselect2 = @v_sqlselect2 + ',COALESCE(d.sortorder, 999) AS sortorder, 0 as taskviewtriggerkey '
    ...
    ELSE BEGIN
      ...
      -- by view's sortorder
      SET @v_sqlselect1 = @v_sqlselect1 + ',v.sortorder, v.taskviewtriggerkey'
      ...
      SET @v_sqlselect2 = @v_sqlselect2 + ',v.sortorder, v.taskviewtriggerkey'
  ...
  IF @i_dropdownuse = 1 OR (@i_elementtype > 0 OR @i_taqelementkey > 0)
    SET @v_sqlstring = @v_sqlwith + @v_sqlselect1 + @v_sqlfrom1 + @v_sqlwhere1
  ELSE
    SET @v_sqlstring = @v_sqlwith + @v_sqlselect1 + @v_sqlfrom1 + @v_sqlwhere1 +
      ' UNION ' + @v_sqlselect2 + @v_sqlfrom2 + @v_sqlwhere2      
      
  -- exec qutl_trace 'qproject_get_project_taskview_dates',
  -- '@v_sqlstring', NULL, @v_sqlstring      
  PRINT @v_sqlwith
  PRINT @v_sqlselect1
  PRINT @v_sqlfrom1
  PRINT @v_sqlwhere1
  ...
  -- SELECT @ElapsedTime = CAST(CAST(GETDATE() - @StartTime AS TIME) AS VARCHAR(100))
  -- exec qutl_trace 'qproject_get_project_taskview_dates', 
    -- 'elapsed', NULL, @ElapsedTime,
    -- '@rowcount_var', @rowcount_var, NULL,
    -- '@v_sqlstring', NULL, @v_sqlstring
    -- '@i_projectkeylist', NULL, @i_projectkeylist,
    -- '@i_contactkeylist', NULL, @i_contactkeylist,
    -- '@i_bookkeylist', NULL, @i_bookkeylist,
    -- '@i_taskviewkey', @i_taskviewkey, NULL,
    -- '@i_printingnum', @i_printingnum, NULL,
    -- '@i_taqelementkey', @i_taqelementkey, NULL,
    -- '@i_datetypecode', @i_datetypecode, NULL
