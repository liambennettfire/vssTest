...
**  01/30/18  Colman    Case 49110 - Partial approval of Master Acquisitions
*******************************************************************************/
BEGIN
	...
	SET @v_partiallyapprovedstatus = NULL
	SELECT @v_partiallyapprovedstatus = datacode
	FROM gentables
	WHERE tableid = 522
	  AND qsicode = 24

	IF COALESCE(@v_partiallyapprovedstatus, 0) <= 0
	BEGIN
		SET @o_error_code = -1
	...
	IF @v_projectstatuscode <> @v_projapprovedstatus AND @v_projectstatuscode <> @v_partiallyapprovedstatus  --If Master Acq not already approved
	BEGIN
