  ...
  BEGIN
    ...
    IF @i_copyvalues = 2 BEGIN --copy everything
      INSERT INTO taqversioncosts
        ...
        versioncostsamount, compunitcost, unitcost, acceptgenerationind, printingnumber, lastuserid, lastmaintdate)
      SELECT
        @v_formatyearkey, acctgcode, plcalccostcode, versioncostsnote, plcalccostsubcode, 0,
        -- CASE WHEN plcalccostcode = 2 THEN 0 ELSE ROUND(versioncostsamount, @v_num_decprecision) END versioncostsamount,
        ROUND(versioncostsamount, @v_num_decprecision) versioncostsamount,
        CASE WHEN plcalccostcode = 1 THEN 0 ELSE compunitcost END compunitcost,
        unitcost, acceptgenerationind, printingnumber, @i_userid, getdate()
