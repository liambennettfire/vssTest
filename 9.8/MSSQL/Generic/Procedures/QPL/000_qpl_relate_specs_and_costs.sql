...
**  --------    -------   --------------------------------------
**	03/03/16    Kate      Update Production Qty on the current project version from related project's FG Qty when the
**                        Prod Qty Entry Type option is "From FG Qty" (see Case 36595)
**  05/31/17    Colman    45291 - When a PO is voided cannot create new PO 
**  08/12/17    Colman    46785 - Copy costs from related project to target
**  08/24/17    Colman    46785 - Printing costs appearing on Purchase Order and not actually copied
****************************************************************************************************************************/
...
DECLARE
  ...
  @v_taqversionformatyearkey INT

BEGIN
  
-- exec qutl_trace 'qpl_relate_specs_and_costs', 
  -- '@i_current_projkey', @i_current_projkey, NULL,
  -- '@i_related_projkey', @i_related_projkey, NULL,
  -- '@i_vendorkey', @i_vendorkey
  ...
  BEGIN
    ...
    -- Check the Prod Qty option on the current version.
    -- If Prod Qty entry type option is 4=Generate from FG Qty, update Production Quantity using the existing FG qty from related FG component record
    SELECT @v_prodqtyentrytype = prodqtyentrytypecode
    FROM taqversion
    WHERE taqprojectkey = @i_current_projkey AND plstagecode = @v_cur_stage AND taqversionkey = @v_cur_version
    
    IF @v_prodqtyentrytype = 4	--Generate from FG Qty
    BEGIN
      --Update current project's version Prod Qty with the Finished Good Quantity from the related project
      SELECT @v_prodqty = quantity
      FROM taqversionspeccategory
      WHERE taqversionformatkey = @v_rel_versionformatkey AND finishedgoodind = 1
      
      IF @v_prodqty > 0
        UPDATE taqversionformatyear
        SET quantity = @v_prodqty, lastuserid = 'FIREBRAND', lastmaintdate = GETDATE()
        WHERE taqprojectkey = @i_current_projkey 
          AND plstagecode = @v_cur_stage 
          AND taqversionkey = @v_cur_version 
          AND taqprojectformatkey = @v_cur_versionformatkey
          AND printingnumber = 1
    END
  END
  ...
  IF COALESCE(@i_vendorkey, 0) > 0 AND @v_allow_relation_of_components = 1 BEGIN
   ...
   SELECT @v_taqversionformatyearkey = taqversionformatyearkey
   FROM taqversionformatyear
   WHERE taqprojectformatkey = @v_cur_versionformatkey AND yearcode = 1
	
	--------------------------------------------------------
	--Find all available componets
	--------------------------------------------------------
  DECLARE
    @v_plstagecode INT,
  ...
  SELECT @v_plstagecode = dbo.qpl_get_most_recent_stage(@i_related_projkey)
  SELECT @v_selected_versionkey =  dbo.qpl_get_selected_version(@i_related_projkey)
    ...
    BEGIN
      ...
      -- exec qutl_trace 'qpl_relate_specs_and_costs', 
        -- '@v_newkey', @v_newkey, NULL,
        -- '@v_cur_versionformatkey', @v_cur_versionformatkey, NULL,
        -- 'relatedspeccategorykey', @v_speccategorykey, NULL,
        -- '@v_speccategorycode', @v_speccategorycode
      ...
      INSERT INTO taqversioncosts
        (taqversionformatyearkey, acctgcode, plcalccostcode, versioncostsnote, versioncostsamount, unitcost, 
        acceptgenerationind, templatechangedind, lastuserid, lastmaintdate, printingnumber, taqversionspeccategorykey, 
        plcalccostsubcode, compunitcost, pocostind)
      SELECT
        @v_taqversionformatyearkey, acctgcode, plcalccostcode, versioncostsnote, versioncostsamount, unitcost, 
        acceptgenerationind, templatechangedind, 'FIREBRAND', getdate(), 1, @v_newkey, 
        plcalccostsubcode, compunitcost, pocostind
      FROM taqversioncosts
      WHERE taqversionspeccategorykey = @v_speccategorykey
