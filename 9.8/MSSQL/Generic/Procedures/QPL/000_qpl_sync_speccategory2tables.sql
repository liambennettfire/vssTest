...
/****** Object:  StoredProcedure [dbo].[qpl_sync_speccategory2tables]    Script Date: 05/15/2015 10:59:38 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[qpl_sync_speccategory2tables]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[qpl_sync_speccategory2tables]
GO
...
CREATE procedure [dbo].[qpl_sync_speccategory2tables] (@i_taqversionspecategorykey int, @i_taqprojectkey int,@i_taqversionkey int,  @v_userid varchar(50))  
AS
BEGIN
	DECLARE 
	...
	--BL 46313 - removed booklock issue
	...
	select @i_selectedversionkey = [dbo].[qpl_get_selected_version] (@i_taqprojectkey)
	...
	BEGIN	
		...
		select @i_itemtype = searchitemcode from coreprojectinfo where projectkey = @i_taqprojectkey
		select @i_usageclass = usageclasscode from coreprojectinfo where projectkey = @i_taqprojectkey
		
		select @i_syncspecs = COUNT(*) from qsiconfigspecsync where usageclass = @i_usageclass and itemtype = @i_itemtype 
		and syncfromspecsind=1 and activeind=1 
		---and tablename in ('printing','bindingspecs') 
		and specitemtype in ('C','CK') --component level fields
		...
		BEGIN --2
			...
			from taqprojecttitle tpt where tpt.taqprojectkey = @i_taqprojectkey and projectrolecode = @v_printing_projectrole and titlerolecode = @v_printing_title_titlerole -- from the printing project, this may need to change
			
			--get the taqprojectformatkey
			select @i_versionformatkey =taqprojectformatkey from taqversionformat where taqprojectkey = @i_taqprojectkey and taqversionkey=@i_selectedversionkey
			...
			select qsiconfigspecsynckey from qsiconfigspecsync where usageclass = @i_usageclass and itemtype = @i_itemtype 
			and syncfromspecsind=1 and activeind=1 
			--and tablename in ('printing','bindingspecs') 
			and specitemtype in ('C','CK') --component level fields
			...
			BEGIN--3
			 ...
			 WHERE rowid = @i_rowcount			
				...
				*/					
				...
				--get the taqversionspecategorykey based on the @i_specitemcategory and @i_taqprojectkey and @i_taqversionkey
				--NOTE:  this will not do multiples of the same component, that will need to be in the multi row style used for inserts and papers, for now this is to solve the main
				--print qty issues
				
				--print '@i_qsiconfigspecsynckey'
				--print @i_qsiconfigspecsynckey
				
				IF coalesce(@i_taqversionspecategorykey,0) = 0
				begin
					select @i_taqversionspecategorykey = min(taqversionspecategorykey) from taqversionspeccategory where taqprojectkey = @i_taqprojectkey and taqversionkey=@i_taqversionkey
					and itemcategorycode = @i_specitemcategory
				end
				
				--print '@i_taqversionspecategorykey'
				--print @i_taqversionspecategorykey
							
				--get the taqversionspecitem values 
				select @i_qtyvalue = tvs.quantity,
				@v_descvalue = tvs.speccategorydescription,
				@i_detailvalue = tvs.vendorcontactkey
				from taqversionspeccategory tvs where taqversionspecategorykey = @i_taqversionspecategorykey
				...
				BEGIN --9						
						...
						IF coalesce(@v_specitemtype,'') in ('C') and coalesce(@v_datatype,'') = 'int'  -- Quantity component 
						BEGIN--13
						...
						IF coalesce(@v_specitemtype,'') in ('CK') and coalesce(@v_datatype,'') = 'int'  -- component vendor
						BEGIN--13
							select @i_targetdetailvalue = @v_targetvalue
							
							IF isnumeric(@i_targetdetailvalue)=1 
								select @i_targetdetailvalue = CAST(@i_targetdetailvalue as INT)
								
							ELSE select @v_errordesc = 'target detail value is not numeric'
						...
						IF coalesce(@v_specitemtype,'')  in ('CD') and coalesce(@v_datatype,'') = 'varchar'  -- description and component description
						BEGIN --16
						...
						IF coalesce(@v_specitemtype,'')='C' --component qty
						begin --21
						
										
						--compare target to target value, if different, update
							IF coalesce(@i_qtyvalue,-1) <> coalesce(@i_targetqtyvalue,-1)
						...
						IF coalesce(@i_detailvalue,-1) <> -1 and coalesce(@v_specitemtype,'')='CK' --component vendor
						begin --22
						--compare target to target value, if different, update
							IF coalesce(@i_detailvalue,0) <> coalesce(@i_targetdetailvalue,0)
								exec dbo.qpl_sync_specitems2tables_update @v_tablename,@v_tablenamekeycode1,@v_tablenamekeycode2,@v_tablenamekeycode3,@i_key1,@i_key2,@i_key3,@v_column,@i_detailvalue,null,null,null,null,null,null,@v_specitemtype,@i_qtydecimalind,@v_userid	
						...
						IF coalesce(@i_targetuomvalue,-1) <> coalesce(@i_uomvalue,-1)
						begin --23
						--compare target to target value, if different, update
						...
						IF coalesce(@v_descvalue,'zemptyz') <> 'zemptyz' and coalesce(@v_specitemtype,'')='D'
						begin --25
						-- trim new source value down to size so can compare to existing table value, determine the target column length
						select @i_targetlength = character_maximum_length from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = @v_tablename and COLUMN_NAME = @v_column
...
GRANT EXEC on dbo.qpl_sync_speccategory2tables to public
go
