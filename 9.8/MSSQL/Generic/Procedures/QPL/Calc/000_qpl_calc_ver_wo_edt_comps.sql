if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc027') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc027
GO

if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_ver_wo_edt_comps') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_ver_wo_edt_comps
GO

CREATE PROCEDURE qpl_calc_ver_wo_edt_comps (  
  @i_projectkey INT,
  @i_plstage    INT,
  @i_plversion  INT,  
...
GRANT EXEC ON qpl_calc_ver_wo_edt_comps TO PUBLIC
GO
