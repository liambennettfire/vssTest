...
/****** Object:  StoredProcedure [dbo].[qpl_sync_tables2specitems]    Script Date: 7/24/2017 8:50:13 AM ******/
SET ANSI_NULLS ON
GO
...
/******************************************************************************
**  Name: qpl_sync_tables2specitems
**  Desc: 
**  Auth: 
**  Date: 
*******************************************************************************
...
**  07/12/2017   UK          Case 45420
**  07/25/2017   BL	     Case 46453	 
*******************************************************************************/
 ...
 BEGIN--2   
  ...
  BEGIN--3  
   ...
   BEGIN--6  
    ...
    IF @v_debug = 'Y'  
     begin  
      print '@i_taqversionspeccategorykey --this should be zero for binding'   
    ...
    where taqprojectkey=@i_projectkey and taqversionkey=@i_selectedversionkey and plstagecode=@i_mostrecentplstagecode and taqprojectformatkey = @i_versionformatkey  
    ...
    BEGIN --6.75  
     ...
     select @i_versionformatyearkey,@i_projectkey,@i_mostrecentplstagecode,@i_selectedversionkey,@i_versionformatkey,1,c.printingnum,null,1,null,@v_userid, getdate(),0   
    ...
    IF coalesce(@v_specitemtype,'') <>'' and coalesce(@i_exceptioncode,0) not in (2,3)  -- papers and inserts  
     BEGIN --9        
      ...
      IF coalesce(@i_bestid,0)<>0  
       ...
       /*Case 46453	 */  
	  IF coalesce(@v_sourcevalue,'')='' and @v_datatype <> 'decimal' and @v_userid='FBTINITCONV'  
	  GOTO blankintconv

	  IF coalesce(@i_sourcedecimalvalue,0)=0 and @v_datatype = 'decimal' and @v_userid='FBTINITCONV'  
	  GOTO blankintconv
      ...
      Begin  
        ...
        print Cast(coalesce(@i_taqversionspecitemkey,-1) as varchar(20))  
        print '@i_sourcedetailvalue'    
        print @i_sourcedetailvalue  
        print '@i_sourcedetail2value'    
        print @i_sourcedetail2value  
        print '@i_sourceqtyvalue'  
        print Cast(coalesce(@i_sourceqtyvalue,-1) as varchar(20))  
        print '@i_sourcedecimalvalue'  
        print Cast(coalesce(@i_sourcedecimalvalue,-1) as varchar(20))  
        print '@v_sourcedescvalue'    
        print @v_sourcedescvalue  
        print '@v_sourcedesc2value'    
        print @v_sourcedesc2value  
      ...
      IF coalesce(@i_taqversionspecitemkey,0) =0 and coalesce(@v_specitemtype,'') <> 'U'  
      /*  
      (coalesce(@i_sourcedetailvalue ,-1)<>-1 or coalesce(@i_sourcedetail2value ,-1)<>-1   
      or coalesce(@i_sourceqtyvalue ,-1)<>-1 or coalesce(@i_sourcedecimalvalue ,-1)<>-1   
      ...
      */  
      ...
      IF @i_specitemcategory = 1 and coalesce(@i_taqversionspecitemkey,0) =0   
       BEGIN   
        ...
        IF @v_debug='Y'  
         begin  
          print 'Inside summary component insert'  
         end  
        exec dbo.get_next_key @v_userid,@i_taqversionspecitemkey OUTPUT  
      ...
      BEGIN  --19  
        ...
        IF coalesce(@i_sourceuomvalue,-1) <> -1  
         ...
         --compare source to target value, if different, update  
        IF  coalesce(@v_specitemtype,'') ='D2' and coalesce(@v_desc2value,'zemptyz') <> coalesce(@v_sourcedesc2value,'zemptyz')  
        begin --24  
           update taqversionspecitems  
        ...
        IF  coalesce(@v_specitemtype,'') ='D' and coalesce(@v_descvalue,'zemptyz') <> coalesce(@v_sourcedescvalue,'zemptyz')  
        begin --25            
           update taqversionspecitems  
        ...
        IF coalesce(@v_specitemtype,'') = 'CD' --,'CD') --do the component update for vendorkey and description  
         BEGIN--26  
