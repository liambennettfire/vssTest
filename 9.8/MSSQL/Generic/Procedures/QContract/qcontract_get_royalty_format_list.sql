...
BEGIN
  ...
  WHERE taqprojectkey = @i_projectkey AND mediacode = 0 AND formatcode = 0 AND saleschannelcode > 0
  ...
  SELECT @v_count = COUNT(*) 
  FROM taqprojectroyalty 
  WHERE taqprojectkey = @i_projectkey AND mediacode = 0
  ...
  BEGIN
    ...
    VALUES
      (@v_royaltykey, @i_projectkey, 0, 0, 0, 1, 'INITIAL', getdate())
  ...
  SELECT DISTINCT r.mediacode, r.formatcode, p.avgroyaltyenteredind, 
    CASE
