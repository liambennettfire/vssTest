if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[SWAG_daterange_sp]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[SWAG_daterange_sp]
GO


create PROCEDURE [dbo].[SWAG_daterange_sp]
...
<Firebrand>
  ...
  (Select content AS [text()]
    ...
    Where lastmaintdate>=@d_FromDate
      and lastmaintdate<@d_ToDate+1
...
grant execute on dbo.SWAG_daterange_sp to public
GO
