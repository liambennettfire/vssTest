...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[imp_300010022001]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[imp_300010022001]
GO

CREATE PROCEDURE dbo.imp_300010022001 
...
BEGIN
  ...
  SET @v_errcode = 1
  SET @v_errmsg = 'Replaces ISBN'
  SET @v_bookkey = dbo.resolve_keyset(@i_titlekeyset,1)
  set @v_associationtypecode = 4
  set @v_associationtypesubcode = 3
  ...
  if @v_count=0
    begin
      --  ?? find bookkey,title,authorname,sortoder ?? --
      ...
      SET @v_errmsg = 'Replaces ISBN added'
    end
  else
    begin
        ...
        begin
          update associatedtitles
            ...
            where bookkey=@v_bookkey
              ...
              and associationtypesubcode=@v_associationtypesubcode
          SET @o_writehistoryind = 1
          SET @v_errmsg = 'Replaces ISBN updated'
        end
      else
        begin
          SET @v_errmsg = 'Replaces ISBN unchanged'
        end
    end
...
GRANT EXECUTE ON dbo.[imp_300010022001] to PUBLIC 
GO
