/******************************************************************************
**  Name: imp_100013010001
**  Desc: IKE US Retail Budget Price break out 
**  Auth: Bennett     
**  Date: 5/9/2016
*******************************************************************************
**  Change History
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[imp_100013010001]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[imp_100013010001]
GO

CREATE PROCEDURE dbo.imp_100013010001 
...
/* US Retail Budget Price break out */
...
BEGIN
	SET @v_errcode = 0
	SET @v_errlevel = 0
	SET @v_msg = 'US Price Budgeted expanded'
	...
	WHERE elementkey = 100013010

	SELECT @v_pricetype = g.datadesc
	FROM imp_element_defs e, gentables g
	WHERE g.tableid = 306 and e.datacode = g.datacode and e.elementkey = 100013010	


	INSERT INTO imp_batch_detail(batchkey,row_id,elementkey,elementseq,originalvalue,lastuserid,lastmaintdate)
    	VALUES (@i_batchkey,@i_row,100013022,@v_new_seq,@v_new_value ,'loader_rule_100013009001',getdate())
...
GRANT EXECUTE ON dbo.[imp_100013010001] to PUBLIC 
GO
