...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[imp_300028200001]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[imp_300028200001]
...
CREATE PROCEDURE [dbo].[imp_300028200001] 
  @i_batch int, 
  @i_row int , 
  @i_dmlkey bigint, 
...
BEGIN
  ...
  if @v_count=1
    begin
      select @v_old_value=floatvalue
        from bookmisc
        where bookkey=@v_bookkey
          and misckey=@v_misckey
      IF @v_old_value<>cast(@v_elementval as float) OR @v_old_value IS null
        BEGIN
          update bookmisc 
            set
              floatvalue=@v_elementval,
              lastuserid=@i_userid,
              lastmaintdate=getdate()
            where bookkey=@v_bookkey
              and misckey=@v_misckey
          SET @v_errmsg='Misc float value updated'
    ...
    begin
      ...
      SET @v_errmsg='Misc float value inserted'
 ...
 if @o_writehistoryind = 1
    begin
      ...
      exec qtitle_update_titlehistory 
        'bookmisc','floatvalue',@v_bookkey,@v_printingkey,null,
