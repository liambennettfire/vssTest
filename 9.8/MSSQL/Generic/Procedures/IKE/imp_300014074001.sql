/******************************************************************************
**  Name: imp_300014074001
**  Desc: IKE bookdetail.csapprovalcode
**  Auth: Bennett     
**  Date: 5/9/2016
*******************************************************************************
**  Change History
...
IF EXISTS (
		SELECT *
		FROM dbo.sysobjects
		WHERE id = object_id(N'[dbo].[imp_300014074001]')
			AND OBJECTPROPERTY(id, N'IsProcedure') = 1
		)
	DROP PROCEDURE [dbo].[imp_300014074001]
GO

CREATE PROCEDURE dbo.imp_300014074001 
	@i_batch INT
	,@i_row INT
	,@i_dmlkey BIGINT
...
BEGIN
	...
	SET @DEBUG = 0
	IF @DEBUG <> 0 PRINT 'dbo.imp_300014074001'
	...
	BEGIN
		BEGIN TRY
			...
			IF NOT EXISTS (SELECT * FROM bookdetail WHERE bookkey=@v_bookkey)
			BEGIN
				SET @v_errmsg = 'The bookkey ' + CAST(@v_bookkey as varchar(max)) + ' does not exist'
				SET @v_errcode = 2
			END ELSE BEGIN
				SET @v_errmsg = 'bookdetail.csapprovalcode was updated'
				
				UPDATE bookdetail 
				SET csapprovalcode = CAST(@v_elementval as int) 
	...
	END ELSE BEGIN
		SET @v_errmsg='Bookdetail.csapprovalcode was not updated: csapprovalcode was not found in the source.'
	END
	IF @DEBUG <> 0 PRINT @v_errmsg
