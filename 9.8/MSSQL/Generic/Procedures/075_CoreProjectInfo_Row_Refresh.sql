...
/******************************************************************************
**  Name: CoreProjectInfo_Row_Refresh
**  Desc: Updates coreprojectinfo table from taqproject and related tables
**
*******************************************************************************
**  Change History
*******************************************************************************
...
**  -----------    -----------    -------------------------------------------
**  11/22/2016     Colman         Case 41855 Updating season for (non-taq) projects does not populate coreprojectinfo
*******************************************************************************/
...
DECLARE 
  ...
  @v_primaryid1 VARCHAR(50),
  @v_primaryid2 VARCHAR(50),
  @v_keyrelatedprojectkey INT,
  @v_keyrelatedprojecttitle VARCHAR(255),
  @v_keyprojectrelationshipcode INT
...
if @i_projectkey is null begin
  ...
  select @v_usageclasscodedesc = datadesc, @v_usageclassqsicode = qsicode
    from subgentables
    where tableid=550
  ...
  -- get season and discount type from primary format for TAQ projects
  IF @v_usageclassqsicode = 1
  BEGIN
    SET @v_seasoncode = 0
    SELECT @v_seasoncode = COALESCE(seasoncode, 0), @v_discounttypecode = discountcode
  ...
  -- Get primary productnumbers
  DECLARE productnumber_cur CURSOR FOR
  SELECT TOP(2) productnumber
  FROM taqproductnumbers n
  JOIN gentables g 
    ON g.tableid = 594 AND g.datacode = n.productidcode
  WHERE taqprojectkey = @i_projectkey
  ORDER BY n.sortorder, g.sortorder

  OPEN productnumber_cur
  FETCH productnumber_cur INTO @v_primaryid1
  FETCH productnumber_cur INTO @v_primaryid2

  CLOSE productnumber_cur
  DEALLOCATE productnumber_cur
  
  SET @v_keyprojectrelationshipcode = 0
  
  SELECT @v_keyprojectrelationshipcode = code2 FROM gentablesrelationshipdetail WHERE gentablesrelationshipkey = 36 AND code1 = @v_searchitemcode AND subcode1 = @v_usageclasscode
  IF @v_keyprojectrelationshipcode <> 0
  BEGIN
    SELECT TOP(1) @v_keyrelatedprojectkey = r.taqprojectkey2, @v_keyrelatedprojecttitle = p.taqprojecttitle 
    FROM taqprojectrelationship r 
    JOIN taqproject p ON p.taqprojectkey = r.taqprojectkey2
    WHERE r.taqprojectkey1 = @i_projectkey AND r.relationshipcode2 = @v_keyprojectrelationshipcode
  END
  ...
  if @v_rows = 0 or @v_rows is null
    insert into coreprojectinfo
      (projectkey,
       ...
       primaryid1,
       primaryid2,
       keyrelatedprojectkey,
       keyrelatedprojecttitle)
      values
      (@i_Projectkey,
       @v_title,
       @v_type,
       ...
       @v_primaryid1,
       @v_primaryid2,
       @v_keyrelatedprojectkey,
       @v_keyrelatedprojecttitle)
  ...
  if @v_rows = 1
    update coreProjectinfo
       set
         ...
         primaryid1 = @v_primaryid1,
         primaryid2 = @v_primaryid2,
         keyrelatedprojectkey = @v_keyrelatedprojectkey,
         keyrelatedprojecttitle = @v_keyrelatedprojecttitle
    where projectkey=@i_projectkey
