...
BEGIN
  ...
  IF @i_eanprefix = '979'
  BEGIN
    IF LEFT(@v_tempstring, 1) <> '1'
    BEGIN
      SET @o_error_code = -3
      SET @o_error_desc = @o_error_desc + 'Language/Country Group (before hyphen) for prefix 979 must begin with 1.'
      GOTO RETURN_ERROR
    END
    ELSE
  ...
  BEGIN  
    IF @i_eanprefix = '979'
      SET @o_error_desc = 'Language/Country Group (before hyphen) must be 2 digits for ISBN Prefix 979.'
    ELSE IF @v_tempint < 60000
