...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[feed_out_primac_data_incr]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[feed_out_primac_data_incr]
GO


create proc dbo.feed_out_primac_data_incr
@in_estkey int, @in_versionkey int
...
where  t.estkey=i.estkey
	and i.estkey = @in_estkey
	and i.versionkey = @in_versionkey 
...
if @i_bookkey <> 0 /*no titles  not likely though*/
 begin	
  ...
  begin
	delete from FEEDOUTPRIMACDATA where estkey = @feedout_estkey and versionkey = @feedout_versionkey
 end
...
GRANT  EXECUTE  ON [dbo].[feed_out_primac_data_incr]  TO [public]
GO
