...
BEGIN
  ...
  SET @v_col_isrange = 0
    ...
    SET @v_rowitem_desc = ltrim(rtrim(dbo.get_subgentables_desc(616,@v_rowitemcategorycode,@v_rowitemcode,'long')))
		...
		SELECT @v_row_int = COALESCE(dbo.qscale_find_specification_value(@i_taqversionformatyearkey,@SpecItemsByPrintingView,@v_rowitemcategorycode,@v_rowitemcode),0),
			   @v_row_numeric = COALESCE(dbo.qscale_find_specification_value(@i_taqversionformatyearkey,@SpecItemsByPrintingView,@v_rowitemcategorycode,@v_rowitemcode),0)
...
--print '@v_col_itemcode= ' + cast(@v_col_itemcode as varchar)
			...
			SELECT @v_col_int = COALESCE(dbo.qscale_find_specification_value(@i_taqversionformatyearkey,@SpecItemsByPrintingView,@v_col_itemcategorycode,@v_col_itemcode),0),
			   @v_col_numeric = COALESCE(dbo.qscale_find_specification_value(@i_taqversionformatyearkey,@SpecItemsByPrintingView,@v_col_itemcategorycode,@v_col_itemcode),0)
...
--print '@v_taqdetailscalekey = ' + cast(@v_taqdetailscalekey as varchar)
      ...
      ELSE BEGIN  -- some or all rows have value for thresholdspeccategorycode
        ...
        IF (@v_count4 = 1) OR (@v_count4 > 1 AND @v_values_same = 'Y') BEGIN  
          SELECT @v_thresholdvalue_int = COALESCE(dbo.qscale_find_specification_value(@i_taqversionformatyearkey,@SpecItemsByPrintingView,@v_saved_thresholdspeccategorycode,@v_saved_thresholdspecitemcode),0),
			      @v_thresholdvalue_numeric = COALESCE(dbo.qscale_find_specification_value(@i_taqversionformatyearkey,@SpecItemsByPrintingView,@v_saved_thresholdspeccategorycode,@v_saved_thresholdspecitemcode),0)
          ...
          ELSE BEGIN --(@v_count4 = 1) OR (@v_count4 > 1 AND @v_values_same = 'Y') AND threshold value is NOT NULL 
              ...
              WHILE (@@FETCH_STATUS=0)
				      BEGIN
                SELECT @v_thresholdvalue_int = COALESCE(dbo.qscale_find_specification_value(@i_taqversionformatyearkey,@SpecItemsByPrintingView,@v_thresholdspeccategorycode,@v_thresholdspecitemcode),0),
			          @v_thresholdvalue_numeric = COALESCE(dbo.qscale_find_specification_value(@i_taqversionformatyearkey,@SpecItemsByPrintingView,@v_thresholdspeccategorycode,@v_thresholdspecitemcode),0)
