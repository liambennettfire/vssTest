...
*   --------     --------    -------------------------------------------
**  05/09/2016   UK          Case 37111
**  05/11/2016   UK          Case 37111 - Task 001
*******************************************************************************/
BEGIN
  DECLARE @v_bookkey INT,
          ...
          @rowcount_var INT,
          @v_itemtypecode INT,
          @v_itemtypecode_work INT,
          @v_workkey INT
          
  SET @v_itemtypecode = 0   
  SET @v_workkey = 0  
  
  SELECT @v_itemtypecode_work = datacode
  FROM gentables WHERE tableid = 550 AND qsicode = 9     
  ...
  IF @v_projectkey > 0 BEGIN
	SELECT @v_itemtypecode = searchitemcode 
	FROM taqproject WHERE taqprojectkey = @v_projectkey
	
	IF @v_itemtypecode_work = @v_itemtypecode BEGIN
		SELECT @v_workkey = workkey
		FROM taqproject 
		WHERE taqprojectkey = @v_projectkey
		
		SET @v_bookkey = @v_workkey
	END	
  END    
