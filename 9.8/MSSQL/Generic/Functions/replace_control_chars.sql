...
IF EXISTS (SELECT * FROM sysobjects WHERE id = object_id('dbo.replace_control_chars') AND type = 'FN')
	DROP function [dbo].[replace_control_chars]
GO

create FUNCTION [dbo].[replace_control_chars]
...
BEGIN 
	...
	set @o_string = replace(@o_string,@v_newline,' ')
	set @o_string = replace(@o_string,CHAR(9),'  ') --horizontal -tab
