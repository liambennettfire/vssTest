...
IF EXISTS (
	SELECT *
	FROM dbo.sysobjects
	WHERE id = Object_id('dbo.udf_dateaddworkdays') AND type = 'FN'
	)
BEGIN
	DROP FUNCTION dbo.udf_dateaddworkdays
END
...
CREATE FUNCTION dbo.udf_dateaddworkdays (
	@date DATETIME
	,@workdays INT
	)
RETURNS DATETIME
