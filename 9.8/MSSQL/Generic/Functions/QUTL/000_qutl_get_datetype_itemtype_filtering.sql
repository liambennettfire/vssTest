...
**    --------     --------        --------------------------------------------------------------------------------------
**    01/29/18     Colman          49071 - Performance
*****************************************************************************************************************************/
...
BEGIN
  ...
  BEGIN
    ...
    ;WITH CTE_Access (accesscode, datacode)
    AS
    (
      SELECT accesscode, datacode
      FROM dbo.qutl_get_gentables_access_table(@i_userkey, @i_windowname, @v_tableid, @i_bookkey, @i_printingkey, 0)
    )      
    ...
    FROM gentablesdesc gd, datetype d, gentablesitemtype i
      LEFT OUTER JOIN CTE_Access a ON i.datacode = a.datacode
