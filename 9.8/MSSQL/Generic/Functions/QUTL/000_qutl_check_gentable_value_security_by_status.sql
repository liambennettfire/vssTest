...
**    --------    --------     -------   --------------------------------------
**    12/13/17    Colman       48339     TMM Dates not editable when not printing one
...
BEGIN 
  DECLARE @error_var    INT,
          ...
          @v_found_security_by_status tinyint
  ...
  SET @v_found_security_by_status = 0
  ...
  WHILE @@fetch_status = 0 BEGIN                   
    ...
    IF @@ROWCOUNT > 0
    BEGIN
      SET @v_found_security_by_status = 1
    END
    ELSE BEGIN
      -- group
      SELECT @object_accessind_var = accessind   
    ...
    IF @v_found_security_by_status = 1
      BREAK
  ...
  IF @v_found_security_by_status = 0 
  BEGIN
