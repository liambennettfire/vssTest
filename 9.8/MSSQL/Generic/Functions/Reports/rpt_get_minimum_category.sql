...
/****** Object:  UserDefinedFunction [dbo].[rpt_get_minimum_category]    Script Date: 12/09/2014 09:54:20 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[rpt_get_minimum_category]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[rpt_get_minimum_category]
GO
...
CREATE function [dbo].[rpt_get_minimum_category](@i_bookkey int, @i_category_tableid int,@i_order int,@i_DescType varchar(10))
returns varchar(255)
as begin
...
SELECT @i_categorycode = categorycode,@i_categorysubcode = categorysubcode, @i_categorysub2code = categorysub2code FROM (
  ...
  FROM booksubjectcategory c where @i_bookkey = bookkey and @i_category_tableid = categorytableid 
) AS foo
...
grant all on [rpt_get_minimum_category] to public