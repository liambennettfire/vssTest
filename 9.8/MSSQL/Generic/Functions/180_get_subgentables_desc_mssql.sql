if exists (select * from dbo.sysobjects where id = object_id(N'dbo.get_subgentables_desc') and xtype in (N'FN', N'IF', N'TF'))
drop function dbo.get_subgentables_desc
GO

CREATE FUNCTION get_subgentables_desc
...
BEGIN 
  ...
  IF lower(rtrim(ltrim(@i_desctype))) = 'short' BEGIN
    -- get datadescshort - get datadesc if short not filled in
...
GRANT EXEC ON dbo.get_subgentables_desc TO public
GO
