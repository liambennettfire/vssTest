USE [APH_ECF]
GO
/****** Object:  StoredProcedure [dbo].[ProductSearchByAdvancedFilterNew]    Script Date: 10/14/2009 13:21:53 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[ProductSearchByAdvancedFilterNew]
	@search nvarchar(500),
	@PriceLow money = NULL,
		...
		WHILE (@@fetch_status = 0) BEGIN
			...
			WHILE (@@fetch_status = 0) 
				...
				IF LEN(@key) > 0 AND LEN(@operator) > 0 AND LEN(@value) > 0 AND dbo.ColumnAlreadyExists(@TableName, @key) = 1 BEGIN
					IF LEN(@wherest) > 0 
						SET @wherest = @wherest + ' AND '
						IF ( @key = 'SKU_Full_Title' ) -- search sku_fulltitle and Sku_subtitle
							BEGIN
								SET @wherest = @wherest + '( (MCT.[' + @key + '] ' + @operator + ' ' + @value + ') OR ( MCT.[SKU_Subtitle] ' + @operator + ' ' + @value + ') )'				
							END
						ELSE IF ( @key = 'PubDate' )  -- search pubdate and copyrightyear
							BEGIN
								SET @wherest = @wherest + '( ( datepart(year, MCT.PubDate) LIKE ' + @value + ') OR ( MCT.[Copyrightyear] ' + @operator + ' ' + @value + ') )'				
							END
						ELSE
							BEGIN
								SET @wherest = @wherest + '(MCT.[' + @key + '] ' + @operator + ' ' + @value + ')'
