if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc142') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc142
GO
 
if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_stg_sales_comm_KAP') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_stg_sales_comm_KAP
GO

CREATE PROCEDURE qpl_calc_stg_sales_comm_KAP (  
  @i_projectkey INT,
  @i_plstage    INT,
...
/******************************************************************************************
**  Name: qpl_calc_stg_sales_comm_KAP
**  Desc: Kaplan Item 142 - Stage/Sales Commission.
**
**  Auth: Kate
...
BEGIN
  ...
  ELSE
    BEGIN       
      -- Version/Sales Commission
      EXEC qpl_calc_ver_sales_comm_KAP @i_projectkey, @i_plstage, @v_selected_versionkey, @o_result OUTPUT
    END
  
END
GO

GRANT EXEC ON qpl_calc_stg_sales_comm_KAP TO PUBLIC
GO
