...
--			print 'qweb_ecf_Category_Insert_MajorPressSubject_Category COMPLETE'
	...
	SET @img_folder = '\\merv\mediachase\KP_images\covers\'
...
--			OR exists (Select * FROM cbd..bookfamily where childbookkey = @i_bookkey and relationcode = 20005) --Journal Special Issue
--
--		begin	
	...
	-- authors should not be linked to other authors
       ...
       and relatedproductid in (select objectid from productex_contributors)
		...
		where code in (select distinct(cast(authorkey as varchar))
                    from cbd..author 
                    where authorkey not in (select globalcontactkey from cbd..globalcontactcategory
		...
		(select globalcontactkey from cbd..globalcontact
		where activeind = 0)                               
