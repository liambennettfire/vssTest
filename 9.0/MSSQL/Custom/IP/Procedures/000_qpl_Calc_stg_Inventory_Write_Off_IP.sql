...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[qpl_Calc_stg_Inventory_Write_Off_IP]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[qpl_Calc_stg_Inventory_Write_Off_IP]
GO
...
CREATE PROCEDURE [dbo].[qpl_Calc_stg_Inventory_Write_Off_IP] (  
  @i_projectkey INT,
  @i_plstage    INT,
...
/******************************************************************************************
**  Name: qpl_Calc_stg_Inventory_Write_Off_IP
...
BEGIN
  ...
  EXEC qpl_calc_stg_tot_prod_unt @i_projectkey, @i_plstage, @v_total_Production_Units OUTPUT
  
  If @v_total_Production_Units is null
  SET @v_total_Production_Units=0

    -- Version - TOTAL Net Units
  EXEC qpl_calc_ver_tot_net_unt @i_projectkey, @i_plstage, 0, @v_Total_net_Units OUTPUT

  IF @v_Total_net_Units is null
  SET @v_Total_net_Units=0
  
	-- Version - TOTAL Comp Units
  EXEC qpl_calc_stg_wo_edt_comps_units @i_projectkey, @i_plstage, @v_Total_Comp_Units OUTPUT
   IF @v_Total_Comp_Units is null
   SET @v_Total_Comp_Units=0
  
  
  -- Version - Prepress & PPBF
  EXEC qpl_calc_stage @i_projectkey, @i_plstage, 'PRODEXP', NULL, 0, @v_Prepress_PPBF OUTPUT
...
GRANT ALL ON qpl_Calc_stg_Inventory_Write_Off_IP TO PUBLIC
