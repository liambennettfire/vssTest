...
/****** Object:  StoredProcedure [dbo].[qean_insert_ean]    Script Date: 12/09/2014 15:48:13 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[qean_insert_ean]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[qean_insert_ean]
GO
...
/**************************************************************************************************
**  Name: qean_insert_ean - CUSTOM for IHPG
...
BEGIN
  ...
  BEGIN
	...
	FROM bookorgentry
	WHERE bookkey = @i_bookkey AND orglevelkey = (SELECT filterorglevelkey FROM filterorglevel WHERE filterkey=18)
	
    EXEC qean_after_itemnumber_update_multiorg @v_orgentrykey, @v_new_itemnumber, @o_error_code OUTPUT, @o_error_desc OUTPUT
  END 
...
Grant All on qean_insert_ean to Public