/****** Object:  StoredProcedure [dbo].[qweb_ecf_Insert_SKUs]    Script Date: 04/09/2009 14:53:56 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
		...
		and b.bookkey = @i_bookkey
				...
				Select @v_title = '('+ UNL.dbo.qweb_get_ISBN(@i_bookkey,'16') + ') ' + Substring (UNL.dbo.qweb_get_Title				(@i_bookkey,'f'),1,84)
				...
				--Select @m_usretailprice = UNL.dbo.qweb_get_BestUSPrice(@i_bookkey,8)
				...
				If exists (Select * 
					...
					begin							 
					Select @m_usretailprice = UNL.dbo.qweb_get_BestUSPrice(@i_bookkey,10)
					end
				Else
					begin
					Select @m_usretailprice = UNL.dbo.qweb_get_BestUSPrice(@i_bookkey,8)
