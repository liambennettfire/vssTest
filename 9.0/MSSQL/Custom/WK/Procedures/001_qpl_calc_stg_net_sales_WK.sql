if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_stg_net_sales_WK') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_stg_net_sales_WK
GO

CREATE PROCEDURE qpl_calc_stg_net_sales_WK (  
  @i_projectkey INT,
  @i_plstage    INT,
...
/******************************************************************************************
**  Name: qpl_calc_stg_net_sales_WK
**  Desc: This stored procedure gets Net Sales dollar value by sales channel and format grouping.
...
BEGIN
  ...
  BEGIN
      ...
      -- Version/Net Sales
      EXEC dbo.qpl_calc_ver_net_sales_WK @i_projectkey, @i_plstage, @v_selected_versionkey, @i_saleschanneltype, @i_formattype, @o_result OUTPUT
...
GRANT EXEC ON dbo.qpl_calc_stg_net_sales_WK TO PUBLIC
GO
