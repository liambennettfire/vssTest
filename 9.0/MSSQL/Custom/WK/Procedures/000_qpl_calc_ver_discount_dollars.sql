if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_ver_discount_dollars') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_ver_discount_dollars
GO

CREATE PROCEDURE qpl_calc_ver_discount_dollars (  
  @i_projectkey INT,
  @i_plstage    INT,
  @i_plversion	INT,
...
/******************************************************************************************
**  Name: qpl_calc_ver_discount_dollars
**  Desc: This stored procedure gets the Discount dollar value by sales channel and format grouping.
...
BEGIN
  ...
  SET @v_formats_clause = dbo.qpl_calc_get_unit_by_type_clause(@i_formattype)
	...
	SET @v_sqlstring = N'INSERT INTO #temp ' +
	  'SELECT COALESCE(f.activeprice,0), COALESCE(c.discountpercent,0), COALESCE(SUM(u.grosssalesunits),0) ' +
  ...
  DECLARE salesunits_cur CURSOR FOR  
    SELECT activeprice, discountpercent, grossunits 
    FROM #temp 

  OPEN salesunits_cur

  FETCH salesunits_cur INTO @v_format_price, @v_discount_percent, @v_sum_grossunits

  SET @v_total_discount = 0
  WHILE (@@FETCH_STATUS=0)
  BEGIN
  
    SET @v_channel_discount = @v_format_price * @v_sum_grossunits * @v_discount_percent / 100
    SET @v_total_discount = @v_total_discount + @v_channel_discount
  ...
  SET @o_result = @v_total_discount
   
END
go

GRANT EXEC ON dbo.qpl_calc_ver_discount_dollars TO PUBLIC
GO
