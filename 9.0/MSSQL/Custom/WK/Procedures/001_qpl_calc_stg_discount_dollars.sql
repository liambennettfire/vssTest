if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_stg_discount_dollars') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_stg_discount_dollars
GO

CREATE PROCEDURE qpl_calc_stg_discount_dollars (  
  @i_projectkey INT,
  @i_plstage    INT,
...
/******************************************************************************************
**  Name: qpl_calc_stg_discount_dollars
**  Desc: This stored procedure gets the Discount dollar amount by sales channel and format grouping.
...
BEGIN
  ...
  BEGIN
      ...
      -- Version/Discount Dollars
      EXEC dbo.qpl_calc_ver_discount_dollars @i_projectkey, @i_plstage, @v_selected_versionkey, @i_saleschanneltype, @i_formattype, @o_result OUTPUT
  END
   
END
go

GRANT EXEC ON dbo.qpl_calc_stg_discount_dollars TO PUBLIC
GO
