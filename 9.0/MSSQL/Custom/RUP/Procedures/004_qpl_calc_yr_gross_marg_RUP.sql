...
if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_yr_gross_marg_RUP') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_yr_gross_marg_RUP
GO

CREATE PROCEDURE qpl_calc_yr_gross_marg_RUP (  
  @i_projectkey INT,
  @i_plstage    INT,
  @i_plversion  INT,
  @i_yearcode   INT,
...
/******************************************************************************************
**  Name: qpl_calc_yr_gross_marg_RUP
**  Desc: Year/Gross Margin for Rutgers.
**
**  Auth: slb	
**  Date: April 7, 2011
*******************************************************************************************/
...
GRANT EXEC ON qpl_calc_yr_gross_marg_RUP TO PUBLIC
GO
