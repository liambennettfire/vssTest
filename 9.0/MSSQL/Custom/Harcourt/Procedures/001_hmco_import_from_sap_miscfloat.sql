/****** Object:  StoredProcedure [dbo].[hmco_import_from_SAP_miscfloat]    Script Date: 02/25/2009 14:08:04 ******/

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[hmco_import_from_SAP_miscfloat]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[hmco_import_from_SAP_miscfloat]SET ANSI_NULLS ON
GO
...
create PROCEDURE [dbo].[hmco_import_from_SAP_miscfloat] 
	@i_bookkey int, 
...
set @newvalue = cast(@i_newvalue as float)

select @existing = isnull(floatvalue,0)
from bookmisc
...
--			select @newvalue = null
	...
	begin
		...
		exec qtitle_update_titlehistory 'bookmisc', 'floatvalue' , @i_bookkey, 1, 0, @newvalue, 'Update', @i_userid, 
			null, @i_miscname, @o_error_code output, @o_error_desc output
	end
	else
	begin
		insert into bookmisc
		(bookkey, misckey, floatvalue, lastuserid, lastmaintdate, sendtoeloquenceind)
		...
		exec qtitle_update_titlehistory 'bookmisc', 'floatvalue' , @i_bookkey, 1, 0, @newvalue, 'Insert', @i_userid, 
			null, @i_miscname, @o_error_code output, @o_error_desc output
	end
end
end