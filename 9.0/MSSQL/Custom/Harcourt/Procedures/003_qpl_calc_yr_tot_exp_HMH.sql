...
GO
if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_yr_tot_exp') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_yr_tot_exp
GO
if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_yr_tot_exp_HMH') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_yr_tot_exp_HMH
GO

CREATE PROCEDURE qpl_calc_yr_tot_exp_HMH (  
  @i_projectkey INT,
  @i_plstage    INT,
  @i_plversion  INT,
  @i_yearcode   INT,
...
/******************************************************************************************
**  Name: qpl_calc_yr_tot_exp_HMH
**  Desc: Houghton Mifflin Item 58 - Year/TOTAL Expenses.
**
**  Auth: Kate
...
BEGIN
  ...
  EXEC qpl_calc_yr_coop_HMH @i_projectkey, @i_plstage, @i_plversion, @i_yearcode, @v_coop_cost OUTPUT
  ...
  EXEC qpl_calc_yr_fulfill_HMH @i_projectkey, @i_plstage, @i_plversion, @i_yearcode, @v_fulfillment_cost OUTPUT
  ...
  -- HMH only: Subsidiary Rights Expense
  EXEC qpl_calc_yr_sub_rights_exp_HMH @i_projectkey, @i_plstage, @i_plversion, @i_yearcode, @v_subrights_expense OUTPUT
  
  IF @v_subrights_expense IS NULL
    SET @v_subrights_expense = 0
  ...
  SET @v_total_expenses = @v_production_cost + @v_marketing_cost + @v_coop_cost + @v_royalty_cost + 
    @v_fulfillment_cost + @v_writeoffs_editcomps + @v_subrights_expense + @v_other_cost
...
GRANT EXEC ON qpl_calc_yr_tot_exp_HMH TO PUBLIC
GO
