...
GO
if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_yr_sub_rights_inc_HMH') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_yr_sub_rights_inc_HMH
GO

CREATE PROCEDURE qpl_calc_yr_sub_rights_inc_HMH (  
  @i_projectkey INT,
  @i_plstage    INT,
  @i_plversion	INT,
  @i_yearcode   INT,
...
/******************************************************************************************
**  Name: qpl_calc_yr_sub_rights_inc_HMH
**  Desc: Houghton Mifflin Item 47 - Year/Subsidiary Rights Income.
**
**  Auth: Kate
...
BEGIN
  ...
  -- Loop through all subright year records to calculate the TOTAL of the Subright Total
  DECLARE subright_pubamt_cur CURSOR FOR  
    SELECT SUM(COALESCE(y.amount,0))
...
GRANT EXEC ON qpl_calc_yr_sub_rights_inc_HMH TO PUBLIC
GO
