...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[create_author_minimum]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[create_author_minimum]
GO


CREATE PROCEDURE create_author_minimum (
  @i_authorkey int,
...
begin
  ...
  if @v_rowcnt > 0
    begin
      set @o_errcode=-1
      set @o_errmsg='authorkey '+cast(@i_authorkey as varchar(20))+' already exists'
      return
    end

  insert into author 
    (authorkey,lastuserid,lastmaintdate)
    values
    (@i_authorkey,@i_userid,getdate())
  ...
  while @v_orgentrykey is not null or @v_count <= @v_access_level 
    begin
      ...
      insert into globalcontactorgentry
        ...
        values
        (@i_authorkey,@v_orglevelkey,@v_orgentrykey,@i_userid,getdate())
...
GRANT EXECUTE ON dbo.create_author_minimum TO PUBLIC 
GO
