...
*******************************************************************************/
 ...
 -- verify that all required values are filled in
  ...
  BEGIN
    EXEC qtitle_set_cspartnerstatus_on_asset @v_taqelementkey,@i_userid,@o_error_code,@o_error_desc
...
--variables were kept the same except @v_count7 was added since the original code reused @v_count.  @v_count is set to @v_count7 lower in the code	
  SELECT @v_count = sum (case when isnull(g.qsicode,0) = 3 then 1 else 0 end),									--metadata
		@v_count5 = count(*),																					--all CS assets
		@v_count7 = sum (case when isnull(g.qsicode,0) <> 3 then 1 else 0 end),									--all non metadata CS assets
		@v_count2 = sum (case when isnull(g.qsicode,0) <> 3 AND cspartnerstatuscode = 1 then 1 else 0 end),		--non metadata, Not Distributed
		@v_count3 = sum (case when isnull(g.qsicode,0) <> 3 AND cspartnerstatuscode = 5 then 1 else 0 end),		--non metadata, Not up to date at all selected Partners
		@v_count4 = sum (case when isnull(g.qsicode,0) <> 3 AND cspartnerstatuscode = 4 then 1 else 0 end),		--non metadata, Distributed to all selected Partners
		@v_count6 = sum (case when isnull(g.qsicode,0) <> 3 AND cspartnerstatuscode = 3 then 1 else 0 end)		--non metadata, scheduled
    FROM taqprojectelement t
	join gentables g
	on t.taqelementtypecode = g.datacode
	and g.tableid = 287
