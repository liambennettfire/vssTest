DROP  Procedure  dbo.bookcommenthtml_replace
GO

CREATE PROCEDURE dbo.bookcommenthtml_replace
  @i_bookkey int,
  @i_printingkey int,
...
BEGIN 
  DECLARE 
    ...
    @v_prefix varchar(80),
    @v_postfix varchar(80),
    @v_plaintext_check varchar(80),
    @v_plaintext_ind int,
    @v_length int,
    @v_length_org int,
    @v_sec_length int,
...
--Init variables
  ...
  set @v_sec_length = 5000
  set @v_sec_point = 0
  set @v_complete = 0
  set @v_plaintext_ind = 0
  set @v_prefix = ''
  set @v_postfix = ''
...
--Reset blob value in table and get blob pointer
  ...
  set @v_length_org = coalesce(datalength(@blob_portion),0)
  WHILE @v_complete = 0
    BEGIN -- loop full comment 
      ...
      WHILE @loop = 0
        BEGIN -- loop comment part
          ...
          set @insert_offset = @insert_offset+coalesce(datalength(@blob_portion),0)
...
--              set @v_sec_point = @v_sec_point +@v_sec_length - @v_sec_adjust
  ...
  updatetext bookcomments.commenthtml @blob_pointer @insert_offset null  @v_postfix
  delete from commentwork where commentkey=@v_workkey
                        
end
GO
GRANT EXEC ON bookcommenthtml_replace TO PUBLIC
GO
