...
/****** Object:  StoredProcedure [dbo].[qutl_insert_taqrelationshiptabconfig_button]    Script Date: 01/09/2015 11:56:09 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
  ...
  IF @i_button_type = 2  BEGIN -- RELATE BUTTON  (Relate done first because it is less complicated)
     --Check to see if button exists already
     SELECT @v_count = COUNT(*) FROM taqrelationshiptabconfig
     WHERE @v_button_itemcode IN (relateitemtypecode, relate2itemtypecode, relate3classcode) 
       AND @v_button_classcode IN (relateclasscode, relate2classcode, relate3classcode)
       AND @o_taqrelationshipconfigkey = taqrelationshiptabconfigkey
