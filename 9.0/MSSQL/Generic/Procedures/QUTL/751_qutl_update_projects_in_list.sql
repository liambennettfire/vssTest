...
DECLARE 
  ...
  BEGIN
    ...
    IF @CurrentProjectStatus = @AcqCompletedDatacode BEGIN
      SET @FailedInd = 1
     ...
     BEGIN
      ...
      BEGIN
        ...
        PRINT ' ' + @SQLUpdate1
		    ...
		    -- Do not allow project to be updated if status to be changed is not allowed for the project  
        SET @TempIndex = CHARINDEX('UPDATE taqproject ', @SQLUpdate1)
        IF @TempIndex > 0
        BEGIN
          SET @TempIndex = CHARINDEX('taqprojectstatuscode=', @SQLUpdate1)
          IF @TempIndex > 0
          BEGIN
            SET @TempName = SUBSTRING(@SQLUpdate1, @TempIndex + 21, 100)
            ...
            BEGIN
              SET @FailedInd = 1
              SET @msg = 'New Status is not allowed for ' + @Title + '.'
        ...
        EXECUTE sp_executesql @SQLUpdate1, 
          N'@p_Key1Value INT,@p_Key2Value INT', @Key1, @Key2
        ...
        BEGIN
          ROLLBACK TRANSACTION
          SET @o_error_code = -1
          SET @o_error_desc = 'Error executing SQL UPDATE 1: ' + @SQLUpdate1 + ' (taqprojectkey=' + CONVERT(VARCHAR, @Key1) + ').'          
          GOTO ExitHandler
        END
      ...
      BEGIN
        ...
        PRINT ' ' + @SQLUpdate2
		    ...
		    -- Do not allow project to be updated if status to be changed is not allowed for the project  
        SET @TempIndex = CHARINDEX('UPDATE taqproject ', @SQLUpdate2)
        IF @TempIndex > 0
        BEGIN
          SET @TempIndex = CHARINDEX('taqprojectstatuscode=', @SQLUpdate2)
          IF @TempIndex > 0
          BEGIN
            SET @TempName = SUBSTRING(@SQLUpdate2, @TempIndex + 21, 100)
            SET @TempIndex = CHARINDEX(', ', @TempName)
            SET @NewProjectStatus = CONVERT(int, LTRIM(RTRIM(SUBSTRING(@TempName, 0, @TempIndex))))
        ...
        EXECUTE sp_executesql @SQLUpdate2, 
          N'@p_Key1Value INT,@p_Key2Value INT', @Key1, @Key2
        ...
        BEGIN
          ROLLBACK TRANSACTION
          SET @o_error_code = -1
          SET @o_error_desc = 'Error executing SQL UPDATE 2: ' + CONVERT(VARCHAR, @SQLUpdate2) + ' (taqprojectkey=' + CONVERT(VARCHAR, @Key1) + ').'
          GOTO ExitHandler
        END
      ...
      BEGIN
        ...
        PRINT ' ' + @SQLUpdate3
		    ...
		    -- Do not allow project to be updated if status to be changed is not allowed for the project  
        SET @TempIndex = CHARINDEX('UPDATE taqproject ', @SQLUpdate3)
        IF @TempIndex > 0
        BEGIN
          SET @TempIndex = CHARINDEX('taqprojectstatuscode=', @SQLUpdate3)
          IF @TempIndex > 0
          BEGIN
            SET @TempName = SUBSTRING(@SQLUpdate3, @TempIndex + 21, 100)
            SET @TempIndex = CHARINDEX(', ', @TempName)
            SET @NewProjectStatus = CONVERT(int, LTRIM(RTRIM(SUBSTRING(@TempName, 0, @TempIndex))))
        ...
        EXECUTE sp_executesql @SQLUpdate3, 
          N'@p_Key1Value INT,@p_Key2Value INT', @Key1, @Key2
        ...
        BEGIN
          ROLLBACK TRANSACTION        
          SET @o_error_code = -1
          SET @o_error_desc = 'Error executing SQL UPDATE 3: ' + CONVERT(VARCHAR, @SQLUpdate3) + ' (taqprojectkey=' + CONVERT(VARCHAR, @Key1) + ').'
          GOTO ExitHandler
        END
      ...
      BEGIN
        ...
        PRINT ' ' + @SQLUpdate4
		    ...
		    -- Do not allow project to be updated if status to be changed is not allowed for the project  
        SET @TempIndex = CHARINDEX('UPDATE taqproject ', @SQLUpdate4)
        IF @TempIndex > 0
        BEGIN
          SET @TempIndex = CHARINDEX('taqprojectstatuscode=', @SQLUpdate4)
          IF @TempIndex > 0
          BEGIN
            SET @TempName = SUBSTRING(@SQLUpdate4, @TempIndex + 21, 100)
            SET @TempIndex = CHARINDEX(', ', @TempName)
            SET @NewProjectStatus = CONVERT(int, LTRIM(RTRIM(SUBSTRING(@TempName, 0, @TempIndex))))
        ...
        EXECUTE sp_executesql @SQLUpdate4, 
          N'@p_Key1Value INT,@p_Key2Value INT', @Key1, @Key2
        ...
        BEGIN
          ROLLBACK TRANSACTION        
          SET @o_error_code = -1
          SET @o_error_desc = 'Error executing SQL UDPATE 4: ' + CONVERT(VARCHAR, @SQLUpdate4) + ' (taqprojectkey=' + CONVERT(VARCHAR, @Key1) + ').'
          GOTO ExitHandler
        END
      ...
      BEGIN
        ...
        PRINT ' ' + @SQLUpdate5
		    ...
		    -- Do not allow project to be updated if status to be changed is not allowed for the project  
        SET @TempIndex = CHARINDEX('UPDATE taqproject ', @SQLUpdate5)
        IF @TempIndex > 0
        BEGIN
          SET @TempIndex = CHARINDEX('taqprojectstatuscode=', @SQLUpdate5)
          IF @TempIndex > 0
          BEGIN
            SET @TempName = SUBSTRING(@SQLUpdate5, @TempIndex + 21, 100)
            SET @TempIndex = CHARINDEX(', ', @TempName)
            SET @NewProjectStatus = CONVERT(int, LTRIM(RTRIM(SUBSTRING(@TempName, 0, @TempIndex))))
        ...
        EXECUTE sp_executesql @SQLUpdate5, 
          N'@p_Key1Value INT,@p_Key2Value INT', @Key1, @Key2
        ...
        BEGIN
          ROLLBACK TRANSACTION
          SET @o_error_code = -1
          SET @o_error_desc = 'Error executing SQL UPDATE 5: ' + CONVERT(VARCHAR, @SQLUpdate5) + ' (taqprojectkey=' + CONVERT(VARCHAR, @Key1) +  ').'
          GOTO ExitHandler
        END
