...
PRINT 'STORED PROCEDURE : dbo.datawarehouse_printdates2'
GO

if exists (select * from dbo.sysobjects where id = Object_id('dbo.datawarehouse_printdates2') and (type = 'P' or type = 'RF'))
begin
 drop proc dbo.datawarehouse_printdates2
end

GO
create proc dbo.datawarehouse_printdates2
...
DECLARE @i_date2status int

DECLARE @temp_actdate datetime

DECLARE warehouseprintdates2 INSENSITIVE CURSOR
FOR
...
OPEN warehouseprintdates2

FETCH NEXT FROM warehouseprintdates2
INTO @i_datetypecode,@d_activedate,@d_estdate

select @i_date2status = @@FETCH_STATUS

 while (@i_date2status<>-1 )
   begin
	IF (@i_date2status<>-2)
	  begin
		select @ware_count = 0
		select @ware_count = count(*) 
			from whcprintingdates2
...
close warehouseprintdates2
deallocate warehouseprintdates2

BEGIN tran
INSERT INTO  whprintingkeydates2
	...
	estdate20,actdate20,bestdate20,estdate21,actdate21,bestdate21,
	...
	estdate40,actdate40,bestdate40,lastuserid,lastmaintdate)
VALUES (@ware_bookkey,@ware_printingkey,@ware_est1,@ware_act1 ,@ware_best1,@ware_est2 ,@ware_act2,
	...
	@ware_best20,@ware_est21,@ware_act21 ,@ware_best21,@ware_est22,@ware_act22,
	...
	@ware_best40,'WARE_STORED_PROC',@ware_system_date)
commit tran
