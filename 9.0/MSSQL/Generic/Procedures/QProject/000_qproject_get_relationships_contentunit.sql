...
IF ( @v_pricetypecode4 > 0 AND @v_price4label IS NULL ) 
  ...
  SELECT cu.contentunitkey otherprojectkey, 
         ...
         cu.issuename, cu.taqprojectrelationshipkey,
		 
		 -- we are retrieving content units for journals - don't display issue to content unit info
     CASE WHEN cu.issuekey > 0 THEN null ELSE cu.quantity1 END AS quantity1,
     CASE WHEN cu.issuekey > 0 THEN null ELSE cu.quantity2 END AS quantity2,
     CASE WHEN cu.issuekey > 0 THEN 0 ELSE cu.indicator1 END AS indicator1,
     CASE WHEN cu.issuekey > 0 THEN 0 ELSE cu.indicator2 END AS indicator2,
   ...
   WHERE cu.contentunitkey = c.projectkey 
     AND cu.journalkey = @i_projectkey
