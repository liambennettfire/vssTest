...
if @i_copy_elementkey > 0
  ...
  where elementkey = @i_copy_elementkey
else
...
begin
  ...
  if @i_copy_elementkey > 0
    ...
    select @newkey, @i_new_projectkey, @i_new_elementkey, productidcode, prefixcode,  
    ...
    insert into taqproductnumbers  
      (productnumberkey, taqprojectkey, elementkey, productidcode, prefixcode, productnumber, sortorder, lastuserid, lastmaintdate)
    select @newkey, @i_new_projectkey, @i_new_elementkey, productidcode, prefixcode,  
      CASE WHEN @cleardata = 'Y' THEN NULL ELSE productnumber END, 
      sortorder, @i_userid, getdate()
    from taqproductnumbers
  ...
  if @i_copy_elementkey > 0
	  ...
	  from taqproductnumbers q
	  where elementkey = @i_copy_elementkey and q.productnumberkey > @tobecopiedkey
  else
	  select @tobecopiedkey = min(q.productnumberkey)
	  from taqproductnumbers q
	  where taqprojectkey = @i_copy_projectkey and q.productnumberkey > @tobecopiedkey
end
...
BEGIN
  ...
  WHERE q1.taqprojectkey = @i_copy2_projectkey AND
    NOT EXISTS (SELECT * FROM taqproductnumbers q2 
                WHERE q1.productidcode = q2.productidcode AND q2.taqprojectkey = @i_copy_projectkey)
  ...
  BEGIN
    ...
    INSERT INTO taqproductnumbers  
      ...
      productnumber, sortorder, lastuserid, lastmaintdate)
    SELECT 
      @newkey, @i_new_projectkey, @i_new_elementkey, productidcode, prefixcode,  
      CASE WHEN @cleardata = 'Y' THEN NULL ELSE productnumber END, @v_sortorder, @i_userid, getdate()
    FROM taqproductnumbers
    WHERE taqprojectkey = @i_copy2_projectkey AND productnumberkey = @tobecopiedkey
