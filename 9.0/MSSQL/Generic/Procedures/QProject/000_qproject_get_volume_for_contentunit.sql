if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qproject_get_volume_for_contentunit') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qproject_get_volume_for_contentunit
GO
...
CREATE PROCEDURE qproject_get_volume_for_contentunit
...
/******************************************************************************
**  Name: qproject_get_volume_for_contentunit
**  Desc: 
**
**    Auth: Jon Hess
**    Date: 7/24/2008
...
-- First thing to do is convert the issuekey to volume key.
 -- First step is to convert the issue key passed into the volume key. 
 SET @v_volumeKey = ( select volumekey from dbo.qproject_get_contentunit() where contentunitkey = @i_projectkey )
	...
	-- reinitialize the variable each iteration
		...
		WHERE taqprojectkey2 = @v_volumeKey AND taqprojectkey1 = @i_projectkey

		SELECT @v_count2 = COUNT(*)
			FROM taqprojectrelationship
		WHERE taqprojectkey1 = @v_volumeKey AND taqprojectkey2 = @i_projectkey
...
SELECT @v_gentablesrelationshipkey = COALESCE(gentablesrelationshipkey,0)
  ...
  SET @v_formtabusageclass = 8 -- volume 
  SET @v_formtabitemtype = 6 -- contentunit doesn't need to translate
...
--  END
  ...
  SELECT projecttitle as volumename, COALESCE(@v_volumekey,0) volumekey, 
		 ...
		 dbo.qproject_get_misc_value( @v_volumeKey, @v_miscitemkey1 ) miscItem1value,

		 @v_miscitemkey2 as miscitemkey2, @v_miscitem2label as miscitem2label, 
		 dbo.qproject_get_misc_value( @v_volumeKey, @v_miscitemkey2 ) miscItem2value, 

		 @v_miscitemkey3 as miscitemkey3, @v_miscitem3label as miscitem3label, 
		 dbo.qproject_get_misc_value( @v_volumeKey, @v_miscitemkey3 ) miscItem3value, 

		 @v_miscitemkey4 as miscitemkey4, @v_miscitem4label as miscitem4label, 
		 dbo.qproject_get_misc_value( @v_volumeKey, @v_miscitemkey4 ) miscItem4value, 

		 @v_miscitemkey5 as miscitemkey5, @v_miscitem5label as miscitem5label, 
		 dbo.qproject_get_misc_value( @v_volumeKey, @v_miscitemkey5 ) miscItem5value, 

		 @v_miscitemkey6 as miscitemkey6, @v_miscitem6label as miscitem6label, 
		 dbo.qproject_get_misc_value( @v_volumeKey, @v_miscitemkey6 ) miscItem6value, 

		 @v_datetypecode1 as datetypecode1,@v_date1label as date1label, 
		 dbo.qproject_get_last_taskdate( @v_volumeKey, @v_datetypecode1 ) as date1value, 
		 @v_datetypecode2 as datetypecode2, @v_date2label as date2label, 
         dbo.qproject_get_last_taskdate( @v_volumeKey, @v_datetypecode2 ) as date2value,
		 @v_datetypecode3 as datetypecode3, @v_date3label as date3label, 
         dbo.qproject_get_last_taskdate( @v_volumeKey, @v_datetypecode3 ) as date3value,
		 @v_datetypecode4 as datetypecode4, @v_date4label as date4label, 
         dbo.qproject_get_last_taskdate( @v_volumeKey, @v_datetypecode4 ) as date4value,
		 @v_datetypecode5 as datetypecode5, @v_date5label as date5label, 
         dbo.qproject_get_last_taskdate( @v_volumeKey, @v_datetypecode5 ) as date5value,
		 @v_datetypecode6 as datetypecode6, @v_date6label as date6label, 
         dbo.qproject_get_last_taskdate( @v_volumeKey, @v_datetypecode6 ) as date6value,

		 @v_pricetypecode1 as pricetypecode1, @v_price1label as price1label, 
         dbo.qproject_get_price_by_pricetype( @v_volumeKey, @v_pricetypecode1 ) as pricetypecode1Value,
		 @v_pricetypecode2 as pricetypecode2, @v_price2label as price2label, 
         dbo.qproject_get_price_by_pricetype( @v_volumeKey, @v_pricetypecode2 ) as pricetypecode2Value,
		 @v_pricetypecode3 as pricetypecode3, @v_price3label as price3label, 
         dbo.qproject_get_price_by_pricetype( @v_volumeKey, @v_pricetypecode3 ) as pricetypecode3Value,
		 @v_pricetypecode4 as pricetypecode4, @v_price4label as price4label, 
         dbo.qproject_get_price_by_pricetype( @v_volumeKey, @v_pricetypecode4 ) as pricetypecode4Value,

		 @v_productidcode1 as productidcode1, @v_productid1label as productid1label, 
		 ( SELECT productnumber FROM taqproductnumbers WHERE ( taqprojectkey = @v_volumeKey AND productidcode = @v_productidcode1 ) ) as productIdCode1Value,
		 @v_productidcode2 as productidcode2, @v_productid2label as productid2label, 
		 ( SELECT productnumber FROM taqproductnumbers WHERE ( taqprojectkey = @v_volumeKey AND productidcode = @v_productidcode2 ) ) as productIdCode2Value,

		 @v_roletypecode1 as roletypecode1, @v_roletype1label as roletype1label, 
		 dbo.qproject_get_participant_name_by_role( @v_volumeKey, @v_roletypecode1) as roletypecode1Value,
		 @v_roletypecode2 as roletypecode2, @v_roletype2label as roletype2label, 
		 dbo.qproject_get_participant_name_by_role( @v_volumeKey, @v_roletypecode2) as roletypecode2Value,
...
GRANT EXEC ON qproject_get_volume_for_contentunit TO PUBLIC
GO
