...
BEGIN
  ...
  SET @o_error_desc = ''
   ...
   BEGIN
	  SELECT DISTINCT g.datacode, g.datadesc, g.qsicode, g.alternatedesc1, g.alternatedesc2, 0 assotitlestabind, CASE WHEN i.sortorder is not null THEN i.sortorder END, g.sortorder
    FROM gentables g, gentablesitemtype i
     WHERE g.tableid = i.tableid and i.tableid = 583
	   ...
	   (0,@i_usageclasscode)
	  UNION
	  SELECT DISTINCT g.datacode, g.datadesc, g.qsicode, COALESCE(gx.gentext1, g.datadesc), 
		'~/PageControls/ProjectRelationships/ProjectsComparativeTitles.ascx', 1 assotitlestabind, CASE WHEN i.sortorder is not null THEN i.sortorder END, g.sortorder
