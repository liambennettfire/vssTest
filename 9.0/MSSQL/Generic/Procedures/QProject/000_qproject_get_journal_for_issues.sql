if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qproject_get_journal_for_issue') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qproject_get_journal_for_issue
GO
...
CREATE PROCEDURE qproject_get_journal_for_issue
...
/******************************************************************************
**  Name: qproject_get_journal_for_issue
**  Desc: This procedure takes an issue key and will return the journal
**		  relationship information. 
**
**	Consumed by: journalVolumeForIssue.ascx.cs
**
*******************************************************************************/
...
-- SET @v_volumekey = ( select volumekey from dbo.qproject_get_issue() where issuekey = @i_projectkey )
 ...
 SELECT @v_journalkey = journalkey
...
SELECT @v_gentablesrelationshipkey = COALESCE(gentablesrelationshipkey,0)
  ...
  -- Setup the formtabitemtype and usageclass ( 550 - itemtype )
...
GRANT EXEC ON qproject_get_journal_for_issue TO PUBLIC
GO
