if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qproject_get_journal_for_contentunit') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qproject_get_journal_for_contentunit
GO
...
CREATE PROCEDURE qproject_get_journal_for_contentunit
...
/******************************************************************************
**  Name: qproject_get_journal_for_contentunit
**  Desc: This procedure takes an content unit key and will return the journal
**		  relationship information. 
**
**	Consumed ( at least ) by: journalVolumeIssueForContentUnit.ascx.cs
**
*******************************************************************************/
  ...
  DECLARE	@rowcount_var INT,
	...
	-- First step is to convert the CU key passed into the volume key. 
 SELECT @v_volumekey = volumekey,
		@v_journalkey = journalkey
 FROM dbo.qproject_get_contentunit() where contentunitkey = @i_projectkey

  SELECT @error_var = @@ERROR, @rowcount_var = @@ROWCOUNT
  IF @error_var <> 0 BEGIN
    SET @o_error_code = -1
    SET @o_error_desc = 'error acessing Volume View: projectkey = ' + cast(@v_volumeKey AS VARCHAR)
    RETURN  
  END 
...
SELECT @v_gentablesrelationshipkey = COALESCE(gentablesrelationshipkey,0)
  ...
  IF @error_var <> 0 BEGIN
    SET @o_error_code = -1
    SET @o_error_desc = 'error acessing gentablesrelationships: projectkey = ' + cast(@v_volumeKey AS VARCHAR)
    RETURN  
  END 
...
IF ( @v_pricetypecode4 > 0 AND @v_price4label IS NULL ) 
  ...
  IF @error_var <> 0 BEGIN
    SET @o_error_code = -1
    SET @o_error_desc = 'error acessing coreprojectinfo: issuekey = ' + cast(@i_projectkey AS VARCHAR)
  END 
