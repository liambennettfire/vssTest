...
**    --------    --------     -------   --------------------------------------
**   02/18/2016   Kusum        36463     Bookweight is not populated on the Printing table for new titles
**   
**********************************************************************************************************/
...
BEGIN
	...
	IF coalesce(@v_type,'') in ('Q','C','CK') AND coalesce(@i_qtydecimalind,0) = 0
		BEGIN
			select @v_updatevalue = cast (@i_value as varchar(50)) --qty and detailcode
			select @v_historyvalue = @v_updatevalue
		END
	
	IF coalesce(@i_detailvalue,null) is not null and coalesce(@v_type,'') in ('DT')
		BEGIN
			select @v_updatevalue = cast (@i_detailvalue as varchar(50)) --qty and detailcode
	...
	IF coalesce(@i_detail2value,null) is not null and coalesce(@v_type,'') = 'T2'
		BEGIN
			select @v_updatevalue = cast (@i_detail2value as varchar(50)) --qty and detailcode
			--select @v_historyvalue = will need to translate these to gentable\subgentable values eventually, for not nothing on the printing requires it for HNA
		END		
		
	IF coalesce(@i_uomvalue,null) is not null and coalesce(@v_type,'') = 'U'
		BEGIN
			select @v_updatevalue = cast (@i_uomvalue as varchar(50)) --qty and detailcode
