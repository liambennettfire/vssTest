...
CREATE PROCEDURE qpl_create_new_version
...
**
**  3/10/10 - KW - Modified to allow copy from existing version.
*****************************************************************************************************/
...
BEGIN
  ...
  -- check to see if P&L version is being created for Acquisition Project (itemtype qsicode = 3, usageclass qsicode = 1)
  -- or Work Project (itemtype qsicode = 9, usageclass qsicode = 1)
  ...
  BEGIN
    ...
    UPDATE taqproject
    SET plenteredcurrency = @v_input_currency, plapprovalcurrency = @v_approval_currency
    WHERE taqprojectkey = @i_new_projectkey
  ...
  BEGIN
    IF @v_copy_acq_project_data = 1
      DECLARE versionformats_cur CURSOR FOR
        ...
        WHERE taqprojectkey = @i_projectkey AND titlerolecode = 2 AND projectrolecode = 2 AND taqprojectformatdesc IS NOT NULL
    ELSE IF @v_copy_work_project_data = 1
      DECLARE versionformats_cur CURSOR FOR
        ...
        WHERE t.bookkey = bd.bookkey 
          ...
          AND s.tableid = 312
          AND t.taqprojectkey = @i_projectkey AND t.titlerolecode = 1 AND t.projectrolecode = 4
  ...
  SELECT
    @i_new_projectkey, plsummaryitemkey, @i_new_plstage, @i_new_plversion, yearcode, longvalue, textvalue, decimalvalue, @v_userid, getdate()
  ...
  INSERT INTO taqversionroyaltyadvance
    (taqprojectkey, plstagecode, taqversionkey, yearcode, datetypecode, dateoffsetcode, amount, lastuserid, lastmaintdate)
  SELECT
    @i_new_projectkey, @i_new_plstage, @i_new_plversion, yearcode, datetypecode, dateoffsetcode, amount, @v_userid, getdate()
  ...
  BEGIN
    ...
    SELECT
      @v_subrightskey, yearcode, amount, @v_userid, getdate()
    ...
    DECLARE formats_cur CURSOR FOR
      SELECT v.taqprojectformatkey versionformatkey, v.mediatypecode, v.mediatypesubcode, 
        CASE
          ...
          ELSE v.activeprice
        END price, 
        v.scaleselectioncode, v.charsperpage, v.formatpercentage, 0 bookkey       
  ...
  ELSE IF @v_copy_work_project_data = 1
    DECLARE formats_cur CURSOR FOR
      SELECT v.taqprojectformatkey versionformatkey, v.mediatypecode, v.mediatypesubcode, v.activeprice price, 
        v.scaleselectioncode, v.charsperpage, v.formatpercentage, p.bookkey
      FROM taqprojecttitle p 
  ...
  ELSE
    ...
    DECLARE formats_cur CURSOR FOR
      SELECT taqprojectformatkey versionformatkey, mediatypecode, mediatypesubcode, activeprice, 
         scaleselectioncode, charsperpage, formatpercentage, 0 bookkey
  ...
  FETCH formats_cur
  INTO @v_cur_formatkey, @v_mediatype, @v_mediasubtype, @v_activeprice, @v_scaleselectcode, @v_charsperpage, @v_format_percent, @v_bookkey
  ...
  BEGIN
    ...
    INSERT INTO taqversionformat
      ...
      activeprice, scaleselectioncode, charsperpage, formatpercentage, lastuserid, lastmaintdate)
    VALUES
      (@i_new_projectkey, @i_new_plstage, @i_new_plversion, @v_new_formatkey, @v_mediatype, @v_mediasubtype,
       @v_activeprice, @v_scaleselectcode, @v_charsperpage, @v_format_percent, @v_userid, getdate())
  ...
  IF @v_count = 0
  BEGIN
    EXEC qpl_recalc_pl_items @i_new_projectkey, @i_new_plstage, @i_new_plversion, @v_userid, @o_error_code, @o_error_desc
