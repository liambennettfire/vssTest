...
BEGIN
  ...
  DECLARE updateprodqty_cur CURSOR FOR
    SELECT taqversionspecategorykey, taqprojectkey, taqversionformatkey, plstagecode, taqversionkey, itemcategorycode,
      COALESCE(finishedgoodind,0), quantity
    FROM taqversionspeccategory
    WHERE taqversionspecategorykey = @i_categorykey
   UNION
    SELECT taqversionspecategorykey, taqprojectkey, taqversionformatkey, plstagecode, taqversionkey, itemcategorycode,
      (select COALESCE(finishedgoodind,0) from taqversionspeccategory where taqversionspecategorykey = @i_categorykey) as finishedgoodind,
      (select quantity from taqversionspeccategory where taqversionspecategorykey = @i_categorykey) as quantity
    FROM taqversionspeccategory
  ...
  INTO @v_categorykey, @v_projectkey, @v_formatkey, @v_plstage, @v_plversion, @v_categorycode, @v_finishedgoodind, @v_compqty
  ...
  BEGIN
    ...
    IF @v_lockedind = 0 BEGIN
      ...
      BEGIN
        ...
        IF @v_prodqtyentrytypecode = 4  --Generate from FG Component Quantity
        BEGIN 
          UPDATE taqversionformatyear
          SET quantity = @v_compqty, lastuserid = @i_userid, lastmaintdate = getdate()
          WHERE taqprojectkey = @v_projectkey 
            AND plstagecode = @v_plstage 
            AND taqversionkey = @v_plversion 
        ...
        BEGIN
          ...
          -- recalc costs associated with new quantity
          DECLARE compqtycosts_cur CURSOR FOR
            SELECT c.taqversionformatyearkey, c.acctgcode, c.taqversionspeccategorykey, c.plcalccostcode, c.versioncostsamount, c.compunitcost, @v_newcompqty,
                   c.printingnumber
            ...
            WHERE c.taqversionformatyearkey = y.taqversionformatyearkey
              ...
              AND taqversionkey = @v_plversion 
              AND taqprojectformatkey = @v_formatkey 
              AND taqversionspeccategorykey = @v_taqversionspecategorykey 
            UNION           
            SELECT c.taqversionformatyearkey, c.acctgcode, c.taqversionspeccategorykey, c.plcalccostcode, c.versioncostsamount, c.compunitcost, @v_newcompqty,
                   c.printingnumber
            FROM taqversioncosts c
            WHERE taqversionspeccategorykey in (select taqversionspecategorykey from taqversionspeccategory where relatedspeccategorykey = @v_taqversionspecategorykey)           
            ...
            INTO @v_formatyearkey, @v_acctgcode, @v_speccategorykey, @v_plcalccostcode, @v_totalcost, @v_compunitcost, @v_qty, @v_printingnumber
            ...
            BEGIN
              ...
              SET versioncostsamount = @v_newtotalcost, compunitcost = @v_newunitcost, lastuserid = @i_userid, lastmaintdate = getdate()
      ...
      BEGIN  
        ...
        IF @v_categorykey = @v_first_categorykey BEGIN
          ...
          BEGIN 
            UPDATE taqversionformatyear
            SET quantity = @v_first_categorykey_qty, lastuserid = @i_userid, lastmaintdate = getdate()
            WHERE taqprojectkey = @v_projectkey 
              AND plstagecode = @v_plstage 
              AND taqversionkey = @v_plversion 
              AND taqprojectformatkey = @v_formatkey 
          ...
          -- For non-Finished Good components get the costs associated with that component and ones not assigned to any component
          DECLARE compqtycosts_cur CURSOR FOR
           SELECT c.taqversionformatyearkey, c.acctgcode, c.taqversionspeccategorykey, c.plcalccostcode, c.versioncostsamount, c.compunitcost, @v_compqty,
                  c.printingnumber
             FROM taqversioncosts c, taqversionformatyear y
        ...
        END
        ELSE BEGIN
          -- For non-Finished Good components, get only the costs associated with that component 
          DECLARE compqtycosts_cur CURSOR FOR
           SELECT c.taqversionformatyearkey, c.acctgcode, c.taqversionspeccategorykey, c.plcalccostcode, c.versioncostsamount, c.compunitcost, @v_compqty,
                  c.printingnumber
             FROM taqversioncosts c, taqversionformatyear y
