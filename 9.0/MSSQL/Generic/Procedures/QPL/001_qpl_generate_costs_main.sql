...
BEGIN
  ...
  CREATE TABLE #scalecostbucket_table
   (formatkey             INT,
    internalcode          INT,
    cost                  FLOAT,
    validforprtgs	        INT,
    calccostcode          TINYINT)
  ...
  END 
    ...
    IF @o_error_code = -1 BEGIN
      ...
      INSERT INTO taqversioncostmessages 
			...
			VALUES
			(@i_taqversionformatyearkey, @v_message, 4, NULL, NULL,getdate(), 'QSIADMIN', @i_processtype)
			
      RETURN
    END 
  END
--select * from taqversionspeccategory where taqversionformatkey in (select taqprojectformatkey from taqversionformatyear where taqprojectkey = 2703141)

--	DECLARE versionspeccategory_cur CURSOR FOR
--		SELECT taqversionspecategorykey,itemcategorycode,scaleprojecttype,speccategorydescription
--		  FROM taqversionspeccategory
--		 WHERE taqversionformatkey = @v_taqprojectformatkey
  DECLARE versionspeccategory_cur CURSOR FOR
		SELECT DISTINCT taqversionspecategorykey,itemcategorycode,scaleprojecttype,speccategorydescription
	...
	FETCH versionspeccategory_cur
	 INTO @v_taqversionspecategorykey, @v_itemcategorycode,@v_scaleprojecttype,@v_speccategorydescription
...
--				    AND itemcode = @v_itemcode
           ...
           IF @v_count3 IS NULL SELECT @v_count3 = 0
            ...
            INSERT INTO taqversioncostmessages 
			      (taqversionformatyearkey, message, messagetypecode, acctgcode, cost, lastmaintdate, lastmaintuser, processtype)
			      VALUES
			      (@i_taqversionformatyearkey, @v_message, 3, NULL, NULL,getdate(), 'QSIADMIN', @i_processtype)
...
--				    AND itemcode = @v_itemcode
    ...
    EXEC qscale_get_scale_for_speccategory @v_taqversionspecategorykey,
      @v_taqprojectkey, @i_taqversionformatyearkey, @i_processtype, @v_scaleprojectkey output,
    ...
    BEGIN
        ...
        FROM taqproject
       WHERE taqprojectkey = @v_scaleprojectkey
		  ...
		  INSERT INTO  #tmp_scaleprojectkeys (scaleprojectkey)
			  VALUES(@v_scaleprojectkey)
        ...
        END 
         ...
         -- Parameter type 1(Scale), 2 (Grid) Messagetypecode = 0(Not a scale item)
            ...
            EXEC qscale_get_scale_detail @i_taqversionformatyearkey,@v_scaleprojectkey,@v_taqprojecttitle,@v_scaletabkey,
  ...
  END
    ...
    -- item, but will have an autoapply ind set to yes and a taqscaledetailkey
		...
		WHILE (@@FETCH_STATUS=0)
		BEGIN
			SELECT @v_taqprojecttitle = taqprojecttitle, @v_scaletype = taqprojecttype
        FROM taqproject
       WHERE taqprojectkey = @v_scaleprojectkey
          ...
          ELSE BEGIN
            ...
            IF @o_error_code = -1 BEGIN
              ...
              INSERT INTO taqversioncostmessages 
			        (taqversionformatyearkey, message, messagetypecode, acctgcode, cost, lastmaintdate, lastmaintuser, processtype)
			        VALUES
			        (@i_taqversionformatyearkey, @v_message, @v_messagetypecode, NULL, NULL,getdate(), 'QSIADMIN', @i_processtype)
...
print '**@v_taqdetailscalekey= ' + cast(@v_taqdetailscalekey as varchar)
        ...
        EXEC qpl_generate_scale_costs @v_taqversionspecitemkey,@v_taqdetailscalekey,@v_autoapplyind,@v_calcind,
          @v_calcsecqsicode,@v_perqty,@v_specialprocess,@i_taqversionformatyearkey,
          @o_error_code output,@o_error_desc output
		...
		-- Write costs
    SELECT @v_prodqtyitemcategorycode = datacode,
           @v_prodqtyitemcode = datasubcode
      FROM subgentables 
     WHERE tableid = 616 
       AND qsicode = 6
        ...
        IF @v_acceptgenind IS NOT NULL AND @v_acceptgenind > 0
          ...
          IF (@v_quantity IS NOT NULL AND @v_quantity <> 0)
            ...
            UPDATE taqversioncosts
               SET versioncostsamount = @v_bucket_cost,
                   unitcost = @v_unitcost
             WHERE acctgcode = @v_bucket_internal 
               AND taqversionformatyearkey = @i_taqversionformatyearkey
          END
          ...
          INSERT INTO taqversioncostmessages 
						(taqversionformatyearkey, message, messagetypecode, acctgcode, cost, lastmaintdate, lastmaintuser, processtype)
					VALUES
							(@i_taqversionformatyearkey, 'New costs for P&L Spec item have been added to this charge code', 4, @v_bucket_internal, @v_bucket_cost,
        ...
        --if there is a quantity, recalculate unit cost and update it
				...
				BEGIN
				  ...
				  INSERT INTO taqversioncosts --insert replacement cost with unitcost
					...
					VALUES
					(@i_taqversionformatyearkey, @v_bucket_internal, @v_bucket_calccost, @v_bucket_cost, @v_unitcost, @v_printing, @v_acceptgenind, 'QSIADMIN', getdate())
								
					INSERT INTO taqversioncostmessages 
					 (taqversionformatyearkey, message, messagetypecode, acctgcode, cost, lastmaintdate, lastmaintuser, processtype)
					VALUES
						(@i_taqversionformatyearkey, 'Costs for P&L Spec item have been added to this charge code', 4, @v_bucket_internal, @v_bucket_cost,
							getdate(), 'QSIADMIN', @i_processtype)
