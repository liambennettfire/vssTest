...
--print '@v_variableamount= ' + cast(@v_variableamount as varchar)
  ...
  -- Generate Fixed Cost
  IF @v_fixedamount > 0 BEGIN
     IF @v_fixedchargecode IS NOT NULL BEGIN
				SELECT @v_acctgcatcode=placctgcategorycode,@v_ext_chargecode=externalcode, @v_costtype=costtype,
         @v_externaldesc = externaldesc
				  FROM cdlist
         ...
         --insert costs into bucket
				...
				SELECT @v_bucket_cost=cost
					FROM #scalecostbucket_table
				 WHERE internalcode=@v_fixedchargecode --AND taqversionspecitemkey=@i_taqversionspecitemkey
					
				IF @v_bucket_cost IS NULL BEGIN --insert value to costbucket
					SET @v_bucket_cost = @v_fixedamount
					INSERT INTO #scalecostbucket_table
             (formatkey,internalcode,cost,validforprtgs,calccostcode)
				  	VALUES (@v_taqprojectformatkey, @v_fixedchargecode, @v_bucket_cost,@v_validforprtscode,@v_bucket_calccost)
				END
        ...
        INSERT INTO taqversioncostmessages 
				   ...
				   VALUES
				   (@i_taqversionformatyearkey, @v_message, 4, @v_acctgcatcode, @v_bucket_cost,getdate(), 'QSIADMIN', 1)
       END
  END
  ...
  BEGIN
    ...
    IF @v_variablecost > 0
    BEGIN
      SELECT @v_acctgcatcode = placctgcategorycode, @v_costtype = costtype, 
        @v_ext_chargecode = externalcode, @v_externaldesc = externaldesc
      FROM cdlist
      ...
      SELECT @v_bucket_cost = cost
      FROM #scalecostbucket_table
      WHERE internalcode = @v_chargecode --AND taqversionspecitemkey=@i_taqversionspecitemkey
      ...
      BEGIN
        ...
        INSERT INTO #scalecostbucket_table
          (formatkey, internalcode, cost, validforprtgs,  calccostcode)
        VALUES 
          (@v_taqprojectformatkey, @v_chargecode, @v_bucket_cost, @v_validforprtscode,  @v_bucket_calccost)
      END
      ELSE  --update already existing bucket value
      BEGIN
      ...
      INSERT INTO taqversioncostmessages 
        (taqversionformatyearkey, message, messagetypecode, acctgcode, cost, lastmaintdate, lastmaintuser, processtype)
      VALUES
        (@i_taqversionformatyearkey, @v_message, 4, @v_chargecode, @v_variablecost, getdate(), 'QSIADMIN', 1)
    END
    ELSE BEGIN
  ...
  ELSE IF @i_specialprocess = 'MANUSPG' 
  BEGIN
  ...
  ELSE IF @i_calcspecqsicode IS NOT NULL AND @i_calcspecqsicode <> 0 BEGIN
     SELECT @v_acctgcatcode=placctgcategorycode,@v_costtype=costtype,@v_ext_chargecode = externalcode,
         @v_externaldesc = externaldesc
		   FROM cdlist
		  WHERE internalcode = @v_chargecode 
...
--print '@v_acctgcatcode  = ' + cast(@v_acctgcatcode AS VARCHAR)	
    ...
    SELECT @v_calcspecitemcategorycode= datacode,@v_calcspecitemcode = datasubcode,@v_calculationtypedesc=datadesc
      FROM subgentables
     WHERE tableid = 616
       AND qsicode = @i_calcspecqsicode

    SELECT @v_calcspec_int = COALESCE(dbo.qscale_find_specification_value(@i_taqversionformatyearkey,@v_calcspecitemcategorycode,@v_calcspecitemcode),0),
			   @v_calcspec_numeric = COALESCE(dbo.qscale_find_specification_value(@i_taqversionformatyearkey,@v_calcspecitemcategorycode,@v_calcspecitemcode),0)
...
--print '@v_calcspec_numeric= ' + cast(@v_calcspec_numeric as varchar)
    ...
    IF @v_calcspec_int = -1 BEGIN
       ...
       INSERT INTO taqversioncostmessages 
				   (taqversionformatyearkey, message, messagetypecode, acctgcode, cost, lastmaintdate, lastmaintuser, processtype)
				   VALUES
				   (@i_taqversionformatyearkey, @v_message, 2, @v_chargecode, NULL,getdate(), 'QSIADMIN', 1)
    END
    ELSE BEGIN
      IF @v_calcspec_int > 0 
    ...
    IF @v_variablecost > 0 BEGIN
      SELECT @v_acctgcatcode=placctgcategorycode,@v_costtype=costtype,@v_ext_chargecode = externalcode,
         ...
         --insert costs into bucket
          ...
          INSERT INTO #scalecostbucket_table
             (formatkey,internalcode,cost,validforprtgs,calccostcode)
				  	VALUES (@v_taqprojectformatkey, @v_chargecode, @v_bucket_cost,@v_validforprtscode,@v_bucket_calccost)
				  				  	
--					INSERT INTO #scalecostbucket_table
--             (taqversionspecitemkey,internalcode,acctgcode,cost,calccostcode)
--				  	VALUES (@i_taqversionspecitemkey, @v_chargecode, @v_acctgcatcode,@v_bucket_cost,@v_bucket_calccost)
				END
				ELSE BEGIN --update already existing bucket value
					SET @v_bucket_cost = @v_bucket_cost + @v_variablecost
					UPDATE #scalecostbucket_table
  ...
  ELSE IF @i_percentcalcind IS NOT NULL AND @i_percentcalcind <> 0 BEGIN
    ...
    WHERE gentablesrelationshipkey = 21 AND code1 = @v_calculationtypecode
    ...
    BEGIN
      ...
      INSERT INTO taqversioncostmessages 
        (taqversionformatyearkey, message, messagetypecode, acctgcode, cost, lastmaintdate, lastmaintuser, processtype)
      VALUES
        (@i_taqversionformatyearkey, @v_message, 3, @v_chargecode, NULL,getdate(), 'QSIADMIN', 1)
    END
    ...
    DECLARE gentablesrelationshipdetail_cur CURSOR FOR
      ...
      WHILE (@@FETCH_STATUS=0)
        ...
        SELECT @v_bucket_cost=cost
				  FROM #scalecostbucket_table
			   WHERE internalcode=@v_code2 --AND taqversionspecitemkey=@i_taqversionspecitemkey
