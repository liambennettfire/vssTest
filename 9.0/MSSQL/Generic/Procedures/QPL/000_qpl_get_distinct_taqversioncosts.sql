...
BEGIN
  ...
  IF @i_category_qsicode = 2  --distinct Production costs
    SELECT DISTINCT c.acctgcode, c.acctgcode origacctgcode, c.plcalccostcode, c.plcalccostsubcode, 
      ...
      g.sortorder, @v_plcurrency_format currencyformat
    ...
    WHERE f.taqversionformatyearkey = c.taqversionformatyearkey AND 
      ...
      c.versioncostsamount IS NOT NULL AND
      ...
      WHERE tableid = 563 AND qsicode IS NULL AND datacode IN
        (SELECT y2.yearcode FROM taqversionformatyear y2, taqversioncosts c2
         WHERE y2.taqversionformatyearkey = c2.taqversionformatyearkey AND 
          ...
          c2.printingnumber = 1 AND
          c2.versioncostsamount IS NOT NULL)),0) sortorder, 
      @v_plcurrency_format currencyformat
  ...
  ELSE IF @i_category_qsicode > 0 -- distinct costs for given category
    SELECT DISTINCT c.acctgcode, c.acctgcode origacctgcode, c.plcalccostcode, c.versioncostsnote, 0.00 total,
  ...
  ELSE -- all distinct costs
    SELECT DISTINCT c.acctgcode, c.acctgcode origacctgcode, c.plcalccostcode, c.versioncostsnote, 0.00 total,
        ...
        dbo.qutl_get_cdlist_desc(c.acctgcode,'externalcode') externalcostcode, @v_plcurrency_format currencyformat
