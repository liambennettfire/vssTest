...
CREATE PROCEDURE qpl_copy_format (  
...
BEGIN
  ...
  BEGIN
    ...
    DECLARE specitems_cur CURSOR FOR
      SELECT taqversionspecategorykey, itemcode, itemdetailcode, itemdetailsubcode, itemdetailsub2code, quantity, validforprtgscode, 
		description, description2, decimalvalue, unitofmeasurecode
  ...
  BEGIN
    ...
    INSERT INTO taqversionroyaltysaleschannel
      ...
      saleschannelcode, pricetypeforroyalty, lastuserid, lastmaintdate)
    VALUES
      (@v_royaltykey, @i_new_projectkey, @i_new_plstage, @i_new_plversion, @v_new_formatkey, 
      @v_saleschannelcode, @v_pricetypeforroyalty, @i_userid, getdate())
  ...
  DECLARE saleschannel_cur CURSOR FOR
    SELECT taqversionsaleskey, saleschannelcode, saleschannelsubcode, discountpercent, returnpercent
  ...
  FETCH saleschannel_cur 
  INTO @v_cur_saleskey, @v_saleschannelcode, @v_saleschannelsubcode, @v_discountpercent, @v_returnpercent
  ...
  BEGIN
    ...
    IF @i_copyvalues = 2 --copy everything
      ...
      VALUES
        ...
        @v_saleschannelcode, @v_saleschannelsubcode, @v_discountpercent, @v_returnpercent, @i_userid, getdate())
  ...
  BEGIN
    ...
    IF @i_copyvalues = 2 BEGIN --copy everything
      INSERT INTO taqversioncosts
        ...
        versioncostsamount, unitcost, acceptgenerationind, printingnumber, lastuserid, lastmaintdate)
      SELECT
        @v_formatyearkey, acctgcode, plcalccostcode, versioncostsnote, plcalccostsubcode, 0,
        versioncostsamount, unitcost, acceptgenerationind, printingnumber, @i_userid, getdate()
    ...
    IF @i_copyvalues = 2  --copy everything
      ...
      SELECT
        @v_formatyearkey, acctgcode, incomenote, incomeamount, @i_userid, getdate()
