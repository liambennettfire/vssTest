...
BEGIN
  ...
  FETCH formats_project_cur INTO @v_taqprojectformatkey	
  ...
  DECLARE formats_on_project_version_cur CURSOR FOR
    SELECT taqversionformat.taqprojectformatkey versionformatkey, taqprojecttitle.taqprojectformatkey projectformatkey, taqversionformat.activeprice
    FROM taqprojecttitle, taqversionformat 
    WHERE taqprojecttitle.taqprojectkey = taqversionformat.taqprojectkey 
      AND taqprojecttitle.mediatypecode = taqversionformat.mediatypecode 
      AND taqprojecttitle.mediatypesubcode = taqversionformat.mediatypesubcode 
      AND taqprojecttitle.taqprojectkey = @i_projectkey
      AND taqversionformat.plstagecode = @i_plstagecode 
      AND taqversionformat.taqversionkey = @i_plversionkey
      AND taqprojecttitle.titlerolecode = @v_titlerolecode
      
  OPEN formats_on_project_version_cur

  FETCH formats_on_project_version_cur INTO @v_versionformatkey, @v_taqprojectformatkey, @v_activeprice	
  ...
  BEGIN
    ...
    SET price = @v_activeprice, initialrun = @v_quantity, lastuserid = @v_userid, lastmaintdate = getdate()
    WHERE taqprojectformatkey = @v_taqprojectformatkey
  ...
  SELECT v.taqprojectformatkey, v.mediatypecode, v.mediatypesubcode, v.activeprice
  ...
  FETCH formats_version_cur INTO @v_versionformatkey, @v_mediatypecode, @v_mediatypesubcode, @v_activeprice
  ...
  BEGIN
    ...
    IF @v_count > 0
      SELECT @v_taqprojectformatdesc = datadesc
      FROM subgentables 
      WHERE tableid = 312 AND datacode = @v_mediatypecode AND datasubcode = @v_mediatypesubcode
    ELSE
    ...
    INSERT INTO taqprojecttitle
      ...
      price, initialrun, printingkey, titlerolecode, projectrolecode , primaryformatind, lastuserid, lastmaintdate)
    VALUES
      (@v_taqprojectformatkey, @i_projectkey, @v_mediatypecode, @v_mediatypesubcode, @v_taqprojectformatdesc, 
      @v_activeprice, @v_quantity, 1, @v_titlerolecode, @v_projectrolecode, 0, @v_userid, getdate())
