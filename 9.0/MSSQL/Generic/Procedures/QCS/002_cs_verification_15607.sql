...
if @i_numcount > 0 begin
	...
	EXEC bookverificationmessage_insert @i_bookkey, @i_verificationtypecode, @v_msgtype_error, @v_msg, @i_username, @o_error_code output, @o_error_desc output
	IF @o_error_code = -1
		RETURN
end 
    ...
    and gc.deletestatus = 'N'
        ...
        and gp.deletestatus = 'N'
				...
				begin
					...
					EXEC bookverificationmessage_insert @i_bookkey, @i_verificationtypecode, @v_msgtype_warning, @v_msg, @i_username, @o_error_code output, @o_error_desc output
					IF @o_error_code = -1
						RETURN			
				end
			end
...
if @v_prodavailability is null  begin
  ...
  begin	
	  EXEC bookverificationmessage_insert @i_bookkey, @i_verificationtypecode, @v_msgtype_info, @v_msg, @i_username, @o_error_code output, @o_error_desc output
  end
	IF @o_error_code = -1
	  RETURN		
end 
...
--content services approval - only check if the format is one set up with CS partners
if @csapproval <> 1 and @csformat = 1
begin
  set @v_failed = 1
  EXEC bookverificationmessage_insert @i_bookkey, @i_verificationtypecode, @v_msgtype_error, 'Title not approved for Distribution through Content Services', @i_username, @o_error_code output, @o_error_desc output
	IF @o_error_code = -1
	  RETURN
end
...
IF @SendRightsToCloud = 3 BEGIN
  -- Verify that all countries (from the commentstring) are present in the Clound region table
  SELECT @v_count2 = 0
  ...
  IF @v_count2 = 1 BEGIN
    ...
    IF @v_salesrights_commenttext <> '' AND @v_salesrights_commenttext IS NOT NULL BEGIN
