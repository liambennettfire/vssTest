...
BEGIN 
  DECLARE
    ...
    @v_taqprojectsearchitemcode INT,
    @v_taqusageclasscode INT,
 ...
 -- PO   
 SELECT  @v_taqprojectsearchitemcode = searchitemcode, @v_taqusageclasscode = usageclasscode
    FROM taqproject WHERE taqprojectkey = @i_related_projectkey
 ...
 -- END
  ...
  -- boundbookdate
  -- determine value of optionid 6 from clientoptions
  SELECT @v_prodboundbookdate_value = optionvalue FROM clientoptions WHERE optionid = 6 -- prod boundbookdate
  IF @v_prodboundbookdate_value = 1 
	SELECT @v_boundbookdatecode = datetypecode FROM datetype WHERE qsicode = 22  -- production bound book date
  ELSE
    SELECT @v_boundbookdatecode = datetypecode FROM datetype WHERE qsicode = 21  -- bound book date
  ...
  -- Vendor Information
   ...
   IF @v_roletypecode > 0 BEGIN
	SELECT @v_vendor_globalcontactkey = c.globalcontactkey, @i_globalcontactrelationshipkey = r.globalcontactrelationshipkey
	  FROM taqprojectcontact c, taqprojectcontactrole r
	  WHERE c.taqprojectcontactkey = r.taqprojectcontactkey AND 
		  c.taqprojectkey = @i_related_projectkey AND
		  r.rolecode = @v_roletypecode
  ...
  IF @v_count_gpo = 0 BEGIN
	  ...
	  select @v_vendorattn = SUBSTRING(@v_vendorattn,1,30)	
	  ...
	  INSERT INTO gpo (gpokey,gponumber,gpochangenum,gpodate,gpostatus,warehousedate,prodcontact,
		...
		vendoraddress2,vendorattn, vendorcity,vendorstate,vendorzipcode,vendorid,maxshiptovendorkey,potypekey)
		VALUES (@v_gpokey,@v_gponumber,@v_gpochangenumber,@v_gpodate, @v_gpostatus,@v_warehousedate,@v_globalcontactkey,
		  ...
		  @v_address2,@v_vendorattn,@v_city,@v_state,@v_zipcode,@v_vendorid,@v_number_of_shiplocations,@v_potypekey)
  END
  ELSE BEGIN
  ...
  EXEC dbo.qpo_generate_po_details @i_projectkey,@i_related_projectkey,@v_gpokey ,@i_lastuserid,  @o_error_code output, @o_error_desc output
  IF @o_error_code < 0 BEGIN
	  SET @o_error_code = -1
  ...
  IF @v_count_task = 0 BEGIN
	 exec get_next_key @lastuserid_var, @v_newkey output
  ...
  FROM coreprojectinfo 
  WHERE projectkey = @i_projectkey
  -- Copy the 1st PO number to of the 1st created report to the Purchase Order.
  ...
  WHERE v.relatedprojectkey = p.taqprojectkey
   ...
   IF @v_count = 1 BEGIN 
    SELECT @v_ponumber_from_report = productnumber FROM projectrelationshipview v
	  ,taqproject p
	  ,taqproductnumbers n
    WHERE v.relatedprojectkey = p.taqprojectkey
	  ...
	  AND p.searchitemcode = @v_po_itemtypecode
	  AND p.usageclasscode <> @v_po_usageclasscode
	  AND n.productidcode in (select datacode from gentables where tableid = 594 and qsicode = 7)
...
GRANT EXEC on [dbo].[qpo_generate_po_report] to PUBLIC
go
