...
@v_ssn_globalcontact varchar (20),
@v_ssn_import varchar (20),
...
--############################################################################################	
    ...
    BEGIN
				...
				if @v_individual = 0 begin
					...
					--compare import ssn and existing ssn
					select @v_ssn_globalcontact = ssn
					from globalcontact
					where globalcontactkey = @v_global_contactkey_match

					select @v_ssn_import = ssn
					from globalcontact_import 
					where globalcontactrequestkey = @i_globalcontactrequestkey

					-- is is null update ssn
					if @v_ssn_import is not null begin
						update globalcontact
						set ssn = @v_ssn_import
						where globalcontactkey = @v_global_contactkey_match
			
					end
					--write message if ssn are diferent
					if @v_ssn_import <> @v_ssn_globalcontact and @v_ssn_globalcontact is not null and @v_ssn_import is not null begin
						update globalcontact
						set globalcontactnotes  = (select isNull(globalcontactnotes, '') + ' Replaced old SSN ' +  @v_ssn_globalcontact + ' with new ' + @v_ssn_import + '.' + @v_cariadgereturn
												   from globalcontact
												   where globalcontactkey = @v_global_contactkey_match) 
						where globalcontactkey = @v_global_contactkey_match
						set @v_errind = 1
