  ...
  if @cnt > 0 begin
    ...
    IF @i_isbn_prefix_code > 0
      insert into taqprojecttitle
        ...
        @i_isbn_with_dashes, @isbn, @i_ean_with_dashes, @ean, @i_gtin_with_dashes, @gtin, @v_new_itemnumber, suser_sname(), getdate())
    ELSE
      insert into taqprojecttitle
        (taqprojectformatkey, taqprojectkey, itemnumber, lastuserid, lastmaintdate)
		  values
        (@i_formatkey, @i_projectkey, @v_new_itemnumber, suser_sname(), getdate())
