...
BEGIN
  ...
  WHERE tableid = 567 AND 
    NOT EXISTS (SELECT * FROM gentables g
                WHERE g.tableid = 567 AND gentablesitemtype.datacode = g.datacode)
  ...
  FROM gentables
  WHERE tableid = 567 AND LOWER(datadesc) = 'n/a' AND qsicode = 1
  ...
  BEGIN
    ...
    BEGIN
      ...
      VALUES
        (@v_newkey, 567, @v_datacode, @v_itemtypecode, @v_usageclass, 'QSIDBA', getdate(), 1)
    END
  END
END
go
