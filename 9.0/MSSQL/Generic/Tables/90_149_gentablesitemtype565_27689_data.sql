...
BEGIN
  ...
  WHERE tableid = 565 AND 
    NOT EXISTS (SELECT * FROM gentables g
                WHERE g.tableid = 565 AND gentablesitemtype.datacode = g.datacode)
  ...
  BEGIN
    ...
    BEGIN
      ...
      VALUES
        (@v_newkey, 565, @v_datacode, @v_itemtypecode, @v_usageclass, 'QSIDBA', getdate(), 1)
    END
  END
END
go
