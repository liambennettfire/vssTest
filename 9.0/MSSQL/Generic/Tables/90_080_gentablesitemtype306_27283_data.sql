DECLARE 
    ...
    SELECT @v_itemtypecode_title = datacode, @v_usageclasscode_title = datasubcode FROM subgentables where tableid = 550 and qsicode = 26
	  ...
	  DECLARE printing_gentablesitemtype_cur CURSOR FOR
		SELECT DISTINCT datacode from gentablesitemtype where tableid = 306 and itemtypecode = @v_itemtypecode_title AND itemtypesubcode IN (0, @v_usageclasscode_title)  
		AND datacode NOT IN (SELECT DISTINCT datacode from gentablesitemtype where tableid = 306 and itemtypecode = @v_itemtypecode_printing)
	  OPEN printing_gentablesitemtype_cur

	  FETCH NEXT FROM printing_gentablesitemtype_cur INTO @v_datacode_pricetype
      ...
      EXEC dbo.get_next_key 'QSIDBA', @v_newkey OUT
		  ...
		  VALUES
			(@v_newkey, 306, @v_datacode_pricetype, 0, @v_itemtypecode_printing, 0, 'QSIDBA', getdate())
