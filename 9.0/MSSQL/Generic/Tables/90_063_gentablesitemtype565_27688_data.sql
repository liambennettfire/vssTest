...
BEGIN
  ...
  BEGIN
    ...
    VALUES
      (@v_newkey, 565, @v_datacode, @v_work_itemtype, @v_work_usageclass, 'QSIDBA', getdate())
...
BEGIN
  ...
  BEGIN
    ...
    INSERT INTO gentablesitemtype
      (gentablesitemtypekey, tableid, datacode, itemtypecode, itemtypesubcode, lastuserid, lastmaintdate)
    VALUES
      (@v_newkey, 565, @v_datacode, @v_project_itemtype, @v_project_usageclass, 'QSIDBA', getdate())
...
IF NOT EXISTS (SELECT * FROM gentables WHERE tableid = 565 AND LOWER(datadesc) = 'active') BEGIN
  SELECT @v_datacode = COALESCE(MAX(datacode),0) + 1 FROM gentables WHERE tableid = 565

  SET @v_datadesc = 'Active'
  SET @v_datadescshort = 'Active'
  SET @v_tableid = 565
...
BEGIN
  ...
  INSERT INTO gentablesitemtype
    ...
    VALUES
      (@v_newkey, 565, @v_datacode, @v_printing_itemtype, @v_printing_usageclass, 'QSIDBA', getdate(),1)
END
go