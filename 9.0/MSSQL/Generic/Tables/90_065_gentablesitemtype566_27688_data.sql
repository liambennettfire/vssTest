...
BEGIN
  ...
  BEGIN
    ...
    VALUES
      (@v_newkey, 566, @v_datacode, @v_work_itemtype, @v_work_usageclass, 'QSIDBA', getdate())
...
BEGIN
  ...
  BEGIN
    ...
    INSERT INTO gentablesitemtype
      (gentablesitemtypekey, tableid, datacode, itemtypecode, itemtypesubcode, lastuserid, lastmaintdate)
    VALUES
      (@v_newkey, 566, @v_datacode, @v_project_itemtype, @v_project_usageclass, 'QSIDBA', getdate())
...
IF NOT EXISTS (SELECT * FROM gentables WHERE tableid = 566 AND qsicode = 1) BEGIN
  SELECT @v_datacode = COALESCE(MAX(datacode),0) + 1 FROM gentables WHERE tableid = 566

  SET @v_datadesc = 'System Generated'
  SET @v_datadescshort = 'System'
  SET @v_tableid = 566
  SET @v_tablemnemonic = 'PLType'
  SET @v_qsicode = 1
...
BEGIN
  ...
  INSERT INTO gentablesitemtype
    ...
    VALUES
      (@v_newkey, 566, @v_datacode, @v_printing_itemtype, @v_printing_usageclass, 'QSIDBA', getdate(),1)
END
go