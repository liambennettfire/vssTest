...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[person_cuts_pos]') and OBJECTPROPERTY(id, N'IsView') = 1)
drop view [dbo].[person_cuts_pos]
GO

CREATE VIEW person_cuts_pos AS
  ...
  (SELECT datadesc FROM gentables 
   ...
   (SELECT TOP 1 rolecode FROM globalcontactrole r 
    WHERE r.globalcontactkey = c.globalcontactkey AND 
      r.keyind = 1 order by rolecode) defaultroletypecode,
...
GRANT  SELECT  ON [dbo].[person_cuts_pos]  TO [public]
GO
