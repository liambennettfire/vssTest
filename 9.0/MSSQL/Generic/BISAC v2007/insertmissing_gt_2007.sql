declare
  ...
  declare missingcodes cursor for 
    select code,Literal
      from temp_gt_bisaccodes_2007
      where code not in 
        (select bisacdatacode from gentables 
           where temp_gt_bisaccodes_2007.code=gentables.bisacdatacode
             and tableid=339) 
  ...
  while @@fetch_status = 0
    begin
       ...
       insert into gentables
         ...
         values
           (339,'BISAC SUBJECT',@gentabledatacode,@datadesc,@bisacdatacode,'N',1,1,0,0,@bisacdatacode,getdate(),'qsi-sql-V2007')
