...
DECLARE replacement_cursor CURSOR FOR
	SELECT code,replacementcode 
      FROM temp_replacement_codes_2014
					...
					WHILE (@@FETCH_STATUS = 0) BEGIN
					    ...
					    IF @v_mediatypecode > 0 BEGIN
							...
							SELECT @v_count4 = count(*)
							  FROM bookbisaccategory WHERE bookkey = @v_bookkey AND printingkey = @v_printingkey AND bisaccategorycode = @v_replacement_datacode
							...
							IF @v_count4 = 0 BEGIN
								 ...
								 UPDATE bookbisaccategory
									SET bisaccategorycode = @v_replacement_datacode,
										...
										lastuserid = 'FB_BISACSUBJECT_UPDATE_2014',
								  ...
								  IF @v_replacement_datacode <> @v_datacode BEGIN
									  ...
									  exec qtitle_update_titlehistory 'bookbisaccategory', 'bisaccategorycode' , @v_bookkey, 1, 0, @v_replacement_datacode_desc, 'Update', 'FB_BISACSUBJECT_UPDATE_2014', null,'Bisac Heading', @o_error_code output, @o_error_desc output
								  END
								  ...
								  IF @v_replacement_datasubcode <> @v_datasubcode BEGIN
									  ...
									  exec qtitle_update_titlehistory 'bookbisaccategory', 'bisaccategorysubcode' , @v_bookkey, 1, 0, @v_replacement_datasubcode_desc, 'Update', 'FB_BISACSUBJECT_UPDATE_2014', null,'Bisac Sub Heading', @o_error_code output, @o_error_desc output
						 		  END
