...
BEGIN 
  ...
  IF COALESCE(@i_tableid,0) = 0 OR COALESCE(@i_datacode,0) = 0 OR COALESCE(@i_datasubcode,0) = 0
    RETURN 2
  ...
  SELECT @object_accessind_var = accessind   
    ...
    WHERE o.availsecurityobjectkey = a.availablesecurityobjectskey AND  
          ...
          ISNULL(o.datacode, @i_datacode) = @i_datacode AND
          ISNULL(o.datasubcode, @i_datasubcode) = @i_datasubcode AND
