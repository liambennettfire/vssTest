...
**	--------    ------  -----------
**  05/17/2018   SB		Removed unnecessary tables and calls to improve speed 
**********************************************************************************************************************/
BEGIN
  DECLARE 
    @v_taqprojectkey	INT,
    ...
    INSERT INTO @contractstitlestable  --Get titles for Works related to this contract
    ...
    --FROM taqproject tp  --(5/17/18)
	FROM coreprojectinfo cp
        LEFT OUTER JOIN @relatedprojectsinfo pv ON cp.projectkey = pv.taqprojectkey
    ...
    WHERE pv.projectsearchitemcode = @v_contract_itemtype -- Contract
	    ...
	    --AND COALESCE(tmw.masterworkprojectkey,0) > 0  (REMOVED 5/17/18 to allow the 2 queries to be combined into 1)
	    ...
	    AND cp.projectkey = @i_taqprojectkey


    --INSERT INTO @contractstitlestable  (REMOVED 5/17/18 - made first query handle the Contracts related to Works withot Masters as well as Works with Masters)
    --SELECT DISTINCT pv.taqprojectkey contractprojectkey,pv.relatedprojectkey workprojectkey,
	   -- ct.bookkey,ct.printingkey,cp.projecttitle contractdisplayname, cp.projectparticipants contractparticipants,
	   -- cp.projecttypedesc contracttypedesc, cp.projectstatusdesc contractstatusdesc, 
	   -- cp.projectowner, cp.searchitemcode, cp.usageclasscode, COALESCE(cp.templateind,0) templateind, 
    ...
    INSERT INTO @contractstitlestable --Get titles for Master Works related to this contract
