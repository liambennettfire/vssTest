/****** Object:  UserDefinedFunction [dbo].[rpt_get_author]    Script Date: 3/4/2016 2:55:09 PM ******/
DROP FUNCTION [dbo].[rpt_get_author]
GO
...
CREATE FUNCTION [dbo].[rpt_get_author] (@i_bookkey	INT,	@i_order	INT, @i_type		INT, 	@v_name		VARCHAR(1))
	RETURNS	VARCHAR(120)
		...
		BEGIN
			...
			ELSE IF @v_name = 'Y' 
				BEGIN
					...
					SELECT @v_desc =  CASE 
							WHEN @v_firstname IS  NULL THEN ''
	            		ELSE @v_firstname End

						+CASE 
							WHEN  nullif(@v_middlename,'') IS  NULL THEN ''
	            		ELSE ' '+@v_middlename End
						+
						CASE 
							WHEN nullif(@v_lastname,'') IS NULL THEN  ''
							else ' '+@v_lastname End
...
Grant All on dbo.rpt_get_author to Public
go