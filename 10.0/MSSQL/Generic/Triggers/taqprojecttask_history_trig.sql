IF EXISTS (SELECT * FROM sysobjects WHERE type = 'TR' AND name = 'taqprojecttask_history')
	BEGIN
		DROP  Trigger dbo.taqprojecttask_history
	END
GO

CREATE TRIGGER [dbo].taqprojecttask_history ON [dbo].taqprojecttask
...
DECLARE 
  ...
  -- Skip logic if this is a cloud based task
  IF EXISTS (SELECT * FROM inserted WHERE cseventid IS NOT NULL)
	RETURN
	
  IF EXISTS (SELECT * FROM deleted WHERE cseventid IS NOT NULL)
	RETURN
  ...
  while @v_tranacation_row_total>=@v_tranacation_row_pointer
    begin
      ...
      SELECT * into #inserted FROM  
        (SELECT Row_Number() OVER (ORDER BY taqprojectformatkey) as transaction_rowid,*  
        FROM inserted) as a  
