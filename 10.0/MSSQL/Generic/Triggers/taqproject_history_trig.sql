IF EXISTS (SELECT * FROM sysobjects WHERE type = 'TR' AND name = 'taqproject_history')
	BEGIN
		DROP  Trigger dbo.taqproject_history
	END
GO

CREATE TRIGGER [dbo].taqproject_history ON [dbo].taqproject
  ...
  while @v_tranacation_row_total>=@v_tranacation_row_pointer
    begin
      ...
      SELECT * into #inserted FROM  
        (SELECT Row_Number() OVER (ORDER BY taqprojectkey,taqprojectownerkey) as transaction_rowid,*  
        FROM inserted) as a  
