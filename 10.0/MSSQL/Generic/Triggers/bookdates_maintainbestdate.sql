...
DECLARE
  ...
  @v_prodbbd_datetypecode INT,
  @v_bbd_datetypecode INT
...
--end
    ...
    SELECT @v_boundbookdatecode = COALESCE(clientdefaultvalue,0) 
      FROM clientdefaults 
     WHERE lower(clientdefaultname) = 'production bound book date type code'
     
    SELECT  @v_prodbbd_datetypecode = datetypecode FROM datetype WHERE qsicode = 22  --Production Bound Book Date
    SELECT  @v_bbd_datetypecode = datetypecode FROM datetype WHERE qsicode = 21  -- Bound Book Date
      
    IF @v_prodbbd_datetypecode = @v_boundbookdatecode
		  SET @v_useProdBBD = 1
	  ELSE IF @v_bbd_datetypecode = @v_boundbookdatecode
		  SET @v_useProdBBD = 0
	  ELSE
		  SET @v_useProdBBD = 0	
	...
	/** REMOVE CURSOR SINCE CAUSING ERROR
		...
		**/
        ...
        if ( @useWebScheduling = 1)
          BEGIN
            EXECUTE dbo.get_next_key 'taqprojecttask', @v_new_key OUTPUT
