...
**********************************************************************************/
  ...
  DECLARE @v_csapprovalcodedesc  VARCHAR(40) 
  ...
  BEGIN
    ...
    IF ([dbo].qcs_get_csapproved(@v_bookkey) = 1) BEGIN  --Approved for distribution
    ...
    END /* @v_csapprovalcode = 1 OR @v_csapprovalcode = 4 */
    ELSE IF @v_csapprovalcode = 3 BEGIN   /*Never Approve */
      ...
      SET @v_messagetypecode = NULL
      SET @v_messagetypeqsicode = NULL
      SET @v_messagelongdesc = 'Title is set to Never Approve. The automated process will not approve this title; it must be done through Title Management' 
      SET @v_messageshortdesc = NULL
    ...
    ELSE BEGIN 
       IF @v_csdisttemplatekey > 0 BEGIN  --Send is requested for all approved assets
		   ...
		   -- Case: 28060 Automated cloud approval process needs to write title history
		   SELECT @v_csapprovalcodedesc = dbo.get_gentables_desc(620,@v_csapprovalcode,'long')
		   
       SET @v_error_code = 0
       SET @v_error_desc = ''
			
       EXEC dbo.qtitle_update_titlehistory 'bookdetail','csapprovalcode', @v_bookkey,1, 0, @v_csapprovalcodedesc,
	          'update', @v_orig_userid, NULL, NULL , @v_error_code output, @v_error_desc output
	     ...
	     IF @v_error_code < 0 BEGIN
		 	  -- Error
				...
				SET @v_messagetypecode = NULL
				SET @v_messagetypeqsicode = NULL
				SET @v_messagelongdesc = @v_error_desc
				SET @v_messageshortdesc = NULL
				SET @v_messagecode = NULL
				SET @o_error_code = 0
				SET @o_error_desc = ''
