...
GO
CREATE PROCEDURE [dbo].[qcs_get_product](
  @bookkey int,
...
BEGIN
	...
	DECLARE @bgpFailProduct bit=0
	DECLARE @bgpErrorFromHistory bit=0
	DECLARE @bgpCount INT
	DECLARE @bgpLastAttemptInt DATETIME
	DECLARE @bgpRetCode INT
	DECLARE @bgpRetStatus VARCHAR(2000)=''
	DECLARE @bgpRetVal VARCHAR(2100)=''

	SELECT @bgpCount=COUNT(*)
	FROM backgroundprocess
	WHERE key1=@bookkey
	AND reqforgetprodind=1

	IF (@bgpCount>0) BEGIN
		SELECT @bgpLastAttemptInt = GETDATE()

		EXEC qutl_processbackgroundjobs
			@i_bookkey=@bookkey,
			@i_numofrows=2147483647,
			@i_failedattemptinterval=@bgpLastAttemptInt,
			@o_error_code=@bgpRetCode,
			@o_error_desc=@bgpRetStatus

		IF (@bgpRetCode = -1) SET @bgpFailProduct=1
	END
	...
	WHERE key1=@bookkey
	AND failgetprodind=1

	IF (@bgpCount>0) SET @bgpFailProduct=1

	-- If this has failed Background Processing calculate return value
	IF (@bgpFailProduct=1 AND @bgpRetStatus='') BEGIN

		DECLARE @retStatuses TABLE (
			sequence int identity(1, 1),
			retStatus varchar(max)
		)
		INSERT INTO @retStatuses
		SELECT DISTINCT ReturnMsgDesc
		FROM backgroundprocess
		WHERE key1=@bookkey
		AND reqforgetprodind=1
		AND ReturnMsgDesc IS NOT NULL AND ReturnMsgDesc!=''
		UNION
		SELECT DISTINCT ReturnMsgDesc
		FROM backgroundprocess_history
		WHERE key1=@bookkey
		...
		SELECT @bgpRetStatus+=retStatus
		FROM @retStatuses
		ORDER BY sequence
	END

	IF (@bgpFailProduct=1) BEGIN
		DECLARE @recentErrorCount INT
		DECLARE @historyErrorCount INT

		SELECT @recentErrorCount = COUNT(*)
		FROM backgroundprocess
		WHERE key1=@bookkey
		AND reqforgetprodind=1
		AND processingind=0

		SELECT @historyErrorCount = COUNT(*)
		FROM backgroundprocess_history
		WHERE key1=@bookkey
		AND failgetprodind=1
		IF (@recentErrorCount < 1 AND @historyErrorCount >= 1) SET @bgpErrorFromHistory = 1
	END
	...
	IF (@bgpFailProduct=1 AND @bgpRetStatus='') BEGIN
		...
		IF(@bgpCount>0) SET @bgpRetStatus = ' This title cannot be sent to the cloud because there are currently processes running that are required to finish first.'
	END

	IF (@bgpFailProduct=1) BEGIN
		SET @o_error_code= -1

		IF (@bgpErrorFromHistory=1)BEGIN
			SET @o_error_desc = 'The title for bookkey: '+ CAST(@bookkey AS VARCHAR(50)) + ' previously failed background processing'
			IF (@bgpRetStatus!='') SET @o_error_desc = @o_error_desc + ' with: ''' + @bgpRetStatus + ''''
			SET @o_error_desc = @o_error_desc + ' and cannot be sent to the cloud until these have been resolved.'
		END
		ELSE BEGIN
			SET @o_error_desc = 'The title for bookkey: '+ CAST(@bookkey AS VARCHAR(50)) + ' failed background processing'
			IF (@bgpRetStatus!='') SET @o_error_desc = @o_error_desc + ' with: ''' + @bgpRetStatus + ''''
		END
		RETURN
	END
  ...
  SELECT @SendRightsToCloud = optionvalue
   ...
   WHERE optionid = 114
    ...
    SELECT TOP 1
		...
		-- 07/20/17 - Case : 46386 new age low and high logic (TT)
		(Case -- most comment scenario is in first when statement, user populates both agelow and agehigh
		WHEN ISNULL(agelow, -1) >= 0 and ISNULL(agehigh, -1) > 0 THEN Cast(agelow as INT) -- case 0 to 99, allow for 0 or any number only if agehigh is greater than 0 
		WHEN ISNULL(allagesind, 0) = 1 THEN 0 -- assuming all ages will be translated to 0 to 99
		WHEN ISNULL(agelowupind, 0) = 1 and ISNULL(agehigh, -1) > 0 THEN 0 -- assuming UP to 10 will be translated to 0-10
		WHEN ISNULL(agehighupind, 0) = 1 and ISNULL(agelow, -1) >=0 THEN Cast(agelow as INT) -- assuming 5 and UP will be translated to 5-99, 0 is allowed for agelow here. TM accepts 0 and UP 
		WHEN ISNULL(agelow, -1) = -1 and ISNULL(agehigh, -1) > 0 THEN 0
		ELSE NULL END) as AgeLow,

		(Case -- most comment scenario is in first when statement, user populates both agelow and agehigh
		WHEN ISNULL(agelow, -1) >= 0 and ISNULL(agehigh, -1) > 0 THEN Cast(agehigh as INT)-- case 0 - 99, allow for 0 or any number only if agehigh is greater than 0 
		WHEN ISNULL(allagesind, 0) = 1 THEN 99 -- assuming all ages will be translated to 0 to 99
		WHEN ISNULL(agelowupind, 0) = 1 and ISNULL(agehigh, -1) > 0 THEN Cast(agehigh as INT) -- assuming UP to 10 will be translated to 0-10, 0 is not allowed for agehigh UP to 0 does not make sense. TM takes it but doesn't save 0 in agehigh
		WHEN ISNULL(agehighupind, 0) = 1 and ISNULL(agelow, -1) > = 0 THEN 99 -- assuming 8 and UP will be translated to 8 to 99
		WHEN ISNULL(agelow, -1) = -1 and ISNULL(agehigh, -1) > 0 THEN Cast(agehigh as INT)
		ELSE NULL END) as AgeHigh,
...
--send product's item number
	...
	UNION 
	/*
	This is the generic table valued function that can return client specific data. See Quarto's database for examples. 
	*/
	SELECT  
		NEWID() AS Id,
		Tag, 
...
GO
GRANT EXEC ON [qcs_get_product] TO PUBLIC
GO