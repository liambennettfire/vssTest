...
**   03/15/2017   Colman         43850     Added datacode parameter
**   11/14/2017   Colman         48292     Return in datasubcode order. Participant by Role column config relies on it.
**   04/05/2018   Colman         50733     Configurable sections should not require a item type filtering row for all columns
*************************************************************************************************************************************/
  ...
  DECLARE @gentablesitemtypeinfo TABLE (
    datacode int null,
  ...
  IF @i_usageclass > 0 BEGIN 
	  ...
	  WHERE gi.tableid = s.tableid AND
		  ...
		  gi.itemtypesubcode = @i_usageclass AND
      (@i_datacode = 0 OR gi.datacode = @i_datacode)
  END
  ...
  INSERT INTO @gentablesitemtypeinfo	  
  SELECT s.datacode, s.datasubcode, s.datadesc, s.sortorder, NULL, NULL, NULL
  FROM subgentables s
  WHERE s.tableid = @i_tableid AND
      (@i_datacode = 0 OR s.datacode = @i_datacode)
      AND NOT EXISTS(SELECT * FROM @gentablesitemtypeinfo t WHERE t.datacode = s.datacode AND t.datasubcode = s.datasubcode)   
  
  SELECT @v_error = @@ERROR
  IF @v_error <> 0 BEGIN
    SET @o_error_code = 1
    SET @o_error_desc = 'Error accessing subgentables table (tableid=' + CONVERT(VARCHAR, @i_tableid) + ').'
    RETURN
  END
