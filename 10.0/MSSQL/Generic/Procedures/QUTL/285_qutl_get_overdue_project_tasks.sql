...
CREATE PROCEDURE [dbo].[qutl_get_overdue_project_tasks]
 (@i_userkey        integer,
  ...
  @i_GlobalRoleAs	integer,  --"0" = Project Owner, "1" = Contact 1, "2" = Contact 2, "3" = All, "4" =  Contact 1 or Contact 2
...
**  10/26/2016 - Uday -  Case 41387 - Go to Printing Summary from Overdue Tasks
**  01/27/2016 - Uday -  Case 41387 - Task 003
**  03/31/2017 - Uday -  Case 44109
...
--print 'ToDate = ' + @ToDate
 ...
 --print 'Number of globalcontact keys = ' + convert(varchar(20), @Numkeys)
  ...
  select @sqlCmd = '	SELECT t.taqtaskkey, t.globalcontactkey, t.taqprojectkey as projectkey, t.bookkey, t.printingkey, t.keyind,
                        ...
                        t.sortorder,
					    p.printingnum,
						v.taqprojectkey as printingprojectkey		                          
						...
						LEFT JOIN printing p on t.bookkey = p.bookkey AND t.printingkey = p.printingkey
						LEFT OUTER JOIN taqprojectprinting_view v ON t.bookkey = v.bookkey
							AND t.printingkey = v.printingkey
						JOIN coreprojectinfo c2 on t.taqprojectkey = c2.projectkey
						WHERE (t.actualind = 0 OR t.actualind IS NULL) 
              AND (COALESCE(d.usedexclusivelybycsind,0) = 0) 
  ...
  else if (@i_GlobalRoleAs = 4) -- Contact 1 OR Contact 2 
  begin
	select @asSQL = ' AND (t.globalcontactkey in ( ' + @KeyList + ' ) OR t.globalcontactkey2 in ( ' + @KeyList + ' ) )'
  end
