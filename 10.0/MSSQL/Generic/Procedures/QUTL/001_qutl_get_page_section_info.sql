...
--  END 
  ...
  IF @v_windowviewkey > 0 BEGIN
    SELECT cd.configdetailkey, cd.configobjectkey, cd.qsiwindowviewkey, co.miscsectionind, COALESCE(co.tabgroupsectionind,0) tabgroupsectionind ,cd.usageclasscode, 
     ...
     WHERE cd.configobjectkey = co.configobjectkey
       ...
       AND visibleind = 1
    UNION
    SELECT configdetailkey, configobjectkey, qsiwindowviewkey, miscsectionind,  COALESCE(tabgroupsectionind,0) tabgroupsectionind, usageclasscode,
