...
**  09/06/2017	 Colman      Case 46736 - @v_sql can overflow
*******************************************************************************/
 ...
 DECLARE
  ...
  END
	...
	ELSE IF @v_misctype = 10 BEGIN -- DATE
		...
		END			
	END	
	ELSE IF @v_misctype = 11 -- SEARCH INT
		EXEC execute_calcsql_searchint @v_sql, @v_calcvalue OUTPUT
	ELSE IF @v_misctype = 12 -- SEARCH STRING
		EXEC execute_calcsql_searchstr @i_misckey, @v_calcvalue OUTPUT
