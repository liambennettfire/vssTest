...
BEGIN
	...
	SELECT @v_itemtypecode = COALESCE(@v_itemtypecode, co.itemtypecode),
		   ...
		   @v_titletabs = CASE WHEN co.sectioncontrolname like '%TabGroupTitleSection.ascx%' THEN 1 ELSE 0 END,
	...
	SELECT i.datacode, COALESCE(i.sortorder, g.sortorder)
		...
		WHERE ((@v_titletabs = 0 AND i.tableid = 583) OR (@v_titletabs = 1 AND i.tableid = 440))
	...
	BEGIN
		...
		IF @v_count = 0 BEGIN
			...
			VALUES(@i_configdetailkey,@v_datacode,@v_sortorder,@v_titletabs,'QSIDBA',GETDATE())
