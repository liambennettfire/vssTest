  ...
  BEGIN
    ...
    BEGIN
      ...
      ELSE
        SELECT @v_count = COUNT(*)
        FROM taqprojecttask
        WHERE ((taqprojectkey = @i_taqprojectkey AND COALESCE(bookkey,0) = COALESCE(@i_bookkey,0))
					OR
					(bookkey = COALESCE(@i_bookkey,0) AND printingkey = COALESCE(@i_printingkey, 0)))
          AND datetypecode = @i_datetypecode
      ...
      BEGIN
        ...
        ELSE
          ...
          WHERE ((t.taqprojectkey = @i_taqprojectkey AND COALESCE(t.bookkey,0) = COALESCE(@i_bookkey,0))
					OR
					(t.bookkey = COALESCE(@i_bookkey,0) AND t.printingkey = COALESCE(@i_printingkey, 0)))
