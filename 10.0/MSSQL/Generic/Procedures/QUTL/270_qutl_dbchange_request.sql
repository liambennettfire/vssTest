...
**    08/19/2016   Colman         Case 39094 Title Territory Propagation
**    07/13/2017   Alan           Case 46191 Changes to Contact Bio not triggering sends to outbox
**    09/28/2017   Colman         Case 46642 Title history not written out to when propagating or copying Keywords
**    05/21/2018   Colman         Case 51333 Printing number does not increment when adding new printing
**    06/12/2018   Colman         Case 42604 Declare large varchar variables as max
***********************************************************************************************/
...
--insert into mk_DebugInfo select @i_transaction_xml,GETDATE(),'line 37 DB_ChangeReq'
  ...
  @CommentTypeCode INT,
	...
	@DBItemDesc     VARCHAR(max),
	@DBItemValue		VARCHAR(max),
	...
	@UserID			VARCHAR(30),
	@SQLInsertColumnString	VARCHAR(max),
	@SQLInsertValueString	VARCHAR(max),
	@SQLDeleteString	VARCHAR(max),
	@SQLUpdateString	VARCHAR(max),
	@SQLWhereString		VARCHAR(max),
	@SQLString		NVARCHAR(max),
	@TestSQLString	NVARCHAR(max),
	@XMLParameter     VARCHAR(max),
  ...
  @v_pricekey INT,
  @v_pricetypecode INT,
  @v_template_fieldtag VARCHAR(255),
  ...
  DECLARE action_cursor CURSOR LOCAL FOR 
    ...
    WITH (ActionSequence VARCHAR(10) 'ActionSequence',
         ...
         Parameters VARCHAR(max) 'Parameters',
         XMLParameter VARCHAR(max) 'XMLParameter')
  ...
  BEGIN
    ...
    IF @ActionTable = 'territoryrights' OR @ActionTable = 'territoryrightcountries'
    BEGIN
      ...
      WHERE columnkey = 272 
        AND activeind = 1
        AND workfieldind = 1
      SET @IsMultiColumnHistoryTable = 1
    END
    ELSE BEGIN
      SELECT @CheckCount = count(*)
      FROM titlehistorycolumns 
      WHERE LOWER(tablename) = LOWER(@ActionTable)
      SET @IsMultiColumnHistoryTable = 0
    END
    ...
    BEGIN
      ...
      IF Lower(@ActionTable) = 'bookkeywords' AND @KeyColumn = 'id'
        SET @KeywordId = @KeyValue
	  ...
	  IF @KeyColumn = 'pricekey' and LOWER(@ActionTable) = 'bookprice' BEGIN
		SET @v_pricekey = @KeyValue
	  END
    ...
    -- Skip propagation if territory rights are not related to a title
    IF @BookKey = 0 AND (LOWER(@ActionTable) = 'territoryrights' OR LOWER(@ActionTable) = 'territoryrightcountries')
      SET @IsTitleHistoryTable = 0
...
--            print @TempKey
        ...
        BEGIN
          ...
          BEGIN
            ...
            ELSE IF LOWER(@ActionTable) = 'bookprice' AND LOWER(@DBItemColumn) = 'pricetypecode' AND LEN(COALESCE(@DBItemValue, '')) > 0 --AND COALESCE(@v_pricekey,0) > 0
			      BEGIN
			        SET @v_pricetypecode = @DBItemValue
			        SET @v_template_fieldtag = NULL

			        SELECT @v_template_fieldtag = gentext1
			        FROM gentables_ext
			        WHERE tableid = 306
			          AND datacode = @v_pricetypecode

			        --insert into cloudscheduleforresend
			        --(bookkey, templatekey, assetkey, lastuserid, lastmaintdate)
			        --values
			        --(@v_pricetypecode, 0, 0, 'TEST', GETDATE())

			        EXEC qtitle_update_titlehistory @ActionTable, @DBItemColumn, @BookKey, 
                      @PrintingKey, @DateTypeCode, @DBItemDesc, @ActionType, @UserID, 
                      @HistoryOrder, @FieldDescDetail, @o_error_code OUTPUT, @o_error_desc OUTPUT, @v_template_fieldtag
			      END
			      ELSE IF @IsMultiColumnHistoryTable = 0
              EXEC qtitle_update_titlehistory @ActionTable, @DBItemColumn, @BookKey, 
                @PrintingKey, @DateTypeCode, @DBItemDesc, @ActionType, @UserID, 
            ...
            IF LOWER(@ActionTable) = 'qsicomments' AND COALESCE(@v_citation_bookkey, 0) > 0 BEGIN
              ...
              END      
            END                      
            ELSE IF @IsMultiColumnHistoryTable = 1 BEGIN
              SELECT @v_count = COUNT(*)
              FROM #propagatetitle
              ...
              IF @v_count = 0 BEGIN
                INSERT INTO #propagatetitle (bookkey,tablename,columnname)
                VALUES (@BookKey,@ActionTable,'(multiple)')
              END 
            END ELSE
            BEGIN
              SELECT @v_count = COUNT(*)
          ...
          BEGIN
            ...
            ELSE IF LOWER(@ActionTable) = 'qsicomments' AND LOWER(@DBItemColumn) = 'commenthtml' BEGIN
              -- see if comment was contact biography
              DECLARE @v_bio_commenttype int,
                      @v_bio_desc VARCHAR(255)
              SELECT @v_bio_commenttype = datacode, @v_bio_desc = datadesc FROM gentables WHERE tableid=528 AND qsicode = 2
              IF coalesce(@v_bio_commenttype,0) = @CommentTypeCode BEGIN
                EXEC qcontact_update_globalcontacthistory @ActionTable, @DBItemColumn, @GlobalContactKey,
                   @v_bio_desc, @ActionType, @UserID, @FieldDescDetail, @o_error_code OUTPUT, @o_error_desc OUTPUT
        ...
        BEGIN
          ...
          ELSE IF LOWER(@ActionTable) = 'filelocation' AND @FilelocationgeneratedKey > 0 BEGIN
            ...
            EXEC qtitle_update_titlehistory @ActionTable, @HistoryColumn, @v_filelocation_bookkey, 
              ...
              @HistoryOrder, @v_filelocation_fielddesc, @o_error_code OUTPUT, @o_error_desc OUTPUT
          END             
          ELSE IF LOWER(@ActionTable) = 'bookkeywords' AND @ActionType = 'delete'
          BEGIN
            SELECT @v_currentstringvalue = keyword FROM bookkeywords WHERE bookkey = @BookKey AND id = @KeywordId
  ...
  -- Case 51333: book.nextprintingnbr is a "legacy" column that can still be (but rarely, if ever, is) used to default the next printing number to a value higher than max(printingnumber)+1
  -- If a value exists, null it out so that the sequence resumes incrementing as normal.
    IF @ActionType = 'insert' AND @ActionTable = 'printing'
    BEGIN
      IF EXISTS (SELECT 1 FROM book WHERE bookkey = @BookKey AND nextprintingnbr IS NOT NULL)
        UPDATE book SET nextprintingnbr = NULL WHERE bookkey = @BookKey
    END
