...
**    2-6-17    Olivia                  gentables 440 handling, allowing the gentables itemtype insert to handle related data code
   ...
   --If Related Data Code is passed in, verify that the related data code exists on gentables for the related tableid
    ...
    SELECT @v_count = COUNT (*) FROM gentablesitemtype
  	...
  	IF @v_count = 0 BEGIN   --Insert gentablesitemtype row
		 ...
		 END
     ELSE IF @i_tableid = 583 OR @i_tableid = 440
		 BEGIN
...
GRANT EXEC ON qutl_insert_gentablesitemtype TO PUBLIC
GO