if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qcontact_remove_comment_duplicate_markets') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qcontact_remove_comment_duplicate_markets
GO
...
CREATE PROCEDURE [dbo].[qcontact_remove_comment_duplicate_markets]
...
/******************************************************************************
**  File: 
**  Name: qcontact_remove_comment_duplicate_markets
**  Desc: Remove markets from any other comments with the same eloquence tag 
...
DELETE qsicommentmarkets FROM qsicommentmarkets m 
...
GRANT EXEC ON qcontact_remove_comment_duplicate_markets TO PUBLIC
GO
