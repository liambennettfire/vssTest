...
*   05/24/2018   Colman      Case 51499 'Job Number Alpha' product id is not generating when printings created in batch from title search
*   06/12/2018   Colman      Case 51864 Project Status Defaults to 'Active' for new printings
*******************************************************************************************************/
DECLARE
  ...
  @v_autogen_productnumber_ind INT = 0,
  @v_autogen_productnumber_proc VARCHAR(MAX)
...
BEGIN
  ...
  -- Get the default printing status
  SELECT TOP 1 @v_def_projstatus = i.datacode
  FROM gentablesitemtype i, gentables g, gentables_ext gx
  WHERE i.tableid = g.tableid AND i.datacode = g.datacode 
    AND i.tableid = gx.tableid AND i.datacode = gx.datacode AND ISNULL(gx.gen3ind, 0) = 0
    AND i.tableid = 522 AND i.itemtypecode = @v_prtg_itemtype AND ISNULL(i.itemtypesubcode, 0) IN (@v_prtg_usageclass, 0)
    AND ISNULL(g.deletestatus, 'N') = 'N'
  ORDER BY COALESCE(i.sortorder, g.sortorder)    
  ...
  SELECT @v_productidcode = g.datacode, @v_autogen_productnumber_ind = ISNULL(g.gen2ind, 0), @v_autogen_productnumber_proc = g.alternatedesc2
  FROM gentablesitemtype i
    JOIN gentables g ON i.tableid = g.tableid AND i.datacode = g.datacode
  WHERE i.tableid = 594
    AND itemtypecode = @v_prtg_itemtype
    
  IF @v_productidcode > 0 
  BEGIN
    IF @v_jobnumberalpha IS NULL AND @v_autogen_productnumber_ind = 1 AND @v_autogen_productnumber_proc IS NOT NULL
    BEGIN
      EXEC qproject_generate_productid @v_new_projectkey, 0, 0, @v_productidcode,
        0, 0, @v_autogen_productnumber_proc, @i_userid, @o_error_code OUTPUT, @o_error_desc OUTPUT
    END
    ELSE IF LEN(@v_jobnumberalpha) > 0 
    BEGIN
			EXEC dbo.get_next_key 'QSIDBA', @v_new_productnumberkey OUT
				
			INSERT INTO taqproductnumbers 
        (productnumberkey,taqprojectkey,productidcode,productnumber,sortorder,lastuserid, lastmaintdate)
			VALUES 
        (@v_new_productnumberkey,@v_new_projectkey,@v_productidcode,@v_jobnumberalpha,1,@i_userid, getdate())
    END
  END
