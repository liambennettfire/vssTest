/******************************************************************************
**  Name: imp_300010000001
**  Desc: IKE Update ISBN table
**  Auth: Bennett     
**  Date: 5/9/2016
*******************************************************************************
**  Change History
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[imp_300010000001]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[imp_300010000001]
GO

CREATE PROCEDURE dbo.imp_300010000001 
...
BEGIN
  ...
  IF @v_isbn <> @v_isbn10
    BEGIN
      ...
      SELECT @v_isbnprefix = SUBSTRING(@v_isbn10,1,1)+'-'+SUBSTRING(@v_isbn10,2,@hyphen-1)
      SELECT @v_isbnpostfix = substring(@v_isbn10,@hyphen+1,9-@hyphen)  +'-'+@varcharCheckSum
      ...
      EXECUTE qean_generate_ean @v_isbnprefix,NULL,NULL,@v_isbnpostfix,1,null,
...
GRANT EXECUTE ON dbo.[imp_300010000001] to PUBLIC 
GO
