/******************************************************************************
**  Name: imp_300021028001
**  Desc: IKE Add/Replace Custom Float 07
**  Auth: Bennett     
**  Date: 5/9/2016
*******************************************************************************
**  Change History
...
IF EXISTS (
		SELECT *
		FROM dbo.sysobjects
		WHERE id = object_id(N'[dbo].[imp_300021028001]')
			AND OBJECTPROPERTY(id, N'IsProcedure') = 1
		)
	DROP PROCEDURE [dbo].[imp_300021028001]
GO

CREATE PROCEDURE dbo.imp_300021028001 @i_batch INT
	,@i_row INT
	,@i_dmlkey BIGINT
...
AS
/* Add/Replace Custom Float 07(LTD Net Sales) */
BEGIN
	SET NOCOUNT ON
	...
	BEGIN
		...
		SET @v_errcode = 1
		SET @v_errmsg = 'Custom Float 08 Updated'
		...
		BEGIN
			...
			SELECT @v_curr_customfloat = COALESCE(customfloat08, -1)--mk20130606>Case 23834 IKE import fails to import non-empty 
			FROM bookcustom
			WHERE bookkey = @v_bookkey
			...
			BEGIN
				UPDATE bookcustom
				SET customfloat08 = @v_new_customfloat
					,lastuserid = @i_userid
		...
		IF @v_rowcount > 1
		BEGIN
			SET @v_errmsg = 'Did not update Custom Float 08'
		END
