...
IF EXISTS (
		SELECT *
		FROM dbo.sysobjects
		WHERE id = object_id(N'[dbo].[imp_300021023001]')
			AND OBJECTPROPERTY(id, N'IsProcedure') = 1
		)
	DROP PROCEDURE [dbo].[imp_300021023001]
GO

CREATE PROCEDURE dbo.imp_300021023001 @i_batch INT
	,@i_row INT
	,@i_dmlkey BIGINT
...
AS
/* Add/Replace Custom Float 03(Last MFG Qty) */
BEGIN
	SET NOCOUNT ON
	...
	BEGIN
		...
		SET @v_errcode = 1
		SET @v_errmsg = 'Custom Float 03 Updated'
		...
		BEGIN
			...
			SELECT @v_curr_customfloat = COALESCE(customfloat03, -1)--mk20130606>Case 23834 IKE import fails to import non-empty 
			FROM bookcustom
			WHERE bookkey = @v_bookkey
			...
			BEGIN
				UPDATE bookcustom
				SET customfloat03 = @v_new_customfloat
					,lastuserid = @i_userid
		...
		IF @v_rowcount > 1
		BEGIN
			SET @v_errmsg = 'Did not update Custom Float 03'
		END
