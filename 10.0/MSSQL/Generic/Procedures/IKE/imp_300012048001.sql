/******************************************************************************
**  Name: imp_300012048001
**  Desc: IKE Actual Trim Width
**  Auth: Bennett     
**  Date: 5/9/2016
*******************************************************************************
**  Change History
...
*******************************************************************************/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[imp_300012048001]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[imp_300012048001]
GO
...
CREATE PROCEDURE [dbo].[imp_300012048001] @i_batch INT,@i_row INT,@i_dmlkey BIGINT,@i_titlekeyset VARCHAR(500),@i_contactkeyset VARCHAR(500),
...
BEGIN
	...
	BEGIN
		...
		SET @v_errcode = 1
		SET @v_errmsg = 'Actual Trim Width Unchanged'
		...
		SELECT @v_elementval = COALESCE(RTRIM(LTRIM(originalvalue)), ''),@v_elementkey = b.elementkey
		 ...
		 WHERE b.batchkey = @i_batch AND b.row_id = @i_row AND b.elementseq = @i_elementseq	AND b.elementkey = d.elementkey AND d.DMLkey = @i_dmlkey
		 
		SELECT @i_options = optionvalue FROM clientoptions  WHERE optionid = 7 

		IF COALESCE(@i_options, 0) = 0
			SELECT @i_width = COALESCE(trimsizewidth, '')FROM printing WHERE bookkey = @v_bookkey AND printingkey = @v_printingkey
		ELSE
			SELECT @i_width = COALESCE(tmmactualtrimwidth, '') FROM printing WHERE bookkey = @v_bookkey	AND printingkey = @v_printingkey
		...
		*/
		...
		IF @i_width <> @v_elementval BEGIN
			SET @v_errmsg = 'Actual Trim Width Updated'

			IF COALESCE(@i_options, 0) = 0
			BEGIN
				UPDATE printing
				 SET trimsizewidth = substring(@v_elementval, 1, 10),trimsizeunitofmeasure = @v_UnitsOfMeasure,lastuserid = @i_userid,lastmaintdate = GETDATE()
			...
			ELSE BEGIN
				UPDATE printing
				   SET tmmactualtrimwidth = substring(@v_elementval, 1, 10),trimsizeunitofmeasure = @v_UnitsOfMeasure,lastuserid = @i_userid,lastmaintdate = GETDATE()
				 WHERE bookkey = @v_bookkey AND printingkey = @v_printingkey
...
GRANT EXECUTE ON dbo.[imp_300012048001] TO PUBLIC
GO
