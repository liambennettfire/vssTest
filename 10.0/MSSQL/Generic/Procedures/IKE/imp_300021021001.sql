...
IF EXISTS (
		SELECT *
		FROM dbo.sysobjects
		WHERE id = object_id(N'[dbo].[imp_300021021001]')
			AND OBJECTPROPERTY(id, N'IsProcedure') = 1
		)
	DROP PROCEDURE [dbo].[imp_300021021001]
GO

CREATE PROCEDURE dbo.imp_300021021001 @i_batch INT
	,@i_row INT
	,@i_dmlkey BIGINT
...
AS
/* Add/Replace Custom Float 01(Weight) */
BEGIN
	SET NOCOUNT ON
	...
	BEGIN
		...
		SET @v_errcode = 1
		SET @v_errmsg = 'Custom Float 01 Updated'
		...
		BEGIN
			...
			SELECT @v_curr_customfloat = COALESCE(customfloat01, -1)--mk20130606>Case 23834 IKE import fails to import non-empty 
			FROM bookcustom
			WHERE bookkey = @v_bookkey
			...
			BEGIN
				UPDATE bookcustom
				SET customfloat01 = @v_new_customfloat
					,lastuserid = @i_userid
		...
		IF @v_rowcount > 1
		BEGIN
			SET @v_errmsg = 'Did not update Custom Float 01'
		END
