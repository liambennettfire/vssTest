/******************************************************************************
**  Name: imp_300012032001
**  Desc: IKE Estimated Trim Width
**  Auth: Bennett     
**  Date: 5/9/2016
*******************************************************************************
**  Change History
...
*******************************************************************************/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[imp_300012032001]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[imp_300012032001]
GO
...
CREATE PROCEDURE [dbo].[imp_300012032001] @i_batch INT,@i_row INT,@i_dmlkey BIGINT,@i_titlekeyset VARCHAR(500),@i_contactkeyset VARCHAR(500),
...
BEGIN
	...
	BEGIN
		...
		SET @v_errcode = 1
		SET @v_errmsg = 'Estimated Trim Width Unchanged'
		...
		SELECT @i_width = COALESCE(esttrimsizewidth, '') FROM printing WHERE bookkey = @v_bookkey AND printingkey = @v_printingkey
		...
		*/
		...
		IF @i_width <> @v_elementval BEGIN
			UPDATE printing
			  SET esttrimsizewidth = @v_elementval,trimsizeunitofmeasure = @v_unitsofmeasure,lastuserid = @i_userid	,lastmaintdate = GETDATE()
			WHERE bookkey = @v_bookkey AND printingkey = @v_printingkey

			SET @v_errmsg = 'Estimated Trim Width Updated'
...
GRANT EXECUTE ON dbo.[imp_300012032001] TO PUBLIC
GO
