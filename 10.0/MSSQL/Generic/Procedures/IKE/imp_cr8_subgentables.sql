...
**  Desc: IKE insert into subgentables
**  Auth: Bennett     
**  Date: 5/9/2016
*******************************************************************************
**  Change History
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[imp_cr8_subgentables]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[imp_cr8_subgentables]
GO

CREATE PROCEDURE dbo.imp_cr8_subgentables
    @i_batchkey int,
    @i_rootkey int,
    @i_elementkey int,
    @i_option int,
...
BEGIN
  ...
  if @v_tableid is null
    ...
    begin
      ...
      set @v_add_sql=@v_add_sql+'declare @v_datasubcode int'+@v_newline
    end
    ...
    begin
      ...
      select @v_row_id=
        (select top 1 row_id
           from imp_batch_detail
           where batchkey=@i_batchkey
             and originalvalue=@v_element_value)

      select @v_datadesc_gt=originalvalue
        from imp_batch_detail
        where batchkey=@i_batchkey
          and elementkey=@i_rootkey
          and row_id=@v_row_id
      select @v_datacode=datacode    
        from gentables
        where tableid=@v_tableid
          and datadesc=@v_datadesc_gt

      select @v_count=count(*) 
        from subgentables
        where tableid=@v_tableid
          and datacode=@v_datacode
          and datadesc=@v_element_value
        ...
        begin
          ...
          if @i_option=2
            begin
              ...
              set @v_add_sql=@v_add_sql+ 'select @v_datacode=max(datacode) from gentables where tableid='+cast(@v_tableid as varchar)+' and datadesc='+CHAR(39)+@v_datadesc_gt+CHAR(39)+@v_newline
              set @v_add_sql=@v_add_sql+ 'set @v_datacode=coalesce(@v_datacode,1)'+@v_newline
              set @v_add_sql=@v_add_sql+ 'select @v_datasubcode=max(datasubcode)+1 from subgentables where tableid='+cast(@v_tableid as varchar)+' and datacode=@v_datacode'+@v_newline
              set @v_add_sql=@v_add_sql+ 'set @v_datasubcode=coalesce(@v_datasubcode,1)'+@v_newline

              set @v_add_sql=@v_add_sql+ 'insert into subgentables'+@v_newline
              set @v_add_sql=@v_add_sql+ ' (tableid,datacode,datasubcode,datadesc,datadescshort,tablemnemonic,lastuserid,lastmaintdate)'+@v_newline
              ...
              set @v_add_sql=@v_add_sql+ '@v_datasubcode,'
...
GRANT EXECUTE ON dbo.imp_cr8_subgentables TO PUBLIC 
GO

/*  example 
exec imp_cr8_subgentables 4,100012051,100012050,2,'fire'
*/