...
SELECT @v_customerkey = customerkey FROM customer WHERE eloqcustomerid =  @i_eloquencecustomerid

SELECT @rowKey = fd.elocustomerkey, @o_file_extension = fd.exportfileextension from filedata fd where fd.elocustomerkey =  @v_customerkey;
...
BEGIN
  ...
  insert into filedata (elocustomerkey, exportfileextension) VALUES
	  (@v_customerkey, @o_file_extension);
END
...
BEGIN
  ...
  UPDATE filedata SET exportfileextension = @initial_extension where elocustomerkey =  @rowKey;
...
BEGIN
  ...
  BEGIN
    SET @o_file_extension = @initial_extension; 
    SET @notDone = 0;
    update filedata set exportfileextension = @o_file_extension where elocustomerkey = @v_customerkey;
  END
  ELSE
  BEGIN
