...
**          0x01 - OPTION_DONTCOPYQUANTITIES
**          0x02 - OPTION_DERIVE_FROM_FG
*****************************************************************************************************
**  Change History
*****************************************************************************************************
...
**  06/16/17     Colman      Case 45603 - When loading a template with a FG to a shared section, uncheck the derived from FG
...
DECLARE
  ...
  @v_flag_forcederivefromfg INT,
  @v_copyquantitiesind INT,
  @v_forcederivefromfgind INT,
  @v_sharedposectionind INT,
  @v_template_has_fg INT,
...
BEGIN
  ...
  SET @v_forcederivefromfgind = 0
  SET @v_template_has_fg = 0

  -- Option flag values
  SET @v_flag_dontcopyquantities = 0x01
  SET @v_flag_forcederivefromfg = 0x02
  
  IF (@i_optionflags & @v_flag_dontcopyquantities) > 0
    SET @v_copyquantitiesind = 0

    IF (@i_optionflags & @v_flag_forcederivefromfg) > 0
    SET @v_forcederivefromfgind = 1
  ...
  SELECT @v_plstage_project = plstagecode, @v_versionkey_project = taqversionkey, @v_sharedposectionind = ISNULL(sharedposectionind, 0)
  ...
  END    
  ELSE IF @i_itemtype = 15 AND @v_qsicode_project <> 42 AND @v_qsicode_project <> 43 BEGIN -- Non-report PO
...
-- ***** BEGIN TRANSACTION ****  
  ...
  IF @v_isApplySpecficationByFormat = 1 BEGIN
    ...
    SELECT c.taqversionspecategorykey, c.itemcategorycode, c.speccategorydescription, c.vendorcontactkey, ISNULL(c.finishedgoodind, 0)
  ...
  INTO @v_categorykey_template, @v_itemcategorycode, @v_speccategorydescription, @v_vendorcontactkey, @v_finishedgoodind
  ...
  BEGIN
    ...
    INSERT INTO taqversionspeccategory
        ...
        SELECT            
           ...
           ,@i_userid
           ,getdate()
           ,CASE WHEN @v_forcederivefromfgind = 1 THEN 0 ELSE finishedgoodind END
           ,sortorder
           ,CASE WHEN @v_copyquantitiesind = 1 THEN quantity ELSE NULL END
           ,CASE WHEN @v_forcederivefromfgind = 1 THEN 1 ELSE deriveqtyfromfgqty END
           ,CASE WHEN @v_forcederivefromfgind = 1 THEN @i_forcespoilagepercent ELSE spoilagepercentage END
    FROM taqversionspeccategory       
    ...
    IF @v_debug = 1
      PRINT 'INSERT INTO taqversioncosts: '+cast(@v_rowcount as varchar)+' rows'
        ...
        -- exec qutl_trace 'qspec_apply_specificationtemplate',
          ...
          -- '@v_taqprojectformatkey_template', @v_taqprojectformatkey_template, NULL,
          -- '@v_taqversionspecategorykey_new', @v_taqversionspecategorykey_new, NULL,
          -- '@v_printingnum', @v_printingnum, NULL,
  ...
  -- If shared sections have a fg component, they cannot have a deriveqtyfromfgqty component
  IF @v_sharedposectionind = 1 AND @v_template_has_fg = 1
  BEGIN
    UPDATE taqversionspeccategory 
      SET deriveqtyfromfgqty = 0, spoilagepercentage = NULL
    WHERE taqprojectkey = @i_projectkey
      AND plstagecode = @v_plstage_project
      AND taqversionkey = @v_versionkey_project
  END
