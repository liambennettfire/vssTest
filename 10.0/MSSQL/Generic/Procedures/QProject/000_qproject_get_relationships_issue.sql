if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qproject_get_relationships_issue') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qproject_get_relationships_issue
GO
...
CREATE PROCEDURE qproject_get_relationships_issue
...
/******************************************************************************
**  Name: qproject_get_relationships_issue
...
*   --------     --------    -------------------------------------------
**	06/10/2016   Colman      38278 - Return sortable string value for numeric misc columns
...
IF ( @v_pricetypecode4 > 0 AND @v_price4label IS NULL ) 
  ...
  SELECT 1 projectnumber,
         ...
         COALESCE(c.projectparticipants,r.project2participants) otherprojectparticipants,  
		 ...
		 r.quantity1, r.quantity2, r.quantity3, r.quantity4, 
     r.indicator1, r.indicator2, 
     @v_quantity1label as quantity1label, @v_quantity2label as quantity2label, @v_quantity3label as quantity3label, @v_quantity4label as quantity4label, 
   ...
   WHERE r.taqprojectkey1 = @i_projectkey 
     AND relationshipcode1 = 9 AND relationshipcode2 = 8 
...
UNION
  SELECT 2 projectnumber, 
   ...
   WHERE r.taqprojectkey1 = c.projectkey
     ...
     AND relationshipcode2 = 9 AND relationshipcode1 = 8 
...
GRANT EXEC ON qproject_get_relationships_issue TO PUBLIC
GO
