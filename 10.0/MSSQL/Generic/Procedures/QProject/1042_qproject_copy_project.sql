...
**  02/28/2017  Colman    Case 42869 - Update Contract Rights and Territory copy project data group to copy related works
**  03/27/2017  Uday      Case 44037
**  05/23/2017  Colman    Case 45158 - Changes for the Shared PO Sections and Components
**  06/06/2017  Colman    Case 45522 - New Copy Shared Section datagroup
**  06/14/2017  Colman    Case 45536 - Comp Quantity needs to be copied in some Copy Specs situations and not in others
**  06/16/2017  Alan      Case 45871 - Call Generate Name stored procedure if configured after project is created
**  03/28/2018  Colman    Case 50246 - Formats tab not locked after copying TAQ and clientoption 101 is set 
...
-- Shared Sections on Print Run PO
IF dbo.find_integer_in_comma_delim_list (@i_copydatagroups_list, 30) = 'Y'
BEGIN
  exec qproject_copy_project_sharedsections
    @i_copy_projectkey,
    @o_new_projectkey,
    @i_userid,
    @o_error_code output,
  ...
  IF @o_error_code <> 0 BEGIN
    ROLLBACK
    print 'shared sections: '+@o_error_desc
    RETURN
  END 
END
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM  clientoptions WHERE optionid = 101 AND optionvalue = 1)
  BEGIN
    EXEC qpl_sync_version_to_acq_project @o_new_projectkey, 1, @v_cur_version, @v_userkey,  1, @o_error_code output, @o_error_desc output
  END
END
...
BEGIN
  SET @o_error_desc = ''
  SET @v_clearind = 0
  IF dbo.find_integer_in_comma_delim_list(@i_cleardatagroups_list,24) = 'Y'
    SET @v_clearind = 1
    
	EXEC qproject_copy_project_contract_rights @i_copy_projectkey, @i_copy2_projectkey, @o_new_projectkey, @v_clearind, @i_userid, 
		@o_error_code OUTPUT, @o_error_desc OUTPUT  
  ...
  IF @o_error_code <> 0 BEGIN
	...
	EXEC qproject_copy_project_relatedworks @i_copy_projectkey, @i_copy2_projectkey, @o_new_projectkey, @i_userid, 
		@o_error_code OUTPUT, @o_error_desc OUTPUT  

  IF @o_error_code <> 0 BEGIN
		ROLLBACK
    PRINT 'Contract Related Works ' + @o_error_desc
		RETURN
	END 
END
  ...
  DECLARE taqversionformat_cursor CURSOR FOR
	...
	BEGIN  			
	  EXEC qspec_apply_specificationtemplate @o_new_projectkey, @i_copy_projectkey
		,@v_TaqProjectFormatKey, @itemtypecode, @usageclasscode, @i_userid, 4, @v_apply_spectemplate_options, 0, @o_error_code OUTPUT, @o_error_desc OUTPUT
...
-- if autogeneratename is on for this project, see if there is an autogenerate procedure configured for this class
-- NOTE: this should be the last step because many of the generate procedures depend on the project existing
SELECT @v_autogeneratenameind = coalesce(autogeneratenameind,0)
  FROM taqproject
 WHERE taqprojectkey = @o_new_projectkey

if @v_autogeneratenameind = 1 begin
  select @v_generatename_sp = coalesce(alternatedesc1,'') from subgentables
   where tableid = 550
     and datacode = @itemtypecode
     and datasubcode = @usageclasscode

  IF (@v_generatename_sp <> '' and charindex('@bookkey', @v_generatename_sp) <= 0) BEGIN
    --exec qproject_run_gen_varchar @o_new_projectkey, @i_related_journalkey, @i_related_volumekey, 0, 0, 
    --   @v_userkey, @v_generatename_sp, @o_error_code output, @o_error_desc output

    SET @v_generatename_sp = REPLACE(@v_generatename_sp, '@userkey', CONVERT(VARCHAR, @v_userkey))
    SET @v_generatename_sp = REPLACE(@v_generatename_sp, '@userid', @v_quote + @i_userid + @v_quote)
    SET @v_generatename_sp = REPLACE(@v_generatename_sp, '@projectkey', CONVERT(VARCHAR, @o_new_projectkey))
    SET @v_generatename_sp = REPLACE(@v_generatename_sp, '@journalkey', CONVERT(VARCHAR, @i_related_journalkey))
    SET @v_generatename_sp = REPLACE(@v_generatename_sp, '@volumekey', CONVERT(VARCHAR, @i_related_volumekey))
    --SET @v_generatename_sp = REPLACE(@v_generatename_sp, '@bookkey', CONVERT(VARCHAR, @i_bookkey))
    --SET @v_generatename_sp = REPLACE(@v_generatename_sp, '@printingkey', CONVERT(VARCHAR, @i_printingkey))
    --SET @v_generatename_sp = REPLACE(@v_generatename_sp, '@printingnum', CONVERT(VARCHAR, @v_printingnum))

    -- DEBUG
    --PRINT @v_generatename_sp

    EXEC qutl_execute_prodidsql2 @v_generatename_sp, @v_result_value1 OUTPUT, @v_result_value2 OUTPUT, @v_result_value3 OUTPUT,
      @o_error_code OUTPUT, @o_error_desc OUTPUT
    ...
    if @v_result_value1 is not null begin
      -- update taqprojecttitle with generated name
      UPDATE taqproject
         SET taqprojecttitle = @v_result_value1
       WHERE taqprojectkey =  @o_new_projectkey
    end 
  END
END
