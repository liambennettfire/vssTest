...
**    12/08/16  UK			    Case 42099
**    05/11/17  Colman      Case 44674 - Error message when trying to delete a contract related to a title/printing 
*******************************************************************************/
  ...
  ELSE IF EXISTS (SELECT 1 FROM gentables WHERE tableid = 550 AND qsicode = 10 AND datacode = @v_searchitemcode) -- Contract
  BEGIN
    IF EXISTS (SELECT 1 FROM taqprojectrights WHERE taqprojectkey = @i_projectkey AND ISNULL(productionbookkey,0) <> 0)
    BEGIN
      SET @o_error_code = -1
      SET @o_error_desc = 'This contract is related to a Title and can not be deleted.'
      RETURN
    END
  END
...
-- taqversionspeccategory and taqversionspecitems
  ...
  -- taqprojectdeliverydetails
  PRINT 'Deleting taqprojectdeliverydetails...'
  ...
  IF @error_var <> 0
    BEGIN
      ...
      SET @o_error_code = -1
      SET @errormsg_var = 
        'Unable to Delete Project: Error accessing taqprojectdeliverydetails table (' + cast
        (@error_var AS VARCHAR) + ').'
      GOTO ExitHandler
    END
  IF @count_var > 0
    BEGIN
      ...
      IF @error_var <> 0
        BEGIN
          ...
          SET @o_error_code = -1
          SET @errormsg_var = 
            'Unable to Delete Project: Error deleting taqprojectdeliverydetails table (' + 
            cast(@error_var AS VARCHAR) + ').'
          GOTO ExitHandler
        END
    END
