if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qproject_get_relationships_contentunit') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qproject_get_relationships_contentunit
GO
...
CREATE PROCEDURE qproject_get_relationships_contentunit
...
/******************************************************************************
**  Name: qproject_get_relationships_contentunit
...
**
**	Rev: 07/14/2008 by jhess for Project Relationship Tab Enhancement
**	Rev: 07/21/2008 by jhess for Project Relationship Tab Enhancement Phase II
**
**  05/11/2016 - AK - This procedure will return tab configurations for contentunits
**                    on journals with the possibility of filtering by volumekey 
**                    or issuekey (Case #37181)
**	06/07/2016   Colman   Case 38278 - Return sortable string value for numeric misc columns
**	01/10/2016   Uday     Case 42548
...
IF ( @v_pricetypecode4 > 0 AND @v_price4label IS NULL ) 
  ...
  SELECT @i_projectkey journalkey,
         COALESCE((select relatedprojectkey FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_issue_relationship_datacode),0) issuekey, 
         COALESCE((select relatedprojectkey FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_volume_relationship_datacode),0) volumekey, 
         cu.relatedprojectkey contentunitkey
    INTO #temp_relationships
    FROM projectrelationshipview cu, coreprojectinfo c
   WHERE cu.relatedprojectkey = c.projectkey 
     AND cu.taqprojectkey = @i_projectkey
     AND c.searchitemcode = @v_otheritemtype and c.usageclasscode = @v_otherusageclass  -- only want contentunits
  ...
  CREATE INDEX temp_relationships_idx on #temp_relationships (issuekey)
  CREATE INDEX temp_relationships2_idx on #temp_relationships (volumekey)

  IF @i_issuekey > 0 BEGIN
    SELECT cu.relatedprojectkey otherprojectkey, 
           (select relatedprojectkey FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_issue_relationship_datacode) issuekey, 
           (select relatedprojectkey FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_volume_relationship_datacode) volumekey, 
           c.templateind,
           ...
           (select relatedprojectname FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_issue_relationship_datacode) issuename, 
           cu.taqprojectrelationshipkey,
		 
		   -- we are retrieving content units for journals - don't display issue to content unit info
       CASE WHEN (select relatedprojectkey FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_issue_relationship_datacode) > 0 THEN null ELSE cu.quantity1 END AS quantity1,
       CASE WHEN (select relatedprojectkey FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_issue_relationship_datacode) > 0 THEN null ELSE cu.quantity2 END AS quantity2,
       CASE WHEN (select relatedprojectkey FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_issue_relationship_datacode) > 0 THEN null ELSE cu.quantity3 END AS quantity3,
       CASE WHEN (select relatedprojectkey FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_issue_relationship_datacode) > 0 THEN null ELSE cu.quantity4 END AS quantity4,
       CASE WHEN (select relatedprojectkey FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_issue_relationship_datacode) > 0 THEN 0 ELSE cu.indicator1 END AS indicator1,
       CASE WHEN (select relatedprojectkey FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_issue_relationship_datacode) > 0 THEN 0 ELSE cu.indicator2 END AS indicator2,
       ...
       @v_indicator1label as indicator1label, @v_indicator2label as indicator2label, 
		   ...
		   dbo.qproject_get_misc_value( cu.relatedprojectkey, @v_miscitemkey1 ) miscItem1value, 
		   dbo.qproject_get_misc_sortvalue( cu.relatedprojectkey, @v_miscitemkey1 ) miscItem1sortvalue, 
		   @v_miscitemkey2 as miscitemkey2, @v_miscitem2label as miscitem2label, 
		   dbo.qproject_get_misc_value( cu.relatedprojectkey, @v_miscitemkey2 ) miscItem2value, 
		   dbo.qproject_get_misc_sortvalue( cu.relatedprojectkey, @v_miscitemkey2 ) miscItem2sortvalue, 
		   @v_miscitemkey3 as miscitemkey3, @v_miscitem3label as miscitem3label, 
		   dbo.qproject_get_misc_value( cu.relatedprojectkey, @v_miscitemkey3 ) miscItem3value, 
		   dbo.qproject_get_misc_sortvalue( cu.relatedprojectkey, @v_miscitemkey3 ) miscItem3sortvalue, 
		   @v_miscitemkey4 as miscitemkey4, @v_miscitem4label as miscitem4label, 
		   dbo.qproject_get_misc_value( cu.relatedprojectkey, @v_miscitemkey4 ) miscItem4value, 
		   dbo.qproject_get_misc_sortvalue( cu.relatedprojectkey, @v_miscitemkey4 ) miscItem4sortvalue, 
		   @v_miscitemkey5 as miscitemkey5, @v_miscitem5label as miscitem5label, 
		   dbo.qproject_get_misc_value( cu.relatedprojectkey, @v_miscitemkey5 ) miscItem5value, 
		   dbo.qproject_get_misc_sortvalue( cu.relatedprojectkey, @v_miscitemkey5 ) miscItem5sortvalue, 
		   @v_miscitemkey6 as miscitemkey6, @v_miscitem6label as miscitem6label, 
		   dbo.qproject_get_misc_value( cu.relatedprojectkey, @v_miscitemkey6 ) miscItem6value, 
		   dbo.qproject_get_misc_sortvalue( cu.relatedprojectkey, @v_miscitemkey6 ) miscItem6sortvalue, 
		   ...
		   dbo.qproject_get_last_taskdate( cu.relatedprojectkey, @v_datetypecode1 ) as date1value, 
		   @v_datetypecode2 as datetypecode2, @v_date2label as date2label, 
           dbo.qproject_get_last_taskdate( cu.relatedprojectkey, @v_datetypecode2 ) as date2value,
		   @v_datetypecode3 as datetypecode3, @v_date3label as date3label, 
           dbo.qproject_get_last_taskdate( cu.relatedprojectkey, @v_datetypecode3 ) as date3value,
		   @v_datetypecode4 as datetypecode4, @v_date4label as date4label, 
           dbo.qproject_get_last_taskdate( cu.relatedprojectkey, @v_datetypecode4 ) as date4value,
		   @v_datetypecode5 as datetypecode5, @v_date5label as date5label, 
           dbo.qproject_get_last_taskdate( cu.relatedprojectkey, @v_datetypecode5 ) as date5value,
		   @v_datetypecode6 as datetypecode6, @v_date6label as date6label, 
           dbo.qproject_get_last_taskdate( cu.relatedprojectkey, @v_datetypecode6 ) as date6value,

		   @v_pricetypecode1 as pricetypecode1, @v_price1label as price1label, 
           dbo.qproject_get_price_by_pricetype( cu.relatedprojectkey, @v_pricetypecode1 ) as pricetypecode1Value,
		   @v_pricetypecode2 as pricetypecode2, @v_price2label as price2label, 
           dbo.qproject_get_price_by_pricetype( cu.relatedprojectkey, @v_pricetypecode2 ) as pricetypecode2Value,
		   @v_pricetypecode3 as pricetypecode3, @v_price3label as price3label, 
           dbo.qproject_get_price_by_pricetype( cu.relatedprojectkey, @v_pricetypecode3 ) as pricetypecode3Value,
		   @v_pricetypecode4 as pricetypecode4, @v_price4label as price4label, 
           dbo.qproject_get_price_by_pricetype( cu.relatedprojectkey, @v_pricetypecode4 ) as pricetypecode4Value,

		   @v_productidcode1 as productidcode1, @v_productid1label as productid1label, 
		   ( SELECT productnumber FROM taqproductnumbers WHERE ( taqprojectkey = cu.relatedprojectkey AND productidcode = @v_productidcode1 ) ) as productIdCode1Value,
		   @v_productidcode2 as productidcode2, @v_productid2label as productid2label, 
		   ( SELECT productnumber FROM taqproductnumbers WHERE ( taqprojectkey = cu.relatedprojectkey AND productidcode = @v_productidcode2 ) ) as productIdCode2Value,

		   @v_roletypecode1 as roletypecode1, @v_roletype1label as roletype1label, 
		   dbo.qproject_get_participant_name_by_role( cu.relatedprojectkey, @v_roletypecode1) as roletypecode1Value,
		   @v_roletypecode2 as roletypecode2, @v_roletype2label as roletype2label, 
		   dbo.qproject_get_participant_name_by_role( cu.relatedprojectkey, @v_roletypecode2) as roletypecode2Value,

       @v_lastmaintdate as lastMaintDate, @v_lastuserid as lastuserid
      FROM projectrelationshipview cu, coreprojectinfo c, #temp_relationships t
     WHERE cu.relatedprojectkey = c.projectkey 
       AND cu.taqprojectkey = t.journalkey 
       AND cu.relatedprojectkey = t.contentunitkey 
       AND t.issuekey = @i_issuekey
       AND cu.taqprojectkey = @i_projectkey
       AND c.searchitemcode = @v_otheritemtype and c.usageclasscode = @v_otherusageclass  -- only want contentunits

  END
  ELSE IF @i_volumekey = -1 BEGIN  -- All Content Units with no Issues and no Volumes
    SELECT cu.relatedprojectkey otherprojectkey, 
           (select relatedprojectkey FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_issue_relationship_datacode) issuekey, 
           (select relatedprojectkey FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_volume_relationship_datacode) volumekey, 
           c.templateind,
     ...
     WHERE cu.relatedprojectkey = c.projectkey 
       ...
       AND c.searchitemcode = @v_otheritemtype and c.usageclasscode = @v_otherusageclass  -- only want contentunits
  END
  ELSE IF @i_volumekey > 0 BEGIN
    IF @i_issuekey = 0 BEGIN  
      -- All Content Units for All Issues within a Volume
      SELECT cu.relatedprojectkey otherprojectkey, 
             (select relatedprojectkey FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_issue_relationship_datacode) issuekey, 
             (select relatedprojectkey FROM projectrelationshipview WHERE taqprojectkey = cu.relatedprojectkey AND relationshipcode = @v_volume_relationship_datacode) volumekey, 
             c.templateind,
...
GRANT EXEC ON qproject_get_relationships_contentunit TO PUBLIC
GO
