...
** 12/15/2017 - Colman - Case 46269 Support for tasks on current printing search list
** 04/18/2018 - Alan - Case 50976 - Prevent Eloquence tasks from displaying in any 
**                                  section outside of eloquence sections
** 06/07/2018   Colman  Case 50971 - Implemented availsecurityobjectkey.firstprintingind support
*******************************************************************************/
  ...
  DECLARE
    ...
    @v_sqljoin     VARCHAR(max),
    @v_sqlcommonjoin  VARCHAR(max),
  ...
  IF LTRIM(RTRIM(ISNULL(@i_projectkeylist, ''))) <> ''
  BEGIN
    SET @v_len = CHARINDEX(',', @i_projectkeylist, 0)
    IF @v_len > 0
      SET @v_key = CONVERT(INT, LEFT(@i_projectkeylist, @v_len - 1))
    ELSE
      SET @v_key = CONVERT(INT, @i_projectkeylist)
    
    SELECT @v_printingprojectind = 1 
    FROM taqproject
    WHERE taqprojectkey = @v_key
      AND searchitemcode = 14
  END

  SET @v_printingkey = 1

  IF @v_printingprojectind = 1
  BEGIN
    ...
    BEGIN
      ...
      SET @v_sqlselect1 = 'SELECT DISTINCT d.datetypecode,COALESCE(d.usedexclusivelybycsind,0) usedexclusivelybycsind,
         ...
         CASE 
          ...
          ELSE dbo.qutl_check_gentable_value_security(' + cast(@i_userkey as varchar) + ','+ @v_quote + 'tasktracking'+ @v_quote + ',323,t.datetypecode,' + cast(@v_printingkey as varchar) + ')
         END accesscode,
         CASE 
      ...
      SET @v_sqlwhere1 = ' WHERE t.datetypecode = d.datetypecode AND COALESCE(d.usedexclusivelybycsind,0) = 0 '      
    END
  ELSE  
    BEGIN
      ...
      IF @v_printingprojectind = 1 BEGIN
        SET @v_tasksourcetable = 'taqprojecttitle tpt '
        IF (@i_taqelementkey > 0) OR (@i_taqelementkey = 0 AND @i_elementtype > 0)
          SET @v_sqljoin = 'JOIN taqprojecttaskelement_view t ON tpt.bookkey = t.bookkey AND tpt.printingkey = t.printingkey '
        ELSE
          SET @v_sqljoin = 'JOIN taqprojecttask t ON tpt.bookkey = t.bookkey AND tpt.printingkey = t.printingkey '
      END
      ELSE BEGIN
      ...
      SET @v_sqljoin = @v_sqljoin + 'LEFT OUTER JOIN taqprojectelement e ON t.taqelementkey = e.taqelementkey ' + @v_sqlcommonjoin
        
	    SET @v_sqlfrom1 = ' FROM datetype d, ' + @v_tasksourcetable + @v_sqljoin
      ...
      END              
	    ...
	    SET @v_sqlfrom2 = ' FROM datetype d, ' + @v_tasksourcetable + @v_sqljoin + @v_sqlcommonjoin

      SET @v_sqlwhere2 = ' WHERE t.datetypecode = d.datetypecode AND t.taqelementkey IS NULL AND tok.taqtaskkey IS NULL AND COALESCE(d.usedexclusivelybycsind,0) = 0 '
  ...
  IF (@i_projectkeylist is not null AND ltrim(rtrim(@i_projectkeylist)) <> '') BEGIN
    IF @v_printingprojectind = 1
      SET @v_sqlwhere_keys = @v_sqlwhere_keys + ' (tpt.taqprojectkey in (' + @i_projectkeylist + '))'
    ELSE
