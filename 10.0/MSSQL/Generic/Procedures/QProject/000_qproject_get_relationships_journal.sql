if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qproject_get_relationships_journal') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qproject_get_relationships_journal
GO
...
CREATE PROCEDURE qproject_get_relationships_journal
...
/******************************************************************************
**  Name: qproject_get_relationships_journal
...
IF ( @v_pricetypecode4 > 0 AND @v_price4label IS NULL ) 
  ...
  SELECT 1 projectnumber,
   ...
   WHERE r.taqprojectkey1 = @i_projectkey 
     AND relationshipcode1 in (SELECT datacode from gentables
                                WHERE tableid = 582
...
UNION
  SELECT 2 projectnumber, 
   ...
   WHERE r.taqprojectkey1 = c.projectkey
     ...
     AND relationshipcode2 in (SELECT datacode from gentables
                                WHERE tableid = 582
                                  AND datacode in (SELECT distinct code1 FROM gentablesrelationshipdetail
                                                    WHERE gentablesrelationshipkey = @v_gentablesrelationshipkey
                                                      AND code2 in (SELECT datacode FROM gentables
...
GRANT EXEC ON qproject_get_relationships_journal TO PUBLIC
GO
