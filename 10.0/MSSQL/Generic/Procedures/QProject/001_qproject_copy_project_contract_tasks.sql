...
**   02/15/2017   Uday         42532     Add item type filtering for Contract Tasks
**   08/29/17     Colman       46909     Item type filtering inefficient and slow
**   04/18/2018   Colman       50917     Clear data is not working for some project copy data groups 
******************************************************************************************************************************/
...
BEGIN
  ...
  SET @v_cleardata = dbo.find_integer_in_comma_delim_list(@i_cleardatagroups_list,21)  --Contract Tasks (21)
  ...
  IF @v_isPrinting = 0 BEGIN
    ...
    (
      datetypecode IN (SELECT datetypecode FROM taskviewdatetype 
             WHERE taskviewkey = (SELECT taskviewkey FROM taskview WHERE qsicode = 5)) 
      OR  
      datetypecode IN (SELECT DISTINCT datetypecode FROM #datetype_itemtype_filtering)
    )
  END
  ELSE IF @v_isPrinting = 1 BEGIN
    SELECT @v_newkeycount = COUNT(*), @v_tobecopiedkey = MIN(taqtaskkey), @v_maxsort = MAX(sortorder)
