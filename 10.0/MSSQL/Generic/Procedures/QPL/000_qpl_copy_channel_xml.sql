if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_copy_channel_xml') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_copy_channel_xml
GO
...
CREATE PROCEDURE qpl_copy_channel_xml
...
DECLARE 
  ...
  SELECT @v_ProjectKey = ProjectKey,
      ...
      @v_FromChannel = FromChannel,
      @v_FromSubChannel = FromSubChannel,
  ...
  WITH (ProjectKey INT 'ProjectKey', 
      ...
      FromChannel INT 'FromChannel',
      FromSubChannel INT 'FromSubChannel',
      CopyValues TINYINT 'CopyValues',
      UserID varchar(30) 'UserID')

  IF @@ERROR <> 0 BEGIN
    SET @o_error_code = @@ERROR
    SET @o_error_desc = 'Error extracting parameters from qpl_copy_channel_xml.'
    GOTO ExitHandler
  END
  ...
  PRINT 'FROM channelcode=' + CAST(@v_FromChannel AS VARCHAR)
  PRINT 'FROM subchannelcode=' + CAST(@v_FromSubChannel AS VARCHAR)
  ...
  EXEC qpl_copy_channel @v_ProjectKey, @v_PLStageCode, @v_VersionKey, @v_FormatKey, 
    @v_FromFormatKey, @v_FromChannel, @v_FromSubChannel, @v_CopyValues, @v_UserID, @o_error_code OUTPUT, @o_error_desc OUTPUT
...
GRANT EXEC ON qpl_copy_channel_xml TO PUBLIC
GO
