...
BEGIN
  ...
  SELECT a.*, a.yearcode origyearcode, a.datetypecode origdatetypecode, a.dateoffsetcode origdateoffsetcode, 
    CASE WHEN a.roletypecode = 0 THEN 'ALL' ELSE (SELECT datadesc FROM gentables WHERE tableid=285 and datacode=a.roletypecode) END AS roletypedesc,
    CASE WHEN a.globalcontactkey = 0 THEN 'ALL' ELSE (SELECT displayname FROM globalcontact WHERE globalcontactkey=a.globalcontactkey) END AS displayname,
  ...
  WHERE a.datetypecode = d.datetypecode AND
      ...
      a.taqversionkey = @i_versionkey AND
      a.roletypecode = @i_roletypecode AND
      a.globalcontactkey = @i_globalcontactkey
