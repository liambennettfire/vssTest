if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_copy_version_royalties_xml') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_copy_version_royalties_xml
GO
...
CREATE PROCEDURE qpl_copy_version_royalties_xml
...
/******************************************************************************************
**  Name: qpl_copy_version_royalties_xml
...
DECLARE 
  ...
  IF @@ERROR <> 0 BEGIN
    SET @o_error_code = @@ERROR
    SET @o_error_desc = 'Error extracting parameters from qpl_copy_version_royalties_xml.'
    GOTO ExitHandler
  END

  /** Call actual procedure **/
  EXEC qpl_copy_version_royalties @v_ProjectKey, @v_PLStage, @v_PLVersion, 
  @v_FromRoleCode, @v_FromContactKey, @v_RoleCode, @v_ContactKey, 
...
GRANT EXEC ON qpl_copy_version_royalties_xml TO PUBLIC
GO
