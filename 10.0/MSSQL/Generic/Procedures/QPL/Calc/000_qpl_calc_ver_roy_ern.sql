IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'qpl_calc_ver_roy_ern')
  DROP PROCEDURE qpl_calc_ver_roy_ern
GO

CREATE PROCEDURE [dbo].[qpl_calc_ver_roy_ern] (  
  @i_projectkey INT,
  @i_plstage    INT,
  @i_plversion	INT,
  @i_debugind	TINYINT,
  ...
  DECLARE saleschannel_cur CURSOR FOR  
    ...
    WHERE taqprojectkey = @i_projectkey AND
	  ...
	  AND EXISTS(SELECT 1 FROM @roleCodes rc
					WHERE rc.dataCode = taqversionroyaltysaleschannel.roleTypeCode)
