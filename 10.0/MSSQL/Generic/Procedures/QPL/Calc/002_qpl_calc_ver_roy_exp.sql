IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'qpl_calc_ver_roy_exp')
  DROP PROCEDURE qpl_calc_ver_roy_exp
GO

CREATE PROCEDURE [dbo].[qpl_calc_ver_roy_exp] (  
  @i_projectkey INT,
  @i_plstage    INT,
  @i_plversion	INT,
  @i_debugind	TINYINT,
...
BEGIN
  ...
  BEGIN
	...
	BEGIN
		PRINT ''
		PRINT ''
		PRINT 'Project Key = ' + CONVERT(VARCHAR, @i_projectkey)
		PRINT 'Version = ' + CONVERT(VARCHAR, @i_plversion)
		PRINT 'Year = ' + CONVERT(VARCHAR, @v_yearcode)
		PRINT 'Role summary item code = ' + CONVERT(VARCHAR, @i_roleSumItemCode)
		PRINT 'All included flag = ' + ISNULL(CONVERT(VARCHAR, @i_allIncludedInd),'Null')
	END

    EXEC qpl_calc_yr_roy_exp @i_projectkey, @i_plstage, @i_plversion, @v_yearcode, 0, @v_year_royalty_expense OUTPUT, @i_roleSumItemCode, @i_allIncludedInd
