...
**  03/20/18  Case# 50398 Bad join on line 86, needed to add an additional join on c2.plcalccostcode=c.plcalccostcode, to prevent multiple values returning(Error:Subquery returned more than 1 value. This is not permitted when the subquery follows =, !=, <
, <= , >, >= or when the subquery is used as an expression.)  
** 04/16/2018 Jason 50918 PO blowing up when navigating to Screen.  needed to add an additional Check when returning taqversioncostnote(and c2.versioncostsnote <> '')
**to prevent multiple values returning(Error:Subquery returned more than 1 value. This is not permitted when the subquery follows =, !=, <
, <= , >, >= or when the subquery is used as an expression.)  
**  05/08/2018  Colman  50918 Missed one.
**************************************************************************************/    
  ...
  IF @i_category_qsicode = 2  --distinct Production costs    
    SELECT DISTINCT @i_projectkey taqprojectkey, @i_formatkey taqversionformatkey, @i_plstage plstagecode, @i_versionkey taqversionkey,     
      ...
      COALESCE(c.versioncostsnote, (SELECT TOP (1) c2.versioncostsnote FROM taqversioncosts c2, taqversionformatyear y2     
                                    ...
                                    AND (c2.versioncostsnote IS NOT NULL and c2.versioncostsnote <> ''))) versioncostsnote,    
...
GRANT EXEC ON qpl_get_distinct_taqversioncosts TO PUBLIC
GO