...
**  06/07/17    Colman    45522 - Don't add new related printing to empty PO section if it is a shared section.
...
BEGIN
  ...
  BEGIN
    ...
    IF @v_num_formats = 0 AND NOT EXISTS (SELECT 1 FROM taqversionformat WHERE taqprojectformatkey = @v_cur_versionformatkey AND sharedposectionind = 1) 
    BEGIN
