...
*   --------	-------   ------------------------------------------------------------------------
*   03/31/16  Kate      Case 35972 - Rewritten recalc procedure to process specific P&L Summary Level only.
******************************************************************************************************************/
DECLARE 
  @v_IsOpen   BIT,
  @v_DocNum   INT,
  ...
  SELECT @v_ProjectKey = ProjectKey,
      ...
      @v_SummaryLevel = PLSummaryLevel,
      @v_JointAcctgInd = JointAcctgInd,
      @v_UserID = UserID
  FROM OPENXML(@v_DocNum,  '//Parameters')
  WITH (ProjectKey VARCHAR(120) 'ProjectKey', 
      PLStage INT 'PLStage',
      PLVersion INT 'PLVersion',
      PLSummaryLevel INT 'PLSummaryLevel',
      JointAcctgInd TINYINT 'JointAcctgInd',
  ...
  EXEC qpl_recalc_pl_items @v_ProjectKey, @v_PLStageCode, @v_VersionKey, @v_SummaryLevel, @v_JointAcctgInd,
