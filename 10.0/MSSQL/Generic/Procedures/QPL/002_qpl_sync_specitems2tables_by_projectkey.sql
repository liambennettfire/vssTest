...
@v_taqversionformatyearkey INT
	...
	and q.syncfromspecsind=1 and q.activeind=1 
  ...
  SELECT @v_taqversionformatyearkey = COALESCE(taqversionformatyearkey, 0) 
    FROM taqversionformatyear
   WHERE taqprojectkey = @i_taqprojectkey  

	SELECT @i_syncspecs2 =  COUNT(distinct v.taqversionspecitemkey)
	  FROM qsiconfigspecsync q, dbo.qproject_get_specitems_by_printingview(@v_taqversionformatyearkey) v
