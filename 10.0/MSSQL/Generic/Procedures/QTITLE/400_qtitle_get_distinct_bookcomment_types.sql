...
*******************************************************************************/
  ...
  SELECT DISTINCT o.orgentrykey, g.datadesc gentables_datadesc, s.datadesc subgentables_datadesc, s.datacode, s.datasubcode, s.sortorder,
	...
	COALESCE((SELECT releasetoeloquenceind FROM bookcomments WHERE bookkey = @i_bookkey AND printingkey = @i_printingkey 
      AND commenttypecode = s.datacode AND commenttypesubcode = s.datasubcode),0) releasetoeloquenceind,
    CASE 
      WHEN (COALESCE(s.exporteloquenceind,0) = 1 AND COALESCE(s.acceptedbyeloquenceind,0) = 1) THEN 1 
      ELSE 0
    END elocommentind,
    dbo.qutl_check_subgentable_value_security_by_status(@i_userkey,'productsummary',s.tableid,s.datacode,s.datasubcode,@i_bookkey,@i_printingkey,0) accesscode,
