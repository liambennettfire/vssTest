...
BEGIN
  ...
  SELECT r.taqprojectformatkey,
    ...
    r.bookkey, 
    r.printingkey,
		COALESCE(p.printingnum, 0) printingnum,
    r.projectrolecode, 
    ...
    @v_tableid3 as tableid3, r.datacode3, dbo.get_gentables_desc(@v_tableid3,r.datacode3,'long') as datacode3Value,
    @v_tableid4 as tableid4, r.datacode4, dbo.get_gentables_desc(@v_tableid4,r.datacode4,'long') as datacode4Value,
  ...
  FROM taqprojecttitle r
	LEFT OUTER JOIN coreprojectinfo c
	ON r.taqprojectkey = c.projectkey
	LEFT JOIN printing p
	ON p.bookkey = r.bookkey AND p.printingkey = r.printingkey
