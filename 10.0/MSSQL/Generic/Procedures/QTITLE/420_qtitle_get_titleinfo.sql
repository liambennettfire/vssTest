...
*******************************************************************************/
  ...
  IF @v_use_web_file_locations = 1 BEGIN
    ...
    END 
  END
  ELSE IF @v_use_web_file_locations = 2 BEGIN
	SELECT TOP 1 @v_cover_image_path = apiurl
	FROM cloudaccess

	--SET @v_cover_image_path = 'http://cloud.firebrandtech.com/api/v2' --DEBUG!

	IF LEN(COALESCE(@v_cover_image_path, '')) > 0
	BEGIN
		IF RIGHT(@v_cover_image_path, 1) <> '/'
		BEGIN
			SET @v_cover_image_path = @v_cover_image_path + '/'
		END

		SET @v_ean13 = ''
		SELECT @v_ean13 = ean13 FROM isbn where bookkey = @i_bookkey

		SET @v_elocustomerkey = 0
		SELECT @v_elocustomerkey = elocustomerkey
		FROM book
		WHERE bookkey = @i_bookkey

		SET @v_elocustomerid = NULL
		SELECT @v_elocustomerid = eloqcustomerid
		FROM customer
		WHERE customerkey = @v_elocustomerkey

		IF LEN(COALESCE(@v_ean13, '')) > 0 AND LEN(COALESCE(@v_elocustomerid, '')) > 0
		BEGIN
			WHILE LEFT(@v_elocustomerid, 1) = '0'
			BEGIN
				SET @v_elocustomerid = SUBSTRING(@v_elocustomerid, 2, 6)
			END

			SET @v_cover_image_path = @v_cover_image_path + 'img/' + CAST(@v_elocustomerid AS VARCHAR) + '/' + @v_ean13 + '/l'
		END
		ELSE BEGIN
			SET @v_cover_image_path = NULL
		END
	END
  END
...
--  which was causing issue in the code below which was modified accordingly but Powerbuilder should change for consistency sake.
 ...
 ---       COALESCE(printing.tmmpagecount,printing.pagecount,printing.tentativepagecount) bestpagecount,
       ...
       CASE 
        ...
        @v_cloudproductid cloudproductid, @v_work_projectkey workprojectkey, @v_work_project_title workprojecttitle, customer.eloqcustomerid, dbo.qcs_get_istitleinoutbox(@i_bookkey) istitleinoutbox, dbo.qcs_get_istitleinresendoutbox(@i_bookkey) istitleinresendoutbox, customer.cloudaccesskey, bookedistatus.previousedistatuscode,
