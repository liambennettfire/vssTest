...
**************************************************************************************/
  ...
  -- we have to assume we want the one associated with the ProductSummary window
  IF LOWER(@i_configobjectid) = 'shtitleinfo' BEGIN
    ...
    WHERE LOWER(configobjectid) = LOWER(@i_configobjectid) AND LOWER(w.windowname) = 'productsummary' AND o.windowid = w.windowid
  END ELSE BEGIN
