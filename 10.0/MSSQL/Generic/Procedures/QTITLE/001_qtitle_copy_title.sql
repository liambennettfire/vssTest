...
**  02/28/17  Colman  Case 43212 - Add right type field to Title Classification section 
**  05/16/17  Uday    Case 44699
...
**  05/15/18  Colman  Case 51413 - Default clear data for copy title data groups not working
********************************************************************************************************************/
...
DECLARE
  ...
  @v_printing_projectrolecode INT,
  @v_comment_cnt INT,
  @v_csverifytype INT,
...
BEGIN
  ...
  --- Get default Copy Title Data Groups
  IF @i_copydatagroups_list IS NULL
  BEGIN
  ...
  --- Get default Copy Title Data Groups to be cleared on copy
  IF @i_cleardatagroups_list IS NULL
  BEGIN
	  SELECT @v_newkeycount = count(*)
	  FROM gentables_ext ext
      JOIN gentables g ON ext.tableid = g.tableid
		WHERE ext.tableid = 592 AND ISNULL(ext.gen3ind, 0) > 0 AND ISNULL(g.qsicode, 0) > 0
	  ...
	  SET @i_cleardatagroups_list = ''
	   ...
	   DECLARE crCopyDataGroups CURSOR FOR
		  SELECT g.qsicode
		  FROM gentables_ext ext
        JOIN gentables g ON ext.tableid = g.tableid AND ISNULL(g.gen2ind, 0) > 0
		  WHERE ext.tableid = 592 AND ISNULL(ext.gen3ind, 0) > 0 AND ISNULL(g.qsicode, 0) > 0
	    ...
	    BEGIN
		  IF @v_count < @v_newkeycount
			  SET @i_cleardatagroups_list = @i_cleardatagroups_list + rtrim(convert(varchar,@v_qsicode)) + ','
		  ELSE 
			  SET @i_cleardatagroups_list = @i_cleardatagroups_list + rtrim(convert(varchar,@v_qsicode))
	    ...
	    DEALLOCATE crCopyDataGroups  
  END
  ELSE IF @i_cleardatagroups_list = 'none'
  BEGIN
    SET @i_cleardatagroups_list = ''
  END
  ...
  SELECT @v_mediatype = mediatypecode, @v_mediasubtype = mediatypesubcode, @v_origin = origincode, 
    ...
    @v_rightstypecode = rightstypecode
  FROM bookdetail
  WHERE bookkey = @i_templatebookkey
  ...
  BEGIN
    SELECT @v_test_mediatype = mediatypecode, @v_test_mediasubtype = mediatypesubcode, @v_test_origin = origincode, 
      ...
      @v_test_rightstypecode = rightstypecode
    FROM bookdetail
    WHERE bookkey = @i_bookkey
  ...
  BEGIN
    ...
    IF @v_rightstypecode > 0 AND ISNULL(@v_test_rightstypecode, 0) = 0
    BEGIN
      SELECT @v_datadesc = LTRIM(RTRIM(REPLACE(REPLACE(datadesc, '&', '&amp;'), '''', '''''')))
      FROM gentables
      WHERE tableid = 157 AND datacode = @v_rightstypecode

      SET @v_bookdetail_xml = @v_bookdetail_xml + '
       <DBItem><DBItemColumn>rightstypecode</DBItemColumn><DBItemValue>' + CONVERT(VARCHAR, @v_rightstypecode) + '</DBItemValue><DBItemDesc>' + @v_quote + @v_datadesc + @v_quote + '</DBItemDesc></DBItem>'
    END
 ...
 SELECT @v_generate_jobnumberalpha = COALESCE(gen2ind,0) FROM gentables WHERE tableid = 594 and qsicode = 14  
  ...
  BEGIN
    ...
    SELECT @v_printing_projectrolecode = datacode
      FROM gentables 
     WHERE tableid = 604 
       and qsicode = 3
             
    if @v_printing_projectrolecode is null begin
      SET @v_printing_projectrolecode = 0
    end
    ...
    DECLARE crTaqProjectTitle CURSOR FOR
		...
		AND COALESCE(tpt.projectrolecode,0) NOT IN (@v_work_projectrolecode, @v_printing_projectrolecode) -- do not copy work and printing relationships
