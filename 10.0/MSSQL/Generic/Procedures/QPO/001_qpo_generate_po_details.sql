...
**	12/01/16	Dustin	  Case 41464
**  01/11/17    Uday	  Case 42531
**  01/11/17    Dustin    Case 41464
**  01/17/17	Dustin	  Case 42686
**  02/13/17    Colman    Case 42803
**  03/27/17    Colman    Case 44050
**  04/12/17    Uday      Case 44396
**  05/10/17    Uday      Case 44989
**  06/22/17    Dustin    Case 40833
**  08/14/17    Colman    Case 46794 - Item filtering not working on Final PO Report
**  09/08/17    Colman    Case 47000 - Add space after showdecimallabel
**	04/19/18	Jen		  add component vendor to Cover, Jacket, & Endpaper Due date line
**  05/01/18	Jen		  populate gposection xpolineponbr & print on PO
*******************************************************************************/
...
DECLARE
  ...
  @v_usageclass INT,
  @v_usageclass_PO  INT,
  @v_usageclass_report  INT,
  @v_userkey  INT,
  @v_uomdesc  VARCHAR(40),
  @v_gpo_finishedgoodind TINYINT,
  @v_sharedrelatedprojkey INT,
  @v_sharedquantity INT,
  @v_shareddetail VARCHAR(2000),
  @v_format_projkey	 INT,
  @v_format_itemtype INT,
  @v_relprintprojkey INT,
  @v_printing_projectkey	int,
  @v_vendorname	varchar(255),
  @sequencenumber	int,
  @xpolineponbr		varchar(10),
  @ponumber		varchar(10),
  @v_detail2	varchar(50)
 ...
 BEGIN
  ...
  -- Get the Usage Class of the report project
  SELECT @v_usageclass_report = usageclasscode
  FROM taqproject
  WHERE taqprojectkey = @i_report_projectkey
  ...
  AND datacode = @v_commenttype_project --Project comment
    ...
    AND itemtypesubcode IN (0, @v_usageclass_PO)
  ...
  END
	...
	SELECT DISTINCT t.taqversionformatkey, t.relatedprojectkey, p.taqprojecttitle, p.searchitemcode, p.usageclasscode,
	f.sharedposectionind, f.[description], f.sortorder
	FROM taqversionrelatedcomponents_view t, taqproject p, taqversionformat f
	WHERE t.relatedprojectkey = p.taqprojectkey 
	AND t.taqprojectkey = @i_po_projectkey
	AND t.taqversionformatkey = f.taqprojectformatkey
	ORDER BY f.sortorder, t.relatedprojectkey

  OPEN cur_relatedprojects

  FETCH cur_relatedprojects
  INTO @v_po_formatkey, @v_projectkey, @v_projectdesc, @v_itemtype, @v_usageclass, @v_sharedposectionind, @v_formatdesc, @v_sortorder_projects
    
  WHILE @@fetch_status = 0
  BEGIN
	
	IF NOT EXISTS (SELECT formatkey FROM @formatsHandled WHERE formatkey = @v_po_formatkey) --don't process the same format more than once
    BEGIN
		INSERT INTO @formatsHandled
		(formatkey)
		VALUES
		(@v_po_formatkey)
		...
		SET @v_format_itemtype = NULL
		IF @v_sharedposectionind <> 1
		BEGIN
			SET @v_format_projkey = 0
			SELECT @v_format_projkey = relatedprojectkey
			FROM taqversionformatrelatedproject
			WHERE taqversionformatkey = @v_po_formatkey

			SELECT @v_format_itemtype = searchitemcode
			FROM taqproject
			WHERE taqprojectkey = @v_format_projkey
		END
    
		IF COALESCE(@v_format_itemtype, @v_itemtype) = @v_itemtype_printing
		BEGIN
		  ...
		  END
		  ELSE BEGIN
			SET @v_detail = @v_formatdesc
		  END
		  SET @v_sectiondesc = @v_detail

  		  SET @v_printing_projectkey = @v_projectkey			--JL 4/19/18

		  -- Keep track of the related Printing project - need for generate_gpocost
		  INSERT INTO @relPrintProj
		  VALUES
		  (@v_projectkey)
		END
		ELSE
		BEGIN
		  ...
		  IF @v_sharedposectionind <> 1
		  BEGIN
			SET @v_detail = @v_projectdesc
			IF @v_format_itemtype <> @v_itemtype_PO
			BEGIN
			  SET @v_detail = @v_detail + ', ' + @v_productnumberstring
			END
		  END
		  ELSE BEGIN
			SET @v_detail = @v_formatdesc
		  ...
		  -- Keep track of the related Printing projects - need for generate_gpocost
		  INSERT INTO @relPrintProj
		  SELECT relatedprojectkey FROM taqversionformatrelatedproject WHERE taqprojectkey = @v_projectkey
		END

    SELECT @v_quantity = dbo.qpo_get_po_section_quantity(@v_po_formatkey)
		...
		BEGIN
		  ...
		  --JL 5/1/18 add xpolinenbr population

		  set @sequencenumber = @sequencenumber + 1
		  select @ponumber = gponumber
		  from gpo
		  where gpokey = @i_gpokey

		  set @xpolineponbr = ltrim(rtrim(@ponumber)) + Char(64 + @sequencenumber)		--this will convert sectionnumber to characters


		  INSERT INTO gposection
			(gpokey, sectionkey, sectiontype, key1, key2, key3, quantity, description, sequencenum, xpolineponbr, taqversionformatkey, bookkey, printingkey, lastuserid, lastmaintdate)
		  VALUES
			(@i_gpokey, @v_new_sectionkey, @v_sectiontype, @v_key1, @v_key2, @v_key3, @v_quantity, @v_sectiondesc, @sequencenumber, @xpolineponbr, @v_po_formatkey, @v_bookkey, @v_printingkey, @v_lastuserid, getdate())
		  ...
		  BEGIN
			EXEC qpo_generate_gpocost @i_po_projectkey, @i_gpokey, @v_new_sectionkey, 0,
		...
		BEGIN
		  ...
		  BEGIN
			...
			--5/2/18 add xpolinenbr
			SET @v_detaillinenbr = @v_detaillinenbr + 100
			EXEC get_next_key @v_lastuserid, @v_new_detailkey OUTPUT

			SET @v_detail2 = 'Line Item PO #: ' + @xpolineponbr

			INSERT INTO gpodetail
			  (gpokey, detailkey, sectionkey, subsectionkey, detaillinenbr, detail, detailtype, reportformatcode, lastuserid, lastmaintdate)
			VALUES
			  (@i_gpokey, @v_new_detailkey, @v_new_sectionkey, 0, @v_detaillinenbr, @v_detail2, 1, 2, @v_lastuserid, getdate())
			...
			IF @v_sharedposectionind = 1
			BEGIN
				DECLARE cur_sharedrelatedproj CURSOR FOR
				SELECT relatedprojectkey, COALESCE(quantity, 0)
				FROM taqversionformatrelatedproject
				WHERE taqversionformatkey = @v_po_formatkey

				OPEN cur_sharedrelatedproj

				FETCH cur_sharedrelatedproj INTO @v_sharedrelatedprojkey, @v_sharedquantity
    
				WHILE @@fetch_status = 0
				BEGIN
					SET @v_shareddetail = ''

					SELECT @v_shareddetail = title,
					@v_sharedprintingnum = COALESCE(printingnum, printingkey), @v_sharedproductnumber = COALESCE(productnumber,'')
					FROM taqprojectprinting_view
					WHERE taqprojectkey = @v_sharedrelatedprojkey

					SET @v_shareddetail = @v_sharedproductnumber + ' ' + @v_shareddetail + ', Prtg: ' + CONVERT(VARCHAR, @v_sharedprintingnum) + ' - ' + CONVERT(VARCHAR, @v_sharedquantity) + ' COPIES'
					...
					INSERT INTO gpodetail
					  (gpokey, detailkey, sectionkey, subsectionkey, detaillinenbr, detail, detailtype, lastuserid, lastmaintdate)
					VALUES
					  (@i_gpokey, @v_new_detailkey, @v_new_sectionkey, 0, @v_detaillinenbr, @v_shareddetail, 1, @v_lastuserid, getdate())

					FETCH cur_sharedrelatedproj INTO @v_sharedrelatedprojkey, @v_sharedquantity
				END

				CLOSE cur_sharedrelatedproj
				DEALLOCATE cur_sharedrelatedproj
			END
		  END
		END
		  ...
		  BEGIN
			...
			BEGIN      
			  ...
			  BEGIN
				...
				INSERT INTO gposubsection
				  (gpokey, sectionkey, subsectionkey, subsectiontype, key1, key2, key3, quantity, description, finishedgoodind, taqversionformatkey, bookkey, printingkey, lastuserid, lastmaintdate)
				VALUES
				  (@i_gpokey, @v_new_sectionkey, @v_new_subsectionkey, @v_sectiontype, @v_key1, @v_key2, @v_key3, @v_quantity, @v_categorydesc, @v_gpo_finishedgoodind, @v_po_formatkey, @v_bookkey, @v_printingkey, @v_lastuserid, getdate())
			  END
			  ELSE
			  BEGIN --single project PO            
				...
				INSERT INTO gposection
				  (gpokey, sectionkey, sectiontype, key1, key2, key3, quantity, description, finishedgoodind, taqversionformatkey, bookkey, printingkey, lastuserid, lastmaintdate)
				VALUES
				  (@i_gpokey, @v_new_sectionkey, @v_sectiontype, @v_key1, @v_key2, @v_key3, @v_quantity, @v_categorydesc, @v_gpo_finishedgoodind, @v_po_formatkey, @v_bookkey, @v_printingkey, @v_lastuserid, getdate())
			  ...
			  Print 'calculating gpo costs for po summary key/ printing key /sectionkey'
			  Print CONVERT(VARCHAR, @i_po_projectkey)
			  Print CONVERT(VARCHAR, @v_relprintprojkey)
			  Print CONVERT (VARCHAR, @v_new_sectionkey)
				EXEC qpo_generate_gpocost @i_po_projectkey, @i_gpokey, @v_new_sectionkey, @v_new_subsectionkey,
			...
			BEGIN
			  ...
			  FROM taqversionspecitems_view  i
				...
				LEFT OUTER JOIN gentablesitemtype gi ON gi.tableid = 616 AND gi.datacode = i.itemcategorycode AND gi.datasubcode = i.itemcode 
				  AND gi.itemtypecode = @v_itemtype_PO AND COALESCE(gi.itemtypesubcode,0) IN (@v_usageclass_report,0)
			  WHERE i.taqversionspecategorykey = @v_categorykey
				...
				BEGIN
					SET @o_error_code = -1
					SET @o_error_desc = 'no spec items exist for this component'
				END
			  ...
			  -- PM 4/10/18 want to show components even if not notes or specs (usually whole book for Co-Edition or Express PO)
			  IF (@v_num_items > 0 OR @v_num_notes > 0) or 1=1 
			  BEGIN
			...
			BEGIN
			  ...
			  BEGIN						
				...
				IF @v_showdecimalind = 1 AND @v_decimalvalue IS NOT NULL AND @v_decimalvalue <> 0
				BEGIN
				  IF @v_detail <> ''
					SET @v_detail = @v_detail + ', '
			...
			BEGIN                  
			  ...
			  BEGIN
				...
				from taqversionspeccategory tc
				join gentables gt
				on tc.itemcategorycode = gt.datacode
				and gt.tableid = 616
				and gt.externalcode = 4
				join globalcontact gc
				on tc.vendorcontactkey = gc.globalcontactkey
				where tc.taqprojectkey = @v_printing_projectkey
				...
				END
				if @v_vendorname is not null begin
				  SET @v_detail = @v_detail + ' ' + @v_vendorname
				END
			  ...
			  SELECT @v_duedate = COALESCE(t.activedate, NULL), @v_taqtasknote = COALESCE(t.taqtasknote,NULL), @v_taskdesc = d.description
			  FROM taqprojecttask t, datetype d
			  WHERE t.datetypecode = d.datetypecode AND d.qsicode = 36  --Endpaper Due
				AND taqprojectkey IS NULL AND bookkey = @v_bookkey AND printingkey = @v_printingkey
						  
			  IF @v_duedate IS NOT NULL OR @v_taqtasknote IS NOT NULL
			  BEGIN
				SET @v_detail = '* ' + @v_taskdesc + ': ' --'* Endpaper Due Date: '
			...
			BEGIN									  
			  ...
			  BEGIN
				...
				EXEC qpo_write_comment_details @i_po_projectkey, @i_gpokey, @v_new_sectionkey, @v_new_subsectionkey, @v_commenttext, 4, @v_detaillinenbr out, @v_lastuserid
