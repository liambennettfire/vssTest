...
**  09/07/16  Colman  Case 39094 Title Territory Propagation
**  10/13/16  Uday    Case 40982
**  10/28/16  Alan    Case 41424
**  12/15/16  Uday    Case 42240
**  09/22/17  Colman  Case 47418 - Updating categories and propagating does not send subordinate titles to desktop outbox
**  09/28/17  Colman  Case 46642 - Title history not written out to when propagating or copying Keywords
**  11/20/17  Colman  Case 48454 - bookmisc propagation always deleted all misc items from propagation target.
**  03/29/18  Colman  Case 50565 - Copy title territories button is not working
********************************************************************************************************************/
BEGIN
...
@v_new_territoryrightskey INT,
@v_tableid INT,
@v_productdetaildesc VARCHAR(255),
@v_productdetailsubdesc VARCHAR(255),
@v_productdetailsub2desc VARCHAR(255),
@v_keyword VARCHAR(255)
...
IF ((@i_tablename is not null and ltrim(rtrim(@i_tablename)) <> '') AND
    ...
    DECLARE titlehistorycolumns_cur CURSOR FOR
      SELECT tablename, columnname, datatype, columndescription, columnkey
       ...
       WHERE columndescription = 'Web Title Dates Propagation'
         AND (workfieldind = 1 OR @i_ignore_workfieldind = 1)
         AND activeind = 1
  END
  ELSE
  IF @i_tablename = 'territoryrights' BEGIN
    DECLARE titlehistorycolumns_cur CURSOR FOR
      SELECT 'territoryrights', columnname, datatype, columndescription, columnkey
        FROM titlehistorycolumns 
       WHERE columnkey = 272
         AND (workfieldind = 1 OR @i_ignore_workfieldind = 1)
         AND activeind = 1
  END
  ELSE
  IF @i_tablename = 'territoryrightcountries' BEGIN
    DECLARE titlehistorycolumns_cur CURSOR FOR
      SELECT 'territoryrightcountries', columnname, datatype, columndescription, columnkey
        FROM titlehistorycolumns 
       WHERE columnkey = 272
         AND (workfieldind = 1 OR @i_ignore_workfieldind = 1)
         AND activeind = 1
  ...
  BEGIN
    DECLARE titlehistorycolumns_cur CURSOR FOR
     select tablename, columnname, datatype, columndescription, columnkey
       from titlehistorycolumns 
      where (workfieldind = 1 or @i_ignore_workfieldind = 1)
        and activeind = 1
...
CREATE TABLE #territoryrights (
  ...
  autoterritorydescind tinyint NULL,
  exclusivecode int NULL,
  singlecountrycode int NULL,
  singlecountrygroupcode int NULL,
  updatewithsubrightsind tinyint NULL,
  note varchar(255) NULL,
  forsalehistory varchar(2000) NULL,
  notforsalehistory varchar(2000) NULL,
  lastuserid varchar(30) NULL,
  lastmaintdate datetime NULL,
  overridepropagationind tinyint null)
...
CREATE TABLE #territoryrightcountries
(
  territoryrightskey int NOT NULL,
...
CREATE TABLE #bookproductdetail (
...
WHILE (@@FETCH_STATUS <> -1) BEGIN
  ...
  IF @v_tablename = 'bookmisc' BEGIN
    ...
    INSERT INTO #bookmisc 
      ...
      FROM bookmisc 
        JOIN bookmiscitems bm ON bookmisc.misckey = bm.misckey
          AND ISNULL(bm.propagatemiscitemind,0) = 1 
      WHERE bookkey = @v_bookkey_copy_from
    ...
    BEGIN
      IF @v_columnkey = 225 SET @v_misctype = 1
      IF @v_columnkey = 226 SET @v_misctype = 2
      IF @v_columnkey = 227 SET @v_misctype = 3
      IF @v_columnkey = 247 SET @v_misctype = 4
      IF @v_columnkey = 248 SET @v_misctype = 5
    ...
    DELETE FROM bookmisc
    WHERE bookkey = @v_bookkey_copy_to
      AND misckey IN (SELECT misckey FROM #bookmisc)
  ...
  IF @v_columnkey = 272   -- territoryrights
  BEGIN
    SELECT @v_count = COUNT(*) FROM territoryrights
      WHERE bookkey = @v_bookkey_copy_to
      AND COALESCE(overridepropagationind,0) = 1
    ...
    BEGIN
      ...
      SELECT * FROM territoryrights WHERE bookkey = @v_bookkey_copy_from
                      
      UPDATE #territoryrights SET bookkey = @v_bookkey_copy_to

      SELECT @v_territoryrightskey = territoryrightskey
      FROM #territoryrights

      execute get_next_key 'qsidba', @v_new_territoryrightskey OUTPUT
      UPDATE #territoryrights SET territoryrightskey = @v_new_territoryrightskey WHERE territoryrightskey = @v_territoryrightskey  

      INSERT INTO territoryrights SELECT * FROM #territoryrights
    
      SELECT @v_territoriescode = territoriescode FROM book WHERE bookkey = @v_bookkey_copy_from
      UPDATE book SET territoriescode = @v_territoriescode WHERE bookkey = @v_bookkey_copy_to
      ...
      INSERT INTO #territoryrightcountries
        SELECT * FROM territoryrightcountries WHERE bookkey = @v_bookkey_copy_from

      UPDATE #territoryrightcountries 
         SET bookkey = @v_bookkey_copy_to, territoryrightskey = @v_new_territoryrightskey

      INSERT INTO territoryrightcountries 
        SELECT * FROM #territoryrightcountries

      exec qtitle_update_titlehistory 'territoryrights', '(multiple)', @v_bookkey_copy_to, 1, null,
         null, 'UPDATE', @v_user_name, @v_sortorder, null, @o_error_code, @o_error_desc
        ...
        IF @v_bisaccategorycode IS NOT NULL AND @v_bisaccategorycode > 0 BEGIN
          ...
          IF @i_columnname = 'bisaccategorycode' BEGIN
            ...
            exec qtitle_update_titlehistory @v_tablename, 'bisaccategorycode', @v_bookkey_copy_to, 1, null,
               @v_fielddesc, 'UPDATE', @v_user_name, @v_sortorder, @v_bisaccategorydesc, @o_error_code, @o_error_desc
          END
            ...
            IF @v_bisaccategorysubcode IS NOT NULL AND @v_bisaccategorysubcode > 0 BEGIN
              ...
              IF @v_bisaccategorysubdesc IS NOT NULL AND @v_bisaccategorysubdesc <> '' BEGIN
                ...
                exec qtitle_update_titlehistory @v_tablename, 'bisaccategorysubcode', @v_bookkey_copy_to, 1, null,
                  @v_bisaccategorysubdesc, 'UPDATE', @v_user_name, @v_sortorder, @v_bisaccategorydesc, @o_error_code, @o_error_desc
              END
            END
          END
        END
  ...
  if @v_tablename = 'bookproductdetail' begin
    delete from #bookproductdetail
    delete from bookproductdetail
    where bookkey = @v_bookkey_copy_to
    insert into #bookproductdetail select * from bookproductdetail where bookkey = @v_bookkey_copy_from
    update #bookproductdetail set bookkey = @v_bookkey_copy_to
    
    DECLARE bookproductdetail_cur CURSOR FOR
      SELECT datacode,datasubcode, datasub2code, sortorder, tableid
        FROM #bookproductdetail
      ORDER BY sortorder ASC

    OPEN bookproductdetail_cur
    
    FETCH NEXT FROM bookproductdetail_cur INTO @v_datacode,@v_datasubcode,@v_datasub2code,@v_sortorder,@v_tableid
    
    WHILE @@FETCH_STATUS = 0
    BEGIN
        IF @v_datacode IS NOT NULL AND @v_datacode > 0 BEGIN
          SELECT @v_productdetaildesc = COALESCE(g.datadesc, e.gentext3) 
      FROM gentables g INNER JOIN gentables_ext e ON g.tableid = e.tableid AND g.datacode = e.datacode
       WHERE g.tableid = @v_tableid AND g.datacode = @v_datacode
          
          IF @i_columnname = 'datacode' BEGIN
            SELECT @v_fielddesc = LEFT(ltrim(rtrim(@v_productdetaildesc)), 255) 
            
            exec qtitle_update_titlehistory @v_tablename, 'datacode', @v_bookkey_copy_to, 1, null,
               @v_fielddesc, 'UPDATE', @v_user_name, @v_sortorder, @v_productdetaildesc, @o_error_code, @o_error_desc
          END
          
          IF @i_columnname = 'datasubcode' BEGIN
            IF @v_datasubcode IS NOT NULL AND @v_datasubcode > 0 BEGIN
        SELECT @v_productdetaildesc = COALESCE(g.datadesc, e.gentext3) 
        FROM gentables g INNER JOIN gentables_ext e ON g.tableid = e.tableid AND g.datacode = e.datacode
        WHERE g.tableid = @v_tableid AND g.datacode = @v_datacode

              SELECT @v_productdetailsubdesc = datadesc FROM subgentables WHERE tableid = @v_tableid AND datacode = @v_datacode
                AND datasubcode = @v_datasubcode
              IF @v_productdetailsubdesc IS NOT NULL AND @v_productdetailsubdesc <> '' BEGIN
                SELECT @v_fielddesc = LEFT(ltrim(rtrim(@v_productdetaildesc)) + ' - ' + ltrim(rtrim(@v_productdetailsubdesc)), 255)

                exec qtitle_update_titlehistory @v_tablename, 'datasubcode', @v_bookkey_copy_to, 1, null,
                  @v_fielddesc, 'UPDATE', @v_user_name, @v_sortorder, @v_productdetailsubdesc, @o_error_code, @o_error_desc
              END
            END
          END

          IF @i_columnname = 'datasub2code' BEGIN
            IF @v_datasub2code IS NOT NULL AND @v_datasub2code > 0 BEGIN
        ...
        WHERE g.tableid = @v_tableid AND g.datacode = @v_datacode
              ...
              SELECT @v_productdetailsub2desc = datadesc FROM sub2gentables WHERE tableid = @v_tableid AND datacode = @v_datacode AND datasubcode = @v_datasubcode
                AND datasub2code = @v_datasub2code
              IF @v_productdetailsub2desc IS NOT NULL AND @v_productdetailsub2desc <> '' BEGIN
                SELECT @v_fielddesc = LEFT(ltrim(rtrim(@v_productdetaildesc)) + ' - ' + ltrim(rtrim(@v_productdetailsubdesc)) + ' - ' + ltrim(rtrim(@v_productdetailsub2desc)), 255)

                exec qtitle_update_titlehistory  @v_tablename, 'datasub2code', @v_bookkey_copy_to, 1, null,
                  @v_fielddesc, 'UPDATE', @v_user_name, @v_sortorder, @v_productdetailsub2desc, @o_error_code, @o_error_desc
              END
            END
          END
        END
    ...
    CLOSE bookproductdetail_cur
    DEALLOCATE bookproductdetail_cur

    insert into bookproductdetail select * from #bookproductdetail
  end

  if @v_tablename = 'citation' and EXISTS (select * from titlehistorycolumns where columnkey IN (67, 68)  and (workfieldind = 1 OR @i_ignore_workfieldind = 1)) begin  
   DECLARE crCitation CURSOR FOR
    ...
    WHILE (@@FETCH_STATUS <> -1) BEGIN                  
        ...
        DECLARE titlehistory_cur CURSOR FOR -- cleanup for citations that connect to a parent citation via propagatefromcitationkey that has been deleted
         select columnname
         from titlehistorycolumns  where (workfieldind = 1 OR @i_ignore_workfieldind = 1) and tablename = @i_tablename
  ...
  if EXISTS (select * from titlehistorycolumns where columnkey IN (67, 68)  and (workfieldind = 1 OR @i_ignore_workfieldind = 1)) begin
  ...
  if @v_tablename = 'bookcategory' begin
    ...
    DECLARE bookcat_cur CURSOR FOR
    ...
    FETCH NEXT FROM bookcat_cur INTO @v_categorycode, @v_sortorder
    
    WHILE @@FETCH_STATUS = 0
    BEGIN
        IF @v_categorycode IS NOT NULL AND @v_categorycode > 0 BEGIN
          SELECT @v_categorydesc = datadesc FROM gentables WHERE tableid = 317 AND datacode = @v_categorycode
          
          IF @i_columnname = 'categorycode' BEGIN
            SELECT @v_fielddesc = ltrim(rtrim(@v_categorydesc)) 
            
            exec qtitle_update_titlehistory @v_tablename, 'categorycode', @v_bookkey_copy_to, 1, null,
               @v_fielddesc, 'UPDATE', @v_user_name, @v_sortorder, @v_categorydesc, @o_error_code, @o_error_desc
          END
  ...
  if @v_tablename = 'booksubjectcategory' begin
    ...
    BEGIN
      ...
      EXEC qtitle_update_titlehistory 'booksubjectcategory', 'categorycode', @v_bookkey_copy_to, 1, NULL, @v_categorydesc, 'insert', @v_user_name, @v_historyorder, @v_fielddetail, @o_error_code OUTPUT, @o_error_desc OUTPUT
      EXEC qtitle_update_titlehistory 'booksubjectcategory', 'categorysubcode', @v_bookkey_copy_to, 1, NULL, @v_categorysubdesc, 'insert', @v_user_name, @v_historyorder, @v_fielddetail, @o_error_code OUTPUT, @o_error_desc OUTPUT
      EXEC qtitle_update_titlehistory 'booksubjectcategory', 'categorysub2code', @v_bookkey_copy_to, 1, NULL, @v_categorysub2desc, 'insert', @v_user_name, @v_historyorder, @v_fielddetail, @o_error_code OUTPUT, @o_error_desc OUTPUT
      EXEC qtitle_update_titlehistory 'booksubjectcategory', 'sortorder', @v_bookkey_copy_to, 1, NULL, @v_sortorder_varchar, 'insert', @v_user_name, @v_historyorder, @v_fielddetail, @o_error_code OUTPUT, @o_error_desc OUTPUT
    ...
    CLOSE booksubjectcat_cur
    DEALLOCATE booksubjectcat_cur
  ...
  IF @v_tablename = 'bookkeywords' 
  BEGIN
    -- Write history for deleted keywords
    DECLARE keyword_del_cur CURSOR FOR
    SELECT keyword
    FROM bookkeywords
    WHERE bookkey = @v_bookkey_copy_to
      AND keyword NOT IN (SELECT keyword FROM bookkeywords WHERE bookkey = @v_bookkey_copy_from)
    ORDER BY sortorder
    
    OPEN keyword_del_cur
    FETCH keyword_del_cur INTO @v_keyword
    
    WHILE @@FETCH_STATUS = 0
    BEGIN
      EXEC qtitle_update_titlehistory 'bookkeywords', 'keyword', @v_bookkey_copy_to, 1, NULL, @v_keyword, 'delete', @v_user_name, NULL, NULL, @o_error_code OUTPUT, @o_error_desc OUTPUT
      FETCH keyword_del_cur INTO @v_keyword
    END
    CLOSE keyword_del_cur
    DEALLOCATE keyword_del_cur

    DECLARE keyword_cur CURSOR FOR
    SELECT keyword
    FROM bookkeywords
    WHERE bookkey = @v_bookkey_copy_from
      AND keyword NOT IN (SELECT keyword FROM bookkeywords WHERE bookkey = @v_bookkey_copy_to)
    ORDER BY sortorder
    
    OPEN keyword_cur
    FETCH keyword_cur INTO @v_keyword
    
    WHILE @@FETCH_STATUS = 0
    BEGIN
      EXEC qtitle_update_titlehistory 'bookkeywords', 'keyword', @v_bookkey_copy_to, 1, NULL, @v_keyword, 'insert', @v_user_name, NULL, NULL, @o_error_code OUTPUT, @o_error_desc OUTPUT
      FETCH keyword_cur INTO @v_keyword
    END
    CLOSE keyword_cur
    ...
    EXEC qtitle_update_Keywords_ONIX @v_bookkey_copy_to, @v_user_name, 0, @o_error_code output, @o_error_desc output
      IF @o_error_code <> 0 BEGIN
      SET @o_error_code = -1
      SET @o_error_desc = 'Unable to check for title task restrictions: ' + @o_error_desc
      RETURN
