...
IF OBJECT_ID('dbo.UpdFld_XVQ_ActiveDate') IS NOT NULL DROP PROCEDURE dbo.UpdFld_XVQ_ActiveDate
GO

CREATE PROCEDURE dbo.UpdFld_XVQ_ActiveDate
@bookkey        int,
@datetypecode   int,
...
@validation_sproc_name varchar(100), -- null, 'isdate', or name of sproc that takes appropriate parameters (see template)
...
begin
	if @transform_sproc_name is not null AND OBJECT_ID(@transform_sproc_name) is not null
		EXEC @transform_sproc_name 1, @datetypecode, @extract output, @length output, @bookkey
	...
	if @is_valid is not null begin
		...
		if OBJECT_ID(@validation_sproc_name) is null begin
			...
			EXEC @validation_sproc_name @datetypecode, @extract, @vldn_result_code output, @vldn_result_desc output, @bookkey
...
begin
	if @transform_sproc_name is not null AND OBJECT_ID(@transform_sproc_name) is not null
		EXEC @transform_sproc_name 2, @datetypecode, @extract output, @length output, @bookkey

	declare @uf_error_code int
	declare @uf_error_desc varchar(2000)

	-- Do insert/update and (if applicable) titlehistory
	EXEC dbo.UpdFld_ActiveDate @bookkey, @datetypecode, @extract, 1, @length, @userid, @uf_error_code output, @uf_error_desc output
...
begin  -- write to the qsijobmessage table
	...
	-- Code specific to ActiveDate data
	declare @datadesc varchar(255)
	select	@datadesc = [description]
	from	datetype
