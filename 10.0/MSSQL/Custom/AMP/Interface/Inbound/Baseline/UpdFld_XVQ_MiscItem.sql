...
IF OBJECT_ID('dbo.UpdFld_XVQ_MiscItem') IS NOT NULL DROP PROCEDURE dbo.UpdFld_XVQ_MiscItem
GO

CREATE PROCEDURE dbo.UpdFld_XVQ_MiscItem
@bookkey        int,
@misckey        int,
...
begin
	if @transform_sproc_name is not null AND OBJECT_ID(@transform_sproc_name) is not null
		EXEC @transform_sproc_name 1, @misckey, @extract output, @length output, @bookkey
	...
	if @is_valid is not null begin
		...
		if OBJECT_ID(@validation_sproc_name) is null begin
			...
			EXEC @validation_sproc_name @bookkey, @misckey, @codeform, @extract, @vldn_result_code output, @vldn_result_desc output
...
begin
	if @transform_sproc_name is not null AND OBJECT_ID(@transform_sproc_name) is not null
		EXEC @transform_sproc_name 2, @misckey, @extract output, @length output, @bookkey

	declare @uf_error_code int
	declare @uf_error_desc varchar(2000)

	-- Do insert/update and (if applicable) titlehistory
	EXEC dbo.UpdFld_MiscItem @bookkey, @misckey, @codeform, @extract, 1, @length, @userid, @uf_error_code output, @uf_error_desc output
...
begin  -- write to the qsijobmessage table
	...
	-- Code specific to MiscItem data
	declare @datadesc varchar(255)
	select	@datadesc = miscname
	from	bookmiscitems
