IF EXISTS (SELECT * FROM sysobjects WHERE id = object_id('dbo.contactrole_coeditionname ') AND type = 'TR')
  DROP TRIGGER dbo.contactrole_coeditionname 
GO

CREATE TRIGGER contactrole_coeditionname  ON taqprojectcontactrole
FOR INSERT AS
...
BEGIN
  ...
  SELECT @v_projectkey = i.taqprojectkey, @v_userid = i.lastuserid, @V_rolecode = rolecode
  FROM inserted i  

  SELECT @v_Customercode = datacode 
  FROM gentables
  WHERE tableid = 285 aND qsicode = 28

  IF @v_rolecode <> @v_Customercode
    RETURN
