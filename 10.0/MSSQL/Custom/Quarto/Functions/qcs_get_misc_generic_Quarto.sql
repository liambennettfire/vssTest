if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].qcs_get_misc_generic') and xtype in (N'FN', N'IF', N'TF'))
...
BEGIN
		...
		-- NEWID() AS Id,
		  ...
		  (case when mediatypecode = 9 then 100 -- PPT is 100% on eBooks 
		        when (Select Coalesce(finalprice, budgetprice) from bookprice where bookkey = @bookkey and pricetypecode = 31 and currencytypecode = 37 and activeind = 1) = (Select Coalesce(finalprice, budgetprice) from bookprice where bookkey = @bookkey and 
pricetypecode = 8 and currencytypecode = 37 and activeind = 1) THEN 0 -- standard book, 0 % taxable
		  ...
		  -- associated format code
			...
			if exists (Select 1 from book where bookkey = @bookkey and usageclasscode = 2)
				BEGIN
					...
					JOIN bookdetail bd 
					on a.associatetitlebookkey = bd.bookkey 
					JOIN subgentables s 
					ON bd.mediatypecode = s.datacode and bd.mediatypesubcode = s.datasubcode 
					where a.bookkey = @bookkey
					and a.associationtypecode = 6 -- titles in sets
					and a.associationtypesubcode = 1 -- set component
					and s.tableid = 312 -- associated format code is stored in format's externalcode 
					...
					-- Hachette sub format codes, stored in externalcode of child format for Quarto
					...
					FROM associatedtitles a 
					JOIN booksimon bs 
					on a.associatetitlebookkey = bs.bookkey 
					JOIN gentables g 
					ON bs.formatchildcode = g.datacode 
					where a.bookkey = @bookkey
					and a.associationtypecode = 6 -- titles in sets
					and a.associationtypesubcode = 1 -- set component
					and g.tableid = 300 -- associated format code is stored in child format
					and g.deletestatus = 'N'
					and a.sortorder = 1
					and ISNULL(g.externalcode, '') <> ''
					and ISNULL(bs.formatchildcode, 0) <> 0
...
grant all on dbo.qcs_get_misc_generic to public
go
