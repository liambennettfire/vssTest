...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[qweb_get_BestPrice_EffDate]') and xtype in (N'FN', N'IF', N'TF'))
drop function [dbo].[qweb_get_BestPrice_EffDate]
GO
...
CREATE FUNCTION [dbo].[qweb_get_BestPrice_EffDate] 
            	(@i_bookkey 	INT,
            	@i_pricetype	INT,
            	@i_currencytype	INT)
