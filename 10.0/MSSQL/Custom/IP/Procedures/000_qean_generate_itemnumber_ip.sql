...
**	--------	------	-----------
**  09/18/17  Colman  Case 47316 - Applied changes made to generic procedure for Case 44815
******************************************************************************************/
...
BEGIN
  ...
  -- Increment next available itemnumber
  ...
  BEGIN
    SET @o_error_code = -1 
    SET @o_error_desc = 'Could not access isbn\taqprojectitle table to verify uniqueness of itemnumber.'
    RETURN
  END

  IF @v_count > 0  -- This itemnumber already exists. Generate another itemnumber
    GOTO DOIT_AGAIN
