...
if exists (select * from dbo.sysobjects where id = Object_id('dbo.insert_comments_para_break') and (type = 'P' or type = 'RF'))
begin
 drop proc dbo.insert_comments_para_break
end

GO
create proc dbo.insert_comments_para_break
AS
...
while (@i_status<>-1 )  /* sttus 1*/
  begin
	...
	select @searchtest1 = charindex('<p>', @online_commenttext)
	  ...
	  while @searchtest1 > 0 
	    begin
		select @online_commenttext = STUFF(@online_commenttext,@searchtest1,3,char(13))
		...
		if @charfind2 > 0 
		  begin
			...
			/* insert the paragraph by itself do not circle the entire paragraph*/
				select @online_commenttext = STUFF(@online_commenttext,@searchtest1,3,'{\par}\pard\ql{\f0\fs20\cf0\up0\dn0 }')
