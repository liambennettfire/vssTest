IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.bookcountries_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.bookcountries_trackertrigger'
	DROP TRIGGER dbo.bookcountries_trackertrigger
	PRINT 'Creating TRIGGER dbo.bookcountries_trackertrigger...'
END

GO

CREATE TRIGGER bookcountries_trackertrigger
ON bookcountries
FOR INSERT, UPDATE
AS
