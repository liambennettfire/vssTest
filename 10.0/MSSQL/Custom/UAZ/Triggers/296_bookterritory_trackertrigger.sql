IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.bookterritory_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.bookterritory_trackertrigger'
	DROP TRIGGER dbo.bookterritory_trackertrigger
	PRINT 'Creating TRIGGER dbo.bookterritory_trackertrigger...'
END

GO

CREATE TRIGGER bookterritory_trackertrigger
ON bookterritory
FOR INSERT, UPDATE
AS
