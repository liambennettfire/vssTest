IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.book_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.book_trackertrigger'
	DROP TRIGGER dbo.book_trackertrigger
	PRINT 'Creating TRIGGER dbo.book_trackertrigger...'
END

GO

CREATE TRIGGER book_trackertrigger
ON book
FOR INSERT, UPDATE
AS
