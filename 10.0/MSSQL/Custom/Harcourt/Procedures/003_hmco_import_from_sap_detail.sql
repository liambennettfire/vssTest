...
--
--	8/21/17 - JL - Case 46757
--	add tentative page count, estimated trim width, estimated trim length, and UPC
-- =================================================================================================================
...
@esttrimwidth			varchar(10)	,
@esttrimlength			varchar(10)	,
@pagecount			int			,
...
@esttrimwidth2			varchar(10)	,
@esttrimlength2			varchar(10)	,
@pagecount2			int			,
@tentativepagecount2			int			,
...
DECLARE @Participant_Sort_order int,@Participant_sort_order_ORIG int--JL20170907> Case: 47092 
...
@esttrimwidth			= isnull(esttrimsizewidth,'')	,
@esttrimlength			= isnull(esttrimsizelength,'')	,
@pagecount			= pagecount			,
...
,@Participant_Sort_order = isnull(Participant_Sort_Order,-1) --JL
...
begin
	...
	select @upc2 = isnull(dbo.get_isbn_item (@i_bookkey, 21),'')

	if @i_update_mode = 'B' and @upc2 <> ''
		select @update = 0

	if @update = 1 and @upc <> @upc2
	begin
		if @upc = '&&&'	
			select @upc = null
		...
		IF @v_error <> 0 OR @v_rowcount = 0 BEGIN
			SET @o_error_code = -1
			SET @o_error_desc = 'Unable to update UPC on isbn table.   Error #' + cast(@v_error as varchar(20))
			RETURN
		END 

		exec qtitle_update_titlehistory 'isbn', 'upc' , @i_bookkey, 1, 0, @upc, 'Update', @i_userid, 
				null, 'UPC', @o_error_code output, @o_error_desc output
	end
end
...
if @esttrimlength <> '' or @esttrimwidth <> ''			--8/21/17 JL case 46757
begin
	SELECT @esttrimlength2 = isnull(esttrimsizelength,''),
			@esttrimwidth2 = isnull(esttrimsizewidth,'')
	FROM printing
	WHERE bookkey = @i_bookkey
	AND  printingkey = 1 
...
begin
	...
	if @i_update_mode = 'B' and @esttrimlength2 <> ''
		select @update = 0

	if @update = 1 and @esttrimlength <> @esttrimlength2
	begin
		if @esttrimlength = '&&&'	
			select @esttrimlength = null

		update printing
		set esttrimsizelength = @esttrimlength,
		lastuserid = @i_userid,
		lastmaintdate = getdate()
		WHERE bookkey = @i_bookkey
		AND  printingkey = 1 
		...
		IF @v_error <> 0 OR @v_rowcount = 0 BEGIN
			SET @o_error_code = -1
			SET @o_error_desc = 'Unable to update est trim length on printing table.   Error #' + cast(@v_error as varchar(20))
			RETURN
		END 

		exec qtitle_update_titlehistory 'printing', 'esttrimsizelength' , @i_bookkey, 1, 0, @esttrimlength, 'Update', @i_userid, 
			null, 'est Trim length', @o_error_code output, @o_error_desc output
	end
end
...
begin
	...
	if @i_update_mode = 'B' and @esttrimwidth2 <> ''
		select @update = 0

	if @update = 1 and @esttrimwidth <> @esttrimwidth2
	begin
		if @esttrimwidth = '&&&'	
			select @esttrimwidth = null

		update printing
		set esttrimsizewidth = @esttrimwidth,
		lastuserid = @i_userid,
		lastmaintdate = getdate()
		WHERE bookkey = @i_bookkey
		AND  printingkey = 1 
		...
		IF @v_error <> 0 OR @v_rowcount = 0 BEGIN
			SET @o_error_code = -1
			SET @o_error_desc = 'Unable to update est trim width on printing table.   Error #' + cast(@v_error as varchar(20))
			RETURN
		END 

		exec qtitle_update_titlehistory 'printing', 'esttrimsizewidth' , @i_bookkey, 1, 0, @esttrimwidth, 'Update', @i_userid, 
			null, 'est Trim width', @o_error_code output, @o_error_desc output
	end
end
...
if @tentativepagecount > 0			--8/21/17 JL case 46757
begin
	select @update = 1

	SELECT @tentativepagecount2 = isnull(tentativepagecount,0) 
	FROM 	printing
	WHERE 	bookkey = @i_bookkey
	AND  printingkey = 1 

	if @i_update_mode = 'B' and @tentativepagecount2 > 0
		select @update = 0

	if @update = 1 and @tentativepagecount <> @tentativepagecount2
	begin
--		if @tentativepagecount = '&&&'	
--			select @tentativepagecount = null

		update printing
		set tentativepagecount = @tentativepagecount,
		lastuserid = @i_userid,
		lastmaintdate = getdate()
		WHERE bookkey = @i_bookkey
		AND  printingkey = 1 
		...
		IF @v_error <> 0 OR @v_rowcount = 0 BEGIN
			SET @o_error_code = -1
			SET @o_error_desc = 'Unable to update tentative page count on printing table.   Error #' + cast(@v_error as varchar(20))
			RETURN
		END 

		exec qtitle_update_titlehistory 'printing', 'tentativepagecount' , @i_bookkey, 1, 0, @tentativepagecount, 'Update', @i_userid, 
			null, 'tentativePage count', @o_error_code output, @o_error_desc output
	end
end
...
BEGIN
	...
	IF @DEBUG <> 0 PRINT 'start processing rolecode ...'
 	...
 	SELECT @bookcontactkey=bookcontactkey FROM bookcontactrole 
	WHERE bookcontactkey in (SELECT bookcontactkey FROM bookcontact
							where bookkey=@i_bookkey AND printingkey = @printingkey) 
	and rolecode = @Participant_Role_type
	...
	BEGIN
		...
		exec get_next_key @i_userid, @bookcontactkey output
		
		SELECT @v_sortorder = 1 + COALESCE(MAX(sortorder),0) FROM bookcontact where bookkey=@i_bookkey AND printingkey = @printingkey
		
		IF @DEBUG <> 0 PRINT @v_sortorder
	...
	--JL put this sort order update down here to make sure it runs - the role found logic above won't update the row if the contact hasn't changed, but maybe just the sort order is changing
	SELECT @Participant_Sort_order_ORIG=COALESCE(Sortorder,0) from bookcontact where bookcontactkey=@bookcontactkey

	IF @DEBUG <> 0 PRINT @participant_sort_order

	if @bookcontactkey > -1 and @Participant_Sort_order > -1 and @Participant_Sort_order <> @Participant_sort_order_ORIG
	begin
		update bookcontact
		set sortorder = @Participant_Sort_order,
		lastuserid = @i_userid,
		lastmaintdate = getdate()
		where bookcontactkey = @bookcontactkey
	end
