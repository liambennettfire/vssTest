...
GO
if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_stg_sub_rights_inc_HMH') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_stg_sub_rights_inc_HMH
GO

CREATE PROCEDURE qpl_calc_stg_sub_rights_inc_HMH (  
  @i_projectkey INT,
  @i_plstage    INT,
...
/******************************************************************************************
**  Name: qpl_calc_stg_sub_rights_inc_HMH
**  Desc: Harcourt Item 4 - Stage/Subsidiary Rights Income.
**
**  Auth: Kate
...
BEGIN
  ...
  ELSE
    BEGIN
      ...
      EXEC qpl_calc_ver_sub_rights_inc_HMH @i_projectkey, @i_plstage, @v_selected_versionkey, @o_result OUTPUT
    END
  
END
GO

GRANT EXEC ON qpl_calc_stg_sub_rights_inc_HMH TO PUBLIC
GO
