if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_stg_gross_print_sales_HMH') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_stg_gross_print_sales_HMH
GO

CREATE PROCEDURE qpl_calc_stg_gross_print_sales_HMH (  
  @i_projectkey INT,
  @i_plstage    INT,
...
BEGIN
  ...
  ELSE
    BEGIN
      ...
      EXEC qpl_calc_ver_gross_print_sales_HMH @i_projectkey, @i_plstage, @v_selected_versionkey, @o_result OUTPUT
    END
  
END
GO

GRANT EXEC ON qpl_calc_stg_gross_print_sales_HMH TO PUBLIC
GO
