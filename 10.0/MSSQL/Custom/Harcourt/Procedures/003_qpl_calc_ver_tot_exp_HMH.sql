...
GO
if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_ver_tot_exp') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_ver_tot_exp
GO
if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_ver_tot_exp_HMH') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_ver_tot_exp_HMH
GO

CREATE PROCEDURE qpl_calc_ver_tot_exp_HMH (  
  @i_projectkey INT,
  @i_plstage    INT,
  @i_plversion  INT,
...
/******************************************************************************************
**  Name: qpl_calc_ver_tot_exp_HMH
**  Desc: Harcourt Item 28 - Version/TOTAL Expenses.
**
**  Auth: Kate
...
BEGIN
  ...
  EXEC qpl_calc_ver_coop_HMH @i_projectkey, @i_plstage, @i_plversion, @v_coop_cost OUTPUT
  ...
  EXEC qpl_calc_ver_fulfill_HMH @i_projectkey, @i_plstage, @i_plversion, @v_fulfillment_cost OUTPUT
  ...
  -- HMH only: Subrights Expenses
  EXEC qpl_calc_ver_sub_rights_exp_HMH @i_projectkey, @i_plstage, @i_plversion, @v_subrights_cost OUTPUT
...
GRANT EXEC ON qpl_calc_ver_tot_exp_HMH TO PUBLIC
GO
