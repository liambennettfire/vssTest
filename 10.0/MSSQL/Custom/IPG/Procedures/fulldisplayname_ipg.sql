...
/*				also change \p to \n 					*/
/*  Anes Hrenovica	author table is changed to globalcontact  */
/**************************************************************************/
...
DECLARE @v_accreditation_code INT
...
DECLARE @v_accreditation varchar(40)

DECLARE c_authorlist CURSOR FOR
   SELECT ISNULL(globalcontact.firstname, ''), 
          ISNULL(globalcontact.lastname, ''), 
          ISNULL(globalcontact.middlename, ''),
          ISNULL(globalcontact.accreditationcode, 0),  
          ISNULL(globalcontact.suffix, ''), 
          ISNULL(globalcontact.degree, ''), 
          ISNULL(gentables.alternatedesc2, ''), 
          gentables.gen1ind,
          bookauthor.sortorder ,
	  bookauthor.authorkey
     FROM bookauthor, gentables, globalcontact
    WHERE globalcontact.globalcontactkey = bookauthor.authorkey AND
          bookauthor.authortypecode = gentables.datacode AND
	  bookauthor.bookkey = @bookkey AND
...
BEGIN
   ...
   FETCH NEXT FROM c_authorlist
      INTO @v_firstname, @v_lastname, @v_middlename,@v_accreditation_code,@v_suffix, @v_degree, @v_description, 
		@v_qsicode, @v_sortorder, @v_authorkey
   ...
   WHILE (@@FETCH_STATUS= 0)  /*LOOP*/
      BEGIN
		...
		if @v_accreditation_code > 0 begin
		select @v_accreditation = datadesc
		from gentables 
		where tableid = 210
		and datacode = @v_accreditation_code
		
		set @v_name = @v_accreditation + ' ' + @v_firstname
		end
 	   ...
 	   IF @v_middlename <> ''
         ...
         FETCH NEXT FROM c_authorlist
            INTO @v_firstname, @v_lastname, @v_middlename,@v_accreditation_code,@v_suffix, @v_degree, @v_description, @v_qsicode, @v_sortorder, @v_authorkey
