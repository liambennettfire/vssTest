...
ALTER procedure [dbo].[qweb_ecf_Category_Insert_Series] as
...
BEGIN
  ...
  DECLARE c_pss_series CURSOR fast_forward FOR
	--USER ALTERNATE DESC 2 instead 10/13/09 updated by TT
	Select rtrim(ltrim(datadesc)), datacode, COALESCE(sortorder,0) sortorder, deletestatus, COALESCE(alternatedesc2,'')
	from UAP..gentables
	...
	FETCH NEXT FROM c_pss_series
		INTO @v_categorydesc,@v_datacode,@i_sortorder,@v_deletestatus, @v_alternatedesc2
		...
		If @v_deletestatus='N'
			...
			If not exists (Select * 
							...
							where parentcategoryid = @parent_categoryid
							  and [name] = @v_alternatedesc2) AND @v_categoryid = 0
		
			begin
				
				exec CategoryInsert
				NULL,				--@CategoryId int = NULL output,
				@v_alternatedesc2,	--@Name nvarchar(50),
			...
			--already exists just update
				DECLARE @catid int
				Select @catid = CategoryId from category where parentcategoryid = @parent_categoryid and [name] = @v_alternatedesc2
