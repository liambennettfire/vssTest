IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.taqprojecttask_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.taqprojecttask_trackertrigger'
	DROP TRIGGER dbo.taqprojecttask_trackertrigger
	PRINT 'Creating TRIGGER dbo.taqprojecttask_trackertrigger...'
END

GO

CREATE TRIGGER taqprojecttask_trackertrigger
ON taqprojecttask
FOR INSERT, UPDATE
AS
