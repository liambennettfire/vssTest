IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.bookauthor_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.bookauthor_trackertrigger'
	DROP TRIGGER dbo.bookauthor_trackertrigger
	PRINT 'Creating TRIGGER dbo.bookauthor_trackertrigger...'
END

GO

CREATE TRIGGER bookauthor_trackertrigger
ON bookauthor
FOR INSERT, UPDATE
AS
