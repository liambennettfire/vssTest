...
create procedure qweb_ecf_insert_product_images (@i_bookkey int, @v_filepath varchar(255)) as
...
BEGIN
	...
	select  @i_productfetchstatus  = @@FETCH_STATUS
	 ...
	 while (@i_productfetchstatus >-1 )
		...
		Select @v_mediatype = UNL.dbo.qweb_get_Media(@i_c_bookkey, 'D')
		Select @d_datetime = getdate()
		select @i_maxsubbookkey=UNL.dbo.qweb_get_subordinate_max_web(@i_c_bookkey)

		
		--  QSI  -- '\\mcdonald\mediachase\unl_images\'
		--  UNP  -- '\\unp-muskie\Images\ '  *** NOTE EXTRA SPACE AT END OF PATH AT UNP

		Select @v_file = @v_filepath + Substring(pathname,5,len(pathname)) 
