...
ALTER proc [dbo].[feed_in_title_info_extra]
AS
...
/* 7-26-04  UPDATE canadian prices only for isbns in the feedin_vista_exclude table, everything in feedin_titles_extra table
is already in the exclude table*/
...
where i.isbn10 = t.isbn and t.isbn in (select isbn from feedin_vista_exclude)
	order by t.isbn
...
/*canada*/
       ...
       /*  end  all updates are NYP only  NO STATUS CHECK*/
	end   /* end bookkey > 0*/
