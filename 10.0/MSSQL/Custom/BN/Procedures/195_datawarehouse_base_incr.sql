PRINT 'STORED PROCEDURE : dbo.datawarehouse_base_incr'
GO

if exists (select * from dbo.sysobjects where id = Object_id('dbo.datawarehouse_base_incr') and (type = 'P' or type = 'RF'))
begin
 drop proc dbo.datawarehouse_base_incr 
end

GO

CREATE  proc dbo.datawarehouse_base_incr @i_titleonly int
AS 

DECLARE @err_msg char(100)
DECLARE @ware_count int
DECLARE @ware_count2 int
DECLARE @ware_total  int

DECLARE @ware_warehousekey int
DECLARE @ware_newhousekey int
DECLARE @ware_activeind int
DECLARE @ware_logkey int
DECLARE @ware_illuspapchgcode int
DECLARE @ware_textpapchgcode int
DECLARE @ware_lastbookkey  int
DECLARE @ware_system_date datetime
DECLARE @ware_company  varchar(20) 

DECLARE @ware_lasttypeofbuild  varchar(30) 
DECLARE @ware_laststarttime  datetime
DECLARE @ware_lastendtime datetime
DECLARE @ware_lastrowsprocessed  int
DECLARE @ware_lasttotalrows int

DECLARE @i_bookkey int
DECLARE @i_printingkey int
DECLARE @i_warestatus int

DECLARE @ware_associationtypecode int
DECLARE @ware_associatedtitles_count int

 SELECT  @ware_warehousekey = max(warehousekey)
    FROM whhistoryinfo 

 if  @ware_warehousekey > 0 
   begin
	  SELECT  @ware_activeind = activerunind 
	    FROM whhistoryinfo
    		where warehousekey = @ware_warehousekey 

  	IF @ware_activeind = 1 
	  begin
	   	select  @ware_count2 = 0
		SELECT @ware_count2 = count(*)
		     FROM wherrorlog
    			 where warehousekey = @ware_warehousekey 
	 	  if @ware_count2 > 0 
		    begin

			SELECT @ware_logkey = max(logkey)
		    	  FROM wherrorlog
				where warehousekey = @ware_warehousekey 
BEGIN tran
   			  INSERT INTO wherrorlog (logkey, warehousekey,errordesc,
	     			 errorseverity, errorfunction,lastuserid, lastmaintdate)
		      VALUES (convert(varchar,@ware_logkey)  ,convert(varchar,@ware_warehousekey),
		      'Status indicates that a Warehouse build already in progress; EXIT INCREMENTAL',
		      'Warning/data error','Stored procedure startup','WARE_STORED_PROC', getdate())	
commit tran
/*7-09-03 print statement so will display in job manager*/
			print      'Status indicates that a Warehouse build already in progress; EXIT INCREMENTAL'
		
		     end
	   	 else
		    begin
     			select @ware_logkey = max(logkey) 
			from wherrorlog
		    	if @ware_logkey is null
			  begin
				select @ware_logkey  = 1
			  end
   		    end
    	END

 end 

DECLARE datawarehouse_base2 INSENSITIVE CURSOR
FOR
/* delete titles/printings from titledeleteaudit table*/

  SELECT bookkey,printingkey
    FROM titledeleteaudit
    WHERE datawarehousedeleted='N'
    ORDER BY  bookkey DESC

FOR READ ONLY


select @ware_count = 1
OPEN datawarehouse_base2

FETCH NEXT FROM datawarehouse_base2
  INTO @i_bookkey, @i_printingkey

select @i_warestatus = @@FETCH_STATUS

 IF @ware_activeind = 0 
  begin
   	
 while (@i_warestatus<>-1 )
   begin
	
    IF (@i_warestatus<>-2)
	  begin
BEGIN tran
	 select @ware_count2 = 0

	 select @ware_count2 = count(*)
		  from whtitleinfo where bookkey=@i_bookkey
 	if  @ware_count2 > 0 
	  begin
		  delete from whtitleinfo where bookkey=@i_bookkey
 	  end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
  		from whauthor where bookkey=@i_bookkey
 	if @ware_count2 > 0 
	  begin
		   delete from whauthor
			   where bookkey=@i_bookkey
	  end
	select  @ware_count2 = 0
	select @ware_count2 = count(*)
 		 from whtitleclass
		   where bookkey=@i_bookkey
 	if @ware_count2 > 0 
	  begin
		  delete from whtitleclass
		   where bookkey=@i_bookkey
	  end
	select  @ware_count2 = 0
 	select @ware_count2 = count(*) 
  		from whtitledates
		  where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	  begin	
		  delete from whtitledates
		   where bookkey=@i_bookkey
	 end

	select  @ware_count2 = 0
 	select @ware_count2 = count(*) 
 		 from whtitlefiles
  		 where bookkey=@i_bookkey
 	if @ware_count2 > 0 
	  begin
  		delete from whtitlefiles
   		where bookkey=@i_bookkey
 	  end

	select  @ware_count2 = 0
 	select @ware_count2 = count(*) 
 		 from whtitlecomments
  		 where bookkey=@i_bookkey
 	if @ware_count2 > 0 
	  begin
   		delete from whtitlecomments
   		where bookkey=@i_bookkey
 	  end

	select  @ware_count2 = 0
	select @ware_count2 = count(*) 
  		from whtitlecomments2
  		 where bookkey=@i_bookkey
 	if @ware_count2 > 0 
	  begin
   		delete from whtitlecomments2
  			 where bookkey=@i_bookkey
 	  end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
 		 from whtitlecomments3
  		 where bookkey=@i_bookkey
 	if @ware_count2 > 0 
	  begin
  		 delete from whtitlecomments3
   		where bookkey=@i_bookkey
	  end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		  from whtitlepersonnel
		   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	  begin
  		delete from whtitlepersonnel
   			where bookkey=@i_bookkey
 	  end

 	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		  from whprintingpersonnel
		   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	  begin
  		delete from whprintingpersonnel
   			where bookkey=@i_bookkey
 	  end

	select  @ware_count2 = 0
	select @ware_count2 = count(*) 
	  from whtitleprevworks
	   where bookkey=@i_bookkey
	if @ware_count2 > 0 
	  begin
	   delete from whtitleprevworks
		   where bookkey=@i_bookkey
	 end

	select  @ware_count2 = 0
	select @ware_count2 = count(*) 
	  from whtitlecustom where bookkey=@i_bookkey

	 if @ware_count2 > 0 
	  begin
		  delete from whtitlecustom
		   where bookkey=@i_bookkey
	  end

	select  @ware_count2 = 0
	select @ware_count2 = count(*) 
	  from whauthorsalestrack where bookkey=@i_bookkey

	 if @ware_count2 > 0 
	  begin
		  delete from whauthorsalestrack 
		   where bookkey=@i_bookkey
	  end

	select  @ware_count2 = 0
	select @ware_count2 = count(*) 
	  from whcompetitivetitles where bookkey=@i_bookkey

	 if @ware_count2 > 0 
	  begin
		  delete from whcompetitivetitles
		   where bookkey=@i_bookkey
	  end
	
	select  @ware_count2 = 0
	select @ware_count2 = count(*) 
	  from whcomparativetitles where bookkey=@i_bookkey

	 if @ware_count2 > 0 
	  begin
		  delete from whcomparativetitles
		   where bookkey=@i_bookkey
	  end

	select  @ware_count2 = 0
 	select @ware_count2 = count(*) 
		  from whprinting
		   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
 	if @ware_count2 > 0 
	  begin
		delete from whprinting
		   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	  end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
	  from whprintingkeydates
	   where bookkey=@i_bookkey
	    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whprintingkeydates
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	   end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		  from whprintingkeydates2
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	    begin
		   delete from whprintingkeydates2
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	   end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		  from whfinalcostest
		   where bookkey=@i_bookkey
		    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whfinalcostest
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		  from whschedule1
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule1
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	   end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		  from whschedule2
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule2
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	   end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		  from whschedule3
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	if @ware_count2 > 0 
	   begin
		  delete from whschedule3
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	   end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		  from whschedule4
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule4
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	   end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule5
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule5	
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule6
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule6	
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule7
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule7	
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule8
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule8	
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule9
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule9	
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule10
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule10	
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		  from whschedule11
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule11
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	   end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		  from whschedule12
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule12
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	   end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		  from whschedule13
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	if @ware_count2 > 0 
	   begin
		  delete from whschedule13
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	   end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		  from whschedule14
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule14
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	   end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule15
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule15	
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule16
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule16	
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule17
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule17	
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule18
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule18	
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule19
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule19	
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule20
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule20	
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule21
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule21	
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule22
			   where bookkey=@i_bookkey
			    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule22	
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	    end

	select  @ware_count2 = 0
	select @ware_count2 = count(*) 
		   from whprintingkeydates
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whprintingkeydates
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	   end

	select  @ware_count2 = 0
	select @ware_count2 = count(*) 
		   from whprintingkeydates2
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	 if @ware_count2 > 0 
	   begin
		   delete from whprintingkeydates2
			   where bookkey=@i_bookkey
				    and printingkey = @i_printingkey
	   end

	select  @ware_count2 = 0
	select @ware_count2 = count(*) 
		   from whtitlepositioning
			   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	   begin
		   delete from whtitlepositioning
			   where bookkey=@i_bookkey
	   end

 /*5-7-02  update datawarehousedeleted indicator to Y*/
 	update titledeleteaudit
     	set datawarehousedeleted='Y'
      	 where bookkey=@i_bookkey
	        and printingkey = @i_printingkey 
commit tran
  end

   FETCH NEXT FROM datawarehouse_base2
  INTO @i_bookkey, @i_printingkey

   select @i_warestatus = @@FETCH_STATUS
 end
end

close datawarehouse_base2 
deallocate datawarehouse_base2


 SELECT @ware_system_date  = getdate()

 select @ware_logkey = count(*)
   from wherrorlog

 if @ware_logkey > 0 
   begin		
  	select @ware_logkey = max(logkey)
   	    from wherrorlog
	select  @ware_logkey = @ware_logkey + 1
   end
 else
   begin
  	select @ware_logkey = 1
   end

 select @ware_warehousekey = max(warehousekey)
   from whhistoryinfo

 if @ware_warehousekey is null
   begin
	select @ware_warehousekey = 0
BEGIN tran
	INSERT INTO wherrorlog (logkey, warehousekey,errordesc,
           errorseverity, errorfunction,lastuserid, lastmaintdate)
	   VALUES (convert(varchar,@ware_logkey)  ,convert(varchar,@ware_newhousekey),
	   'Unable to access whhistoryinfo- select max(warehousekey)',
	  'Warning/data error','Stored procedure startup','WARE_STORED_PROC', @ware_system_date)
commit tran
	end

 if @ware_warehousekey > 0 
  begin
	select  @ware_warehousekey = @ware_warehousekey + 1
	select @ware_newhousekey = @ware_warehousekey 
  end
 else
   begin
  	select @ware_newhousekey = 1
  	select @ware_logkey = @ware_logkey + 1

 	 SELECT  @ware_activeind = COUNT(*) 
	     FROM whhistoryinfo
     			where warehousekey = @ware_warehousekey 
	 IF @ware_activeind > 0  
	  begin
		select  @ware_activeind = 0
   		SELECT @ware_activeind = activerunind
		      FROM whhistoryinfo
			      where warehousekey = @ware_warehousekey 
		if @ware_activeind is null
	 	 begin
			select @ware_activeind = 0   
	 	 end
	  end
  end
  IF @ware_activeind = 0 
     begin
/* insert row into whhistoryinfo for this build*/
BEGIN tran
      insert into whhistoryinfo
  	  (warehousekey,starttime,endtime,typeofbuild,activerunind)
    VALUES (@ware_newhousekey, getdate(),null,'INCREMENTAL',1)

/* delete all errors older than 1 week*/
	  DELETE FROM wherrorlog
	    WHERE lastmaintdate <= (getdate() - 7)
		SELECT @ware_total = count(*)
	     FROM bookwhupdate
commit tran
	 select @ware_count = count(*) 
	   from defaults
		if @ware_count is null 
	  begin
		select @ware_count = 0
	  end
	IF @ware_count <> 1 
	  begin
BEGIN tran
	  	INSERT INTO wherrorlog (logkey,warehousekey,errordesc,
	          errorseverity, errorfunction,lastuserid, lastmaintdate)
		  VALUES (convert(varchar,@ware_logkey) ,convert(varchar,@ware_newhousekey),
		  'Unable to access defaults table',
		  'Warning/data error','Stored procedure startup','WARE_STORED_PROC', @ware_system_date)
commit tran
	  end
	else
	  begin
	 	select @ware_illuspapchgcode = textpaperchgcode,@ware_textpapchgcode = illuspaperchgcode
	  	   from defaults

		if @i_titleonly <> 1 
		   begin
BEGIN tran
			insert into bookwhupdate (bookkey,lastuserid,lastmaintdate)
				SELECT distinct bu.bookkey, 'qsidba', getdate()
	 			   FROM book bu, bookauthor b,  authorpreviousworks a
	  				WHERE b.bookkey=bu.bookkey
						and b.authorkey=a.authorkey
						and a.lastmaintdate > getdate() -1
						   and bu.standardind <> 'Y'
							and b.bookkey not in (select bl.bookkey from booklock bl)
					      and b.bookkey not in (select bl2.bookkey from bookwhupdate bl2)
commit tran
			end
		 	select @ware_count  = 0
		DECLARE datawarehouse_base INSENSITIVE CURSOR
		  FOR
			(SELECT bu.bookkey
			    FROM bookwhupdate b,book bu
				  WHERE bu.bookkey=b.bookkey
					   and bu.standardind <> 'Y'
				and b.bookkey not in (select bl.bookkey from booklock bl)
			union
			SELECT distinct bu.bookkey
			    FROM book bu, bookauthor b,  authorpreviousworks a
				  WHERE b.bookkey=bu.bookkey
					and b.authorkey=a.authorkey
					and a.lastmaintdate> convert(varchar,getdate(),101)
				   and bu.standardind <> 'Y'
					and b.bookkey not in (select bl.bookkey from booklock bl))
		     ORDER BY bu.bookkey DESC
  	  FOR READ ONLY
	
		OPEN datawarehouse_base 

		FETCH NEXT FROM datawarehouse_base 
			INTO @i_bookkey
			   select @i_warestatus = @@FETCH_STATUS
			while (@i_warestatus<>-1 )
			   begin

		    IF (@i_warestatus<>-2)
		      begin
BEGIN tran
			 select  @ware_count2 = 0
			 select @ware_count2 = count(*) 
				from whtitleinfo where bookkey=@i_bookkey
		 	 if @ware_count2 > 0 
			   begin
			  	delete from whtitleinfo
			  	  where bookkey=@i_bookkey
			    end

			 select  @ware_count2 = 0
			 	select @ware_count2 = count(*) 
			   		from whauthor where bookkey=@i_bookkey
		  	if @ware_count2 > 0 
			  begin
    				delete from whauthor
			    		where bookkey=@i_bookkey
		   	  end
	 select  @ware_count2 = 0
 	 select @ware_count2 = count(*) 
 		  from whtitleclass
 		   where bookkey=@i_bookkey
 	if @ware_count2 > 0 
	  begin
   		delete from whtitleclass
   			 where bookkey=@i_bookkey
 	 end
	 select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
  		 from whtitledates
   			 where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
  		 delete from whtitledates
   			 where bookkey=@i_bookkey
  	  end

 	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
  		 from whtitlefiles
   			 where bookkey=@i_bookkey
 	 if @ware_count2 > 0 
	  begin
  		 delete from whtitlefiles
  		  where bookkey=@i_bookkey
        end

 	select  @ware_count2 = 0
 	 select @ware_count2 = count(*) 
   		from whtitlecomments
    			where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
    		delete from whtitlecomments
    			where bookkey=@i_bookkey
  	 end

 	select  @ware_count2 = 0
 	select @ware_count2 = count(*) 
   		from whtitlecomments2
  			 where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
    		delete from whtitlecomments2
    			where bookkey=@i_bookkey
 	  end

 	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
   		from whtitlecomments3
   			 where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
  		  delete from whtitlecomments3
  			  where bookkey=@i_bookkey
 	  end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		  from whprintingpersonnel
		   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	  begin
  		delete from whprintingpersonnel
   			where bookkey=@i_bookkey
 	  end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		  from whtitlepersonnel
		   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	  begin
  		delete from whtitlepersonnel
   			where bookkey=@i_bookkey
 	  end
 	select  @ware_count2 = 0
 	select @ware_count2 = count(*) 
   		from whtitleprevworks
    			where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
    		delete from whtitleprevworks
    			where bookkey=@i_bookkey
  	  end
	select  @ware_count2 = 0
	select @ware_count2 = count(*) 
	  from whauthorsalestrack where bookkey=@i_bookkey

	 if @ware_count2 > 0 
	  begin
		  delete from whauthorsalestrack 
		   where bookkey=@i_bookkey
	  end

	select  @ware_count2 = 0
	select @ware_count2 = count(*) 
	  from whcompetitivetitles where bookkey=@i_bookkey

	 if @ware_count2 > 0 
	  begin
		  delete from whcompetitivetitles
		   where bookkey=@i_bookkey
	  end
	
	select  @ware_count2 = 0
	select @ware_count2 = count(*) 
	  from whcomparativetitles where bookkey=@i_bookkey

	 if @ware_count2 > 0 
	  begin
		  delete from whcomparativetitles
		   where bookkey=@i_bookkey
	  end

 	select  @ware_count2 = 0
 	select @ware_count2 = count(*) 
  		 from whprinting
   			 where bookkey=@i_bookkey
 	 if @ware_count2 > 0 
	  begin
    		delete from whprinting
    			where bookkey=@i_bookkey
  	  end

 	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
   		from whprintingkeydates
 		   where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
    		delete from whprintingkeydates
   		 where bookkey=@i_bookkey
 	  end

	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
   		from whprintingkeydates2
    			where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	   begin
    		delete from whprintingkeydates2
    			where bookkey=@i_bookkey
  	   end

	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
   		from whtitlecustom
    			where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
    		delete from whtitlecustom
   			 where bookkey=@i_bookkey
 	  end

 	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
   		from whfinalcostest
   			 where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
    		delete from whfinalcostest
    			where bookkey=@i_bookkey
  	  end

 	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
   		from whschedule1
    			where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
    		delete from whschedule1
    			where bookkey=@i_bookkey
  	  end

 	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
   		from whschedule2
    			where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
    		delete from whschedule2
    			where bookkey=@i_bookkey
 	  end

 	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
   		from whschedule3
   			 where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
  		 delete from whschedule3
    			where bookkey=@i_bookkey
  	  end

 	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
   		from whschedule4
    			where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
    		delete from whschedule4
    			where bookkey=@i_bookkey
  	  end

 	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
    		from whschedule5
    			where bookkey=@i_bookkey
 	 if @ware_count2 > 0 
	  begin
    		delete from whschedule5
    			where bookkey=@i_bookkey
  	   end
select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule6
			   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule6	
			   where bookkey=@i_bookkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule7
			   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule7	
			   where bookkey=@i_bookkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule8
			   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule8	
			   where bookkey=@i_bookkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule9
			   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule9	
			   where bookkey=@i_bookkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule10
			   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule10	
			   where bookkey=@i_bookkey
	    end

	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
   		from whschedule11
    			where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
    		delete from whschedule11
    			where bookkey=@i_bookkey
  	  end

 	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
   		from whschedule12
    			where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
    		delete from whschedule12
    			where bookkey=@i_bookkey
 	  end

 	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
   		from whschedule13
   			 where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
  		 delete from whschedule13
    			where bookkey=@i_bookkey
  	  end

 	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
   		from whschedule14
    			where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	  begin
    		delete from whschedule14
    			where bookkey=@i_bookkey
  	  end

 	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
    		from whschedule15
    			where bookkey=@i_bookkey
 	 if @ware_count2 > 0 
	  begin
    		delete from whschedule15
    			where bookkey=@i_bookkey
  	   end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule16
			   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule16	
			   where bookkey=@i_bookkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule17
			   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule17	
			   where bookkey=@i_bookkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule18
			   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule18	
			   where bookkey=@i_bookkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule19
			   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule19	
			   where bookkey=@i_bookkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule20
			   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule20	
			   where bookkey=@i_bookkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule21
			   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule21	
			   where bookkey=@i_bookkey
	    end
	select  @ware_count2 = 0
	 select @ware_count2 = count(*) 
		   from whschedule22
			   where bookkey=@i_bookkey
	 if @ware_count2 > 0 
	   begin
		   delete from whschedule22	
			   where bookkey=@i_bookkey
	    end
 	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
    		from whprintingkeydates
    			where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	   begin
		 delete from whprintingkeydates
    			where bookkey=@i_bookkey
  	   end
	select  @ware_count2 = 0
  	select @ware_count2 = count(*) 
    		from whtitlepositioning
    			where bookkey=@i_bookkey
  	if @ware_count2 > 0 
	   begin
		 delete from whtitlepositioning
    			where bookkey=@i_bookkey
  	   end
commit tran

	  exec datawarehouse_author @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date
	  exec datawarehouse_bookinfo @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date /*whtitleinfo and whtitleclass*/
	  exec datawarehouse_bisac @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date /*whtitleclass*/
	  exec datawarehouse_category @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date /*whtitleclass*/
	  exec datawarehouse_audience @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date /*whtitleclass*/
	  exec datawarehouse_orgentry @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date  /*whtitleclass*/
	  exec datawarehouse_bookprice @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date /*whtitleinfo*/
	  exec datawarehouse_bookdates @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date /*whtitledates*/
	  exec datawarehouse_bookfiles @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date /*whtitlefiles*/
	  exec datawarehouse_bookrole @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date  /*whtitlepersonnel whprintingpersonnel*/
	  exec datawarehouse_bookcomment @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date /*whtitlecomments*/
	  exec datawarehouse_prevauth @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date /*whtitleprevworks*/
	  exec datawarehouse_printing @i_bookkey,@ware_illuspapchgcode,@ware_textpapchgcode,
	  @ware_logkey,@ware_newhousekey,@ware_system_date /*whprinting*/

	  exec datawarehouse_bookcustom @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date /*whtitlecustom*/
	  exec datawarehouse_bookmisc @i_bookkey, 'WARE_STORED_PROC', @ware_system_date

	  exec datawarehouse_bookqtybreakdown @i_bookkey, @ware_system_date
 	  exec datawarehouse_whtitlecatalog @i_bookkey, @ware_logkey,@ware_newhousekey, @ware_system_date
      
	select @ware_associatedtitles_count = 0
	 select @ware_associationtypecode = 3   /* whauthorsalestrack */
	  exec datawarehouse_titlepositioning @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date,@ware_associationtypecode
			
	  select @ware_associationtypecode = 1     /* whcompetitivetitles */
	  exec datawarehouse_titlepositioning @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date,@ware_associationtypecode
		  
	  select @ware_associationtypecode = 2     /* whcomparativetitles */
	  exec datawarehouse_titlepositioning @i_bookkey,@ware_logkey,@ware_newhousekey,@ware_system_date,@ware_associationtypecode

	  exec dw_whtitlepositioning @i_bookkey, @ware_logkey,@ware_newhousekey, @ware_system_date  

	select @ware_lastbookkey = @i_bookkey

	 select  @ware_count2 = 0
	   select @ware_count2 = count(*)
     		from bookwhupdate
	    	  where bookkey = @i_bookkey
	  if @ware_count2 > 0  
	    begin
BEGIN tran
		   delete from bookwhupdate
    			where bookkey = @i_bookkey
commit tran
	    end

	    select @ware_count  = @ware_count  + 1
	  end  /*<>*/
	FETCH NEXT FROM datawarehouse_base 
	INTO @i_bookkey
 	select @i_warestatus = @@FETCH_STATUS
  end  /*end while*/

close datawarehouse_base
deallocate datawarehouse_base

/******    3-9-04  add estimates incremental which also runs  P and L incremental ******/
/*exec  datawarehouse_estimate_incr @ware_logkey,@ware_newhousekey,@ware_system_date*/


 if @i_titleonly  <> 1   /* 8-27-02 only do titles if this =1 otherwise run all */
  begin
BEGIN tran
	delete from whest
	delete from whestcost
commit tran


/*Processing Estimates*/
	/* select @ware_company = upper(orgleveldesc)*/
	/*	   from orglevel*/
	/*	    where orglevelkey= 1 */
	/*if @ware_company <> 'CONSUMER' */
	/*   begin */

/*not sure exclude ss since inside Script account for SS */
	/*	exec datawarehouse_estimate @ware_company,@ware_logkey,@ware_newhousekey,@ware_system_date*/  /*whest/whestcost*/
	/*  end */

/* Processing catalog*/
BEGIN tran
	 delete from whcatalog
	 delete from whcatalogtitle
commit tran
	 exec datawarehouse_catalog @ware_logkey,@ware_newhousekey,@ware_system_date  /*whcatalog*/
	 exec datawarehouse_catalogtitle @ware_logkey,@ware_newhousekey,@ware_system_date  /*whcatalogtitle*/
end /*i_title<> 1*/

 /* do only if build complete*/
BEGIN tran
	 update whhistoryinfo
	  set endtime = getdate(),
	   activerunind = 0,
	   totalrows =  @ware_total,
	   rowsprocessed = @ware_count,
	   lastuserid  ='WARE_STORED_PROC',
	   lastmaintdate = @ware_system_date,
	   lastbookkey =  @ware_lastbookkey
	    where warehousekey=@ware_newhousekey
commit tran
end /*<>1*/
end  /* activeind= 0*/

/**  not sure how to do an exception as yet EXCEPTION
  when OTHERS begin
 update whhistoryinfo
  set endtime = sysdate,
   activerunind = 0,
   totalrows =  @ware_total,
   rowsprocessed = @ware_count,
   lastuserid  ='WARE_STORED_PROC',
   lastmaintdate = @ware_system_date,
   lastbookkey =  @ware_lastbookkey
    where warehousekey=@ware_newhousekey
 commit
 INSERT INTO wherrorlog (logkey,warehousekey,errordesc,
          errorseverity, errorfunction,lastuserid, lastmaintdate)
  VALUES (to_char(@ware_logkey) ,to_char(@ware_newhousekey),
  'Unable to access table',
  ('build stop at bookkey ' || to_char(@ware_lastbookkey)),
  'Stored procedure startup','WARE_STORED_PROC', sysdate)
 COMMIT
**/


GO
