/********************************************************************
    ...
    Mediachase ECF 4.0 Release 4.1.1 to Release 4.1.2 Upgrade Script
*********************************************************************/
...
--## Schema Patch ##
exec sp_ExecuteSQL N'ALTER TRIGGER [dbo].[PageTemplateLanguageDeleteTrigger] ON [dbo].[PageTemplateLanguage] 
FOR DELETE 
AS

declare @sqlStmt nvarchar(1000)
declare @tblName nvarchar(255)
declare @pagetemplatelanguage_id int

declare @page_type as int
set @page_type=4

declare pageLangCursor cursor for 
	select P.[PageTemplateLanguageId], MC.[TableName] from deleted P inner join [MetaClass] MC on P.[MetaClassId]=MC.[MetaClassId]
open pageLangCursor
fetch next from pageLangCursor into @pagetemplatelanguage_id, @tblName
while @@fetch_status=0 begin
	delete from [Categorization] where [ObjectTypeId]=@page_type and [ObjectId]=@pagetemplatelanguage_id
	delete from [ObjectLanguage] where [ObjectTypeId]=@page_type and [ObjectId]=@pagetemplatelanguage_id
	delete from [ObjectAccess] where [ObjectTypeId]=@page_type and [ObjectId]=@pagetemplatelanguage_id
	set @sqlStmt = N''mdpsp_avto_''+@tblName+N''_Delete ''+CAST(@pagetemplatelanguage_id as nvarchar(10))
	exec (@sqlStmt)
	if @@error!=0 goto errPageLangDelete
	fetch next from pageLangCursor into @pagetemplatelanguage_id, @tblName
end

errPageLangDelete:
close pageLangCursor
deallocate pageLangCursor'
