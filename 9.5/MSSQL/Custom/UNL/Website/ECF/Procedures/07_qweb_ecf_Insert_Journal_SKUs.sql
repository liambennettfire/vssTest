...
------------------------------------------------------
		...
		Select bookkey, finalprice, UNL.dbo.get_gentables_desc(306, pricetypecode, 'D') as pricetype, 
		Cast(bookkey as varchar) + '-' + CAST(pricetypecode as varchar) + '-' + CAST(currencytypecode as varchar) as pss_sku_code,
		pricetypecode, activeind
		from UNL..bookprice
		...
		order by pricetypecode
			...
			FETCH NEXT FROM c_qweb_journalpricetypes 
				INTO @i_bookkey, @m_usretailprice, @v_pricetype, @v_pss_sku_code, @ptc, @activeind
					...
					IF (@i_titlefetchstatus <>-2) 
						...
						--17 institution, 18 individual single issue price type
							...
							If @i_mediatypesubcode <> 1 --journal single issues
								...
								--if inactive in TMM set visible to 0
								If 	@activeind = 0 
									BEGIN
										SET @isVisible = 0
										PRINT 'Pricetype ' + @v_pricetype + ' inactive for bookkey ' + Cast(@i_bookkey as varchar) 	
									END
								ELSE --active but if publishtoweb = 0 set visible to 0
									If 	@i_publishtowebind = 0
										SET @isVisible = 0
									else
										SET @isVisible = 1									

						
								Select @v_title = UNL.dbo.qweb_get_Title(@i_bookkey,'f') + ' (' + @v_pricetype + ')'
