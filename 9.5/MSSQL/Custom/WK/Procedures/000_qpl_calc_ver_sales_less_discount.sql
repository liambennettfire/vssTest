if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_ver_sales_less_discount') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_ver_sales_less_discount
GO

CREATE PROCEDURE qpl_calc_ver_sales_less_discount (  
  @i_projectkey INT,
  @i_plstage    INT,
  @i_plversion	INT,
...
/******************************************************************************************
**  Name: qpl_calc_ver_sales_less_discount
...
BEGIN
  ...
  BEGIN
    ...
    SET @v_channel_amt = @v_format_price * @v_sum_grossunits * (1 - @v_discount_percent / 100)
    SET @v_total_amt = @v_total_amt + @v_channel_amt
  ...
  SET @o_result = @v_total_amt
   
END
go

GRANT EXEC ON dbo.qpl_calc_ver_sales_less_discount TO PUBLIC
GO
