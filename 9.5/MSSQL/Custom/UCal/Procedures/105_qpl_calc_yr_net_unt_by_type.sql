if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_yr_net_unt_by_type') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_yr_net_unt_by_type
GO

CREATE PROCEDURE dbo.qpl_calc_yr_net_unt_by_type (  
  @i_projectkey INT,
  @i_plstage    INT,
  @i_plversion	INT,
  @i_yearcode   INT,
...
/******************************************************************************************
**  Name: qpl_calc_yr_net_unt_by_type
**  Desc: Generic Routine that allows the client to get net year units by "type".  This type is 
...
DECLARE
  @v_net_units  INT,
	...
	SET @v_whereclause = 'u.taqversionsaleskey = c.taqversionsaleskey AND
      ...
      c.taqprojectkey = ' +  convert(varchar,@i_projectkey)  + ' AND c.plstagecode = ' + convert(varchar,@i_plstage) 
		+  ' AND c.taqversionkey =   ' +  convert(varchar,@i_plversion)  
		+ ' AND u.yearcode = ' + CONVERT(varchar, @i_yearcode)
        +  ' AND (' + @v_built_clause + ')'
...
END
GO
GRANT EXEC ON dbo.qpl_calc_yr_net_unt_by_type TO PUBLIC
GO
