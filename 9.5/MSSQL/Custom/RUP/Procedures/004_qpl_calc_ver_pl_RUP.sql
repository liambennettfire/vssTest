if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_ver_pl_RUP') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_ver_pl_RUP
GO

CREATE PROCEDURE qpl_calc_ver_pl_RUP (  
  @i_projectkey INT,
  @i_plstage    INT,
  @i_plversion  INT,
...
/******************************************************************************************
**  Name: qpl_calc_ver_pl_RUP
**  Desc: P&L - Version/Profit-Loss for Rutgers.
...
BEGIN
  ...
  -- Version - Subsidy
  EXEC qpl_calc_version @i_projectkey, @i_plstage, @i_plversion, 'MISCINC', 'PRODSUB', 1, @v_subsidy OUTPUT

  IF @v_subsidy IS NULL
    SET @v_subsidy = 0

  -- Version - Total Expense
  ...
  -- Version - Total Operating Expense
  EXEC qpl_calc_ver_tot_op_exp_RUP @i_projectkey, @i_plstage, @i_plversion, @v_total_op_expense OUTPUT
...
GRANT EXEC ON qpl_calc_ver_pl_RUP TO PUBLIC
GO
