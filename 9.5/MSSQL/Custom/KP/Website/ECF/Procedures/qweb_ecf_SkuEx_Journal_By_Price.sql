...
------------------------------------------------------
	...
	SET @Journals_Single_Issues_Available = (CASE Upper(cbd.dbo.[get_Tab_Journals_Single_Issues_Available?](@i_bookkey))
					WHEN 'YES' Then 1
					ELSE 0 END)
	...
	select  @i_titlefetchstatus  = @@FETCH_STATUS
	 ...
	 while (@i_titlefetchstatus >-1 )
		...
		begin
			...
			if not exists (Select * from cbd..bookdetail where bookkey = @i_bookkey and mediatypecode = 6 and mediatypesubcode = 1)
				begin
				...
				Select @v_ISBN = cbd.dbo.qweb_get_ISBN(@i_bookkey,'13')
				Select @v_EAN = cbd.dbo.qweb_get_ISBN(@i_bookkey,'16')
				Select @v_Display = ''
				Select @v_Format = cbd.dbo.qweb_get_Format(@i_bookkey,'D')
				...
				Select @v_Journals_Brief_Description = commenttext FROM  cbd..bookcomments WHERE commenttypecode = 3 AND commenttypesubcode = 7 AND bookkey = @i_bookkey
				...
				Select @v_Journal_ISSN = itemnumber from cbd..isbn i where i.bookkey = @i_bookkey
				...
				Select @v_Awards = cbd.dbo.qweb_ecf_get_sku_awards(@i_bookkey)
				Select @v_journalsubscriptionlocation = alternatedesc1 from cbd..gentables where datacode = @i_pricetypecode and tableid = 306
				Select @v_journalsubsrcriptionlength = alternatedesc2 from cbd..gentables where datacode = @i_pricetypecode and tableid = 306
			

				--if journal special issue use journal bookkey for the following fields
				If @Journal_IssueInd = 1
					Begin
							Select @v_fullauthordisplayname = cbd.dbo.[qweb_get_Author](@masterJournal_bookkey,0,0,'F') + ' ' + 
								  cbd.dbo.[qweb_get_Author](@masterJournal_bookkey,0,0,'M') + ' ' + 
								  cbd.dbo.[qweb_get_Author](@masterJournal_bookkey,0,0,'L')
							...
							Select @v_Journal_ISSN = itemnumber from cbd..isbn i where i.bookkey = @masterJournal_bookkey
							Select @v_authorbylineprepro = commenthtmllite from cbd..bookcomments where commenttypecode = 3 and commenttypesubcode = 73 and bookkey = @masterJournal_bookkey


					End
				Else
					Begin
							Select @v_fullauthordisplayname = cbd.dbo.[qweb_get_Author](@i_bookkey,0,0,'F') + ' ' + 
