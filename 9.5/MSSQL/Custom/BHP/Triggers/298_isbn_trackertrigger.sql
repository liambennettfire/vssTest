IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.isbn_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.isbn_trackertrigger'
	DROP TRIGGER dbo.isbn_trackertrigger
	PRINT 'Creating TRIGGER dbo.isbn_trackertrigger...'
END

GO

CREATE TRIGGER isbn_trackertrigger
ON isbn
FOR INSERT, UPDATE
AS
