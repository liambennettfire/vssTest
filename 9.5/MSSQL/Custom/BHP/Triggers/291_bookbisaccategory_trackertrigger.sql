IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.bookbisaccategory_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.bookbisaccategory_trackertrigger'
	DROP TRIGGER dbo.bookbisaccategory_trackertrigger
	PRINT 'Creating TRIGGER dbo.bookbisaccategory_trackertrigger...'
END

GO

CREATE TRIGGER bookbisaccategory_trackertrigger
ON bookbisaccategory
FOR INSERT, UPDATE
AS
