IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[qpl_calc_stg_tot_exp_IP]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[qpl_calc_stg_tot_exp_IP]
GO




CREATE PROCEDURE qpl_calc_stg_tot_exp_IP (  
...
BEGIN
  ...
  -- Stage Allocated marketing
   ...
   EXEC qpl_calc_stage @i_projectkey, @i_plstage, 'MISCExp', 'MktgAllo', 0, @v_Allocated_Marketing OUTPUT
  ...
  EXEC qpl_calc_stage @i_projectkey, @i_plstage, 'MISCExp', 'EdOp', 0, @v_Editorial_Operations OUTPUT
  ...
  EXEC qpl_calc_stage @i_projectkey, @i_plstage, 'MISCExp', 'MktgDisc', 0, @v_Discretioanry_Marketing OUTPUT
  ...
  EXEC qpl_calc_stage @i_projectkey, @i_plstage, 'MISCExp', 'MktgOp', 0, @v_Marketing_Operations OUTPUT
  ...
  -- Production Operations
   ...
   EXEC qpl_calc_stage @i_projectkey, @i_plstage, 'MISCExp', 'ProdOp', 0, @v_Production_Operations OUTPUT
  ...
  IF @v_Production_Operations IS NULL
	...
	EXEC qpl_calc_stage @i_projectkey, @i_plstage, 'MISCExp', 'PresidentO', 0, @v_President_Operations OUTPUT
  ...
  IF @v_President_Operations IS NULL
	...
	EXEC qpl_calc_stage @i_projectkey, @i_plstage, 'MISCExp', 'SharedExp', 0, @v_Shared_Expenses OUTPUT
  ...
  IF @v_Shared_Expenses IS NULL
	...
	--Fullfillment Costs
	EXEC qpl_calc_stg_fulfill @i_projectkey, @i_plstage, @v_Fullfillment_Costs OUTPUT
...
END
Go
Grant all on qpl_calc_stg_tot_exp_IP to public