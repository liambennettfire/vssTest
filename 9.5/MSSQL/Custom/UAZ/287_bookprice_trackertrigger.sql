IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.bookprice_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.bookprice_trackertrigger'
	DROP TRIGGER dbo.bookprice_trackertrigger
	PRINT 'Creating TRIGGER dbo.bookprice_trackertrigger...'
END

GO

CREATE TRIGGER bookprice_trackertrigger
ON bookprice
FOR INSERT, UPDATE
AS
