IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.filelocation_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.filelocation_trackertrigger'
	DROP TRIGGER dbo.filelocation_trackertrigger
	PRINT 'Creating TRIGGER dbo.filelocation_trackertrigger...'
END

GO

CREATE TRIGGER filelocation_trackertrigger
ON filelocation
FOR INSERT, UPDATE
AS
