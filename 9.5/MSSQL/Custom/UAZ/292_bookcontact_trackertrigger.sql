IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.bookcontact_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.bookcontact_trackertrigger'
	DROP TRIGGER dbo.bookcontact_trackertrigger
	PRINT 'Creating TRIGGER dbo.bookcontact_trackertrigger...'
END

GO

CREATE TRIGGER bookcontact_trackertrigger
ON bookcontact
FOR INSERT, UPDATE
AS
