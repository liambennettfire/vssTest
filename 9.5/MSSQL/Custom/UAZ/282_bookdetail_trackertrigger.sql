IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.bookdetail_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.bookdetail_trackertrigger'
	DROP TRIGGER dbo.bookdetail_trackertrigger
	PRINT 'Creating TRIGGER dbo.bookdetail_trackertrigger...'
END

GO

CREATE TRIGGER bookdetail_trackertrigger
ON bookdetail
FOR INSERT, UPDATE
AS
