IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.bookorgentry_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.bookorgentry_trackertrigger'
	DROP TRIGGER dbo.bookorgentry_trackertrigger
	PRINT 'Creating TRIGGER dbo.bookorgentry_trackertrigger...'
END

GO

CREATE TRIGGER bookorgentry_trackertrigger
ON bookorgentry
FOR INSERT, UPDATE
AS
