IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.bookmisc_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.bookmisc_trackertrigger'
	DROP TRIGGER dbo.bookmisc_trackertrigger
	PRINT 'Creating TRIGGER dbo.bookmisc_trackertrigger...'
END

GO

CREATE TRIGGER bookmisc_trackertrigger
ON bookmisc
FOR INSERT, UPDATE
AS
