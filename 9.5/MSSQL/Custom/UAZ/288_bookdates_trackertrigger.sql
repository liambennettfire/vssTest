IF EXISTS (
	SELECT
		*
	FROM dbo.sysobjects
	WHERE id = OBJECT_ID('dbo.bookdates_trackertrigger')
	AND (type = 'P'
	OR type = 'TR'))
BEGIN
	PRINT 'DROP TRIGGER dbo.bookdates_trackertrigger'
	DROP TRIGGER dbo.bookdates_trackertrigger
	PRINT 'Creating TRIGGER dbo.bookdates_trackertrigger...'
END

GO

CREATE TRIGGER bookdates_trackertrigger
ON bookdates
FOR INSERT, UPDATE
AS
