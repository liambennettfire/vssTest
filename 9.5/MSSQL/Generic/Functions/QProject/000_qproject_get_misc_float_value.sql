if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qproject_get_misc_float_value') and xtype in (N'FN', N'IF', N'TF'))
  drop function dbo.qproject_get_misc_float_value
GO

CREATE FUNCTION dbo.qproject_get_misc_float_value
(
...
/*******************************************************************************************************
**  Name: qproject_get_misc_float_value
**  Desc: This function returns the miscellaneous float value for specific Project as a varchar.
**
**  Auth: Uday A. Khisty
**  Date: April 10 2015
*******************************************************************************************************/
...
BEGIN 
  DECLARE
    ...
    @v_orgentrykey INT,
    @v_floatvalue_return FLOAT
    
  SET @v_floatvalue_return = NULL  
  ...
  SELECT @v_orgentrykey = orgentrykey FROM taqprojectorgentry WHERE taqprojectkey = @i_projectkey   
  ...
  IF EXISTS (SELECT * FROM taqprojectmisc WHERE taqprojectkey = @i_projectkey AND misckey = @i_misckey) BEGIN  
	  ...
	  IF @v_misctype = 2	--Float
		SET @v_floatvalue_return = @v_floatvalue
  END  
  ELSE BEGIN
  ...
  RETURN @v_floatvalue_return
  
END
GO

GRANT EXEC ON dbo.qproject_get_misc_float_value TO public
GO
