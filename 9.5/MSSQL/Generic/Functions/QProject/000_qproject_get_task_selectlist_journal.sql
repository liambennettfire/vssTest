/******************************************************************************
**  Name: qproject_get_task_selectlist_journal
...
**    --------    --------    -------------------------------------------
**    01/21/2016   UK		   Case 32526
*******************************************************************************/

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].qproject_get_task_selectlist_journal') and xtype in (N'FN', N'IF', N'TF'))
drop function [dbo].qproject_get_task_selectlist_journal
GO
...
CREATE FUNCTION qproject_get_task_selectlist_journal(
...
BEGIN
  ...
  -- add this journal info        
  SELECT @v_itemdesc = c.projecttitle, @v_itemtypecode = c.searchitemcode, 
         @v_usageclasscode = COALESCE(c.usageclasscode,0),
         @v_projectkey = projectkey, @v_contactkey = 0, 
         @v_bookkey = 0, @v_printingkey = 0, @v_usageclassdesc = usageclasscodedesc, @v_projectstatus = projectstatus
    FROM coreprojectinfo c
   WHERE c.projectkey = @i_projectkey AND c.searchitemcode = 6 

  SELECT @error_var = @@ERROR, @rowcount_var = @@ROWCOUNT
  IF @error_var = 0 and @rowcount_var > 0 BEGIN
    INSERT INTO @selectionlist (projectkey,contactkey,bookkey,printingkey,itemtypecode,usageclasscode,itemdesc,usageclassdesc, projectstatus)
    VALUES (@v_projectkey,@v_contactkey,@v_bookkey,@v_printingkey,@v_itemtypecode,@v_usageclasscode,@v_itemdesc,@v_usageclassdesc, @v_projectstatus)
  END 
  ...
  IF (@i_related_projects > 0) BEGIN
    ...
    DECLARE project_cur CURSOR fast_forward FOR
      SELECT taqprojectkey2 projectkey,
             ...
             r.keyind, COALESCE(r.sortorder,0) sortorder, usageclasscodedesc, c.projectstatus
    ...
    FETCH from project_cur INTO @v_projectkey,@v_contactkey,@v_bookkey,@v_printingkey,@v_itemdesc,@v_itemtypecode,
      @v_usageclasscode,@v_keyind,@v_sortorder,@v_usageclassdesc, @v_projectstatus
  ...
  IF (@i_related_contacts > 0) BEGIN
   ...
   ORDER BY COALESCE(pc.sortorder, 0)
    ...
    FETCH from contact_cur INTO @v_itemdesc,@v_itemtypecode,@v_usageclasscode,@v_projectkey,
    ...
    WHILE @@fetch_status = 0 BEGIN
      ...
      FETCH NEXT from contact_cur INTO @v_itemdesc,@v_itemtypecode,@v_usageclasscode,@v_projectkey,
        @v_contactkey,@v_bookkey,@v_printingkey,@v_usageclassdesc
    END
  ...
  IF (@i_related_titles > 0) BEGIN
    INSERT INTO @selectionlist (projectkey,contactkey,bookkey,printingkey,itemtypecode,usageclasscode,itemdesc,usageclassdesc)
    SELECT 0, 0, c.bookkey, c.printingkey, c.itemtypecode, c.usageclasscode, c.title, c.formatname
