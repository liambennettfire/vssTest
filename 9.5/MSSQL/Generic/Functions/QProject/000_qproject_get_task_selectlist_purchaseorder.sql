/******************************************************************************
**  Name: qproject_get_task_selectlist_purchaseorder
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].qproject_get_task_selectlist_purchaseorder') and xtype in (N'FN', N'IF', N'TF'))
drop function [dbo].qproject_get_task_selectlist_purchaseorder
GO
...
CREATE FUNCTION qproject_get_task_selectlist_purchaseorder(
...
BEGIN
  ...
  IF (@i_related_projects > 0) BEGIN
    ...
    DECLARE project_cur CURSOR fast_forward FOR
      SELECT relatedprojectkey projectkey,
             ...
             r.keyind, COALESCE(r.sortorder,0) sortorder, c.usageclasscodedesc, c.projectstatus
  ...
  IF (@i_related_titles > 0) BEGIN
    ...
    SELECT 0, 0, c.bookkey, c.printingkey, c.itemtypecode, c.usageclasscode, c.title, c.formatname
      FROM contractstitlesview t, coretitleinfo c
     WHERE t.bookkey = c.bookkey
       AND t.printingkey = c.printingkey
       AND t.contractprojectkey = @i_projectkey 
       AND t.bookkey > 0
  END
