...
if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qtitle_get_edition') and xtype in (N'FN', N'IF', N'TF'))
  drop function dbo.qtitle_get_edition
GO

CREATE FUNCTION [dbo].[qtitle_get_edition]
		(@i_bookkey	INT,
		@v_column	VARCHAR(1))

RETURNS VARCHAR(255)

/*	The purpose of the qtitle_get_edition function is to return a specific description column from gentables for a Edition
...
Grant All on dbo.qtitle_get_edition to Public
go