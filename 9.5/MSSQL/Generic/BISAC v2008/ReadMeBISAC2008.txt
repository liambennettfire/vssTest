KB 1/13/2008
Process Order for Bisac Code Update v2008:
...
1.  SQL files to load seed data
	...
	temp_gt_bisaccodes_2008.sql    (load temporary table with all bisac categories and prefixes)
		- creates and loads table temp_gt_bisaccodes_2008
	temp_sgt_bisaccodes_2008.sql   (load temporary table with all bisac codes and descriptions)
		- creates and loads table temp_sgt_bisaccodes_2008
	temp_inactivecodes_2008.sql load table with inactive bisac codes
      - creates and loads table temp_inactivecodes_2008

2.  SQL file to update datadesc for existing datacodes:  updexisting_2008.sql
	A)Compares the datadesc on existing rows between gentable 339 and the category desc on the temporary 
          table temp_gt_bisaccodes_2008 and updates the datadesc on the gentable from the one on the temporary
	...
	B)Compares the datadesc on existing rows between subgentable 339 and the category desc on the temporary 
          table temp_sgt_bisaccodes_2008 and updates the datadesc on the subgentable from the one on the temporary
...
3.  The following are stored procedures to insert missing data into gentable and subgentable 339. This looks 
	 ...
	 subgentable and inserts these rows into the gentable/subgentable.
	  ...
	  insertmissing_gt_2008.sql    (gentable)
     insertmissing_sgt_2008.sql	  (subgentable)

4.   This is a SQL file to update datadesc on subgentable 339 to remove leading and trailing 
     spaces for '/' in the datadesc (added during the execution of the stored procedures): code_upd_2008.sql


5.   setinactive_2008.sql
