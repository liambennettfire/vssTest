...
IF EXISTS (SELECT * FROM sysobjects WHERE type = 'TR' AND name = 'estversion_maintainwhs_del')
	DROP TRIGGER estversion_maintainwhs_del
GO

CREATE TRIGGER estversion_maintainwhs_del ON estversion
FOR DELETE AS 
...
IF (@@error != 0)
  BEGIN
	...
	select @v_err_msg = 'Could not select from estversion table (trigger).'
	print @v_err_msg
  END
ELSE
  BEGIN
