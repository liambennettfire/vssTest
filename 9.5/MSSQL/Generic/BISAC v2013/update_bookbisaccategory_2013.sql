...
DECLARE replacement_cursor CURSOR FOR
	SELECT code,replacementcode 
      FROM temp_replacement_codes_2013 
...
BEGIN
	...
	BEGIN
		...
		SELECT @v_count = count(*) FROM subgentables WHERE tableid = 339 AND bisacdatacode = @v_code

		IF @v_count = 1 BEGIN
			SELECT @v_datacode = datacode, @v_datasubcode = datasubcode,@v_deletestatus = deletestatus
              FROM subgentables WHERE tableid = 339 AND bisacdatacode = @v_code

			SELECT @v_count = 0

            SELECT @v_count2 = count(*)
              FROM bookbisaccategory WHERE bisaccategorycode = @v_datacode AND bisaccategorysubcode = @v_datasubcode

			IF @v_count2 > 0 BEGIN
				SELECT @v_count3 = 0
				
				SELECT @v_count3 = count(*) FROM subgentables WHERE tableid = 339 AND bisacdatacode = @v_replacement_code 

				IF @v_count3 = 1 BEGIN				
					SELECT @v_replacement_datacode = datacode, @v_replacement_datasubcode = datasubcode
    			      FROM subgentables WHERE tableid = 339 AND bisacdatacode = @v_replacement_code 
					...
					WHILE (@@FETCH_STATUS = 0) BEGIN
					    ...
					    SELECT @v_mediatypecode = COALESCE(mediatypecode,0) FROM bookdetail WHERE bookkey = @v_bookkey
					    
					    IF @v_mediatypecode > 0 BEGIN

							SELECT @v_count4 = 0

							SELECT @v_count4 = count(*) FROM bookbisaccategory WHERE bookkey = @v_bookkey AND printingkey = @v_printingkey
								AND bisaccategorycode = @v_replacement_datacode AND bisaccategorysubcode =  @v_replacement_datasubcode
								 ...
								 UPDATE bookbisaccategory
									SET bisaccategorycode = @v_replacement_datacode,
										bisaccategorysubcode =  @v_replacement_datasubcode,
							 			lastuserid = 'FB_BISACSUBJECT_UPDATE_2013',
								  ...
								  IF @v_replacement_datacode <> @v_datacode BEGIN
									  SET  @v_replacement_datacode_desc = ltrim(rtrim(dbo.get_gentables_desc(339,convert(int,@v_replacement_datacode),'long'))) 
									  exec qtitle_update_titlehistory 'bookbisaccategory', 'bisaccategorycode' , @v_bookkey, 1, 0, @v_replacement_datacode_desc, 'Update', 'FB_BISACSUBJECT_UPDATE_2013', null,'Bisac Heading', @o_error_code output, @o_error_desc output
								  END
	  							
								  IF @v_replacement_datasubcode <> @v_datasubcode BEGIN
									  ...
									  exec qtitle_update_titlehistory 'bookbisaccategory', 'bisaccategorysubcode' , @v_bookkey, 1, 0, @v_replacement_datasubcode_desc, 'Update', 'FB_BISACSUBJECT_UPDATE_2013', null,'Bisac Sub Heading', @o_error_code output, @o_error_desc output
						 		  END
							 END
					     END --@v_mediatypecode > 0
					     ELSE 
							print 'There is no mediatypecode on bookdetail for bookkey: ' + CAST(@v_bookkey AS VARCHAR) + '. Bisaccategorycode(subcode) will not be updated.'
