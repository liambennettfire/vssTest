...
BEGIN
 ...
 --PRINT '@v_specs_formatyearkey=' + convert(varchar, @v_specs_formatyearkey)
  ...
  IF COALESCE(@v_scaleprojectttype,0) = 0 BEGIN
    SET @o_error_code = -1
    SET @o_error_desc = 'scaleprojecttype is empty on taqversionspeccategory (speccategorykey = ' + cast(@v_relatedspeccategorykey as varchar) + ').'
    RETURN
  END
  ...
  DECLARE get_specitems_cur CURSOR fast_forward FOR
   SELECT si.itemcategorycode,si.itemcode,
          dbo.qscale_find_specification_value(@v_specs_formatyearkey,si.itemcategorycode,si.itemcode),
 ...
 ORDER BY si.itemcategorycode, si.itemcode
  ...
  IF @v_message is not null BEGIN
    ...
    IF @v_num_parameters > 0 AND @v_parameter1desc is not null BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter1desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter1categorycode,@v_parameter1code),' ')     
    END
    IF @v_num_parameters > 1 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter2desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter2categorycode,@v_parameter2code),' ')      
    END
    IF @v_num_parameters > 2 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter3desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter3categorycode,@v_parameter3code),' ')      
    END
    IF @v_num_parameters > 3 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter4desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter4categorycode,@v_parameter4code),' ')     
    END
    IF @v_num_parameters > 4 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter5desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter5categorycode,@v_parameter5code),' ')      
    END
    IF @v_num_parameters > 5 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter6desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter6categorycode,@v_parameter6code),' ')     
    END
    IF @v_num_parameters > 6 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter7desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter7categorycode,@v_parameter7code),' ')      
    END
    IF @v_num_parameters > 7 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter8desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter8categorycode,@v_parameter8code),' ')      
    END
    IF @v_num_parameters > 8 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter9desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter9categorycode,@v_parameter9code),' ')      
    END
    IF @v_num_parameters > 9 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter10desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter10categorycode,@v_parameter10code),' ')     
    END
    IF @v_num_parameters > 10 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter11desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter11categorycode,@v_parameter11code),' ')      
    END
    IF @v_num_parameters > 11 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter12desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter12categorycode,@v_parameter12code),' ')      
    END
    IF @v_num_parameters > 12 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter13desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter13categorycode,@v_parameter13code),' ')      
    END
    IF @v_num_parameters > 13 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter14desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter14categorycode,@v_parameter14code),' ')      
    END
    IF @v_num_parameters > 14 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter15desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter15categorycode,@v_parameter15code),' ')      
    END
    IF @v_num_parameters > 15 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter16desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter16categorycode,@v_parameter16code),' ')      
    END
    IF @v_num_parameters > 16 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter17desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter17categorycode,@v_parameter17code),' ')      
    END
    IF @v_num_parameters > 17 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter18desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter18categorycode,@v_parameter18code),' ')      
    END
    IF @v_num_parameters > 18 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter19desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter19categorycode,@v_parameter19code),' ')     
    END
    IF @v_num_parameters > 19 BEGIN
      SET @v_message = @v_message + ' ' + @v_parameter20desc + ':' + COALESCE(dbo.qscale_find_specification_desc(@v_specs_formatyearkey,@v_parameter20categorycode,@v_parameter20code),' ')    
    END
