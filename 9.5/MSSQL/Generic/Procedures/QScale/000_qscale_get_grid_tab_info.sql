if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qscale_get_grid_tab_info') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure dbo.qscale_get_grid_tab_info
GO
...
CREATE PROCEDURE qscale_get_grid_tab_info
 (@i_projectkey          integer,
  @i_scaletabkey         integer,
  @i_adminspeckey          integer,
...
/******************************************************************************
**  File: 
**  Name: qscale_get_grid_tab_info
**  Desc: This stored procedure returns grid tab info. 
**
**    Auth: Alan Katzen
**    Date: 1 March 2012
*******************************************************************************/
...
BEGIN
  ...
  SELECT @v_parametervaluecode = COALESCE(parametervaluecode,0),
   ...
   WHERE scaleadminspeckey = @i_adminspeckey
  ...
  SELECT @v_row_tableid tableid, @v_fieldtype fieldtype, @v_row_itemcategorycode itemcategorycode, 
...
GO
GRANT EXEC ON qscale_get_grid_tab_info TO PUBLIC
GO
