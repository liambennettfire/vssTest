  ...
  BEGIN          
    ...
    BEGIN
      ...
      IF @CurrentCSApprovalCode = 3 BEGIN
        -- Not allowed to update CS ApprovalCode - write to qse_updatefeedback table
--        SET @FailedInd = 1
                      
        INSERT INTO qse_updatefeedback (userkey, searchitemcode, key1, key2, itemdesc, runtime, message)
        VALUES (@UserKey, @SearchItem, @BookKey, @PrintingKey, @Title, getdate(),
          'Cannot change CS Approval Code when it is set to ' + '''' + dbo.get_gentables_desc(620, @CurrentCSApprovalCode, 'long') + '''')
