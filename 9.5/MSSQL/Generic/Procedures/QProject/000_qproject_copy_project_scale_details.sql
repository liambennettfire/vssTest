...
begin
  ...
  END
	...
	select @newkey, @i_new_projectkey, @v_new_rowkey, @v_new_columnkey, 
		CASE
			WHEN (COALESCE(itemcategorycode, 0) = 0 OR itemcategorycode IN (SELECT datacode FROM qutl_get_gentable_itemtype_filtering(616, @v_newprojectitemtype, @v_newprojectusageclass)))
			THEN NULL 
			ELSE itemcategorycode
		END as itemcategorycode, 
		CASE
			WHEN (COALESCE(itemcode, 0) = 0 OR itemcode IN (SELECT datasubcode FROM qutl_get_gentable_itemtype_filtering(616, @v_newprojectitemtype, @v_newprojectusageclass) WHERE datacode = itemcategorycode))
			THEN NULL 
			ELSE itemcode
		END as itemcode,	
		CASE
			WHEN (COALESCE(itemdetailcode, 0) = 0 OR itemcode IN (SELECT datasub2code FROM qutl_get_gentable_itemtype_filtering(616, @v_newprojectitemtype, @v_newprojectusageclass) WHERE datacode = itemcategorycode AND datasubcode  = itemcode))
			THEN NULL 
			ELSE itemdetailcode
		END as itemdetailcode,				
		autoapplyind, fixedchargecode, 
		CASE
			WHEN (COALESCE(calculationtypecode, 0) = 0 OR itemcode IN (SELECT datacode FROM qutl_get_gentable_itemtype_filtering(627, @v_newprojectitemtype, @v_newprojectusageclass)))
			THEN NULL 
			ELSE calculationtypecode
		END as calculationtypecode,		
		CASE
			WHEN (COALESCE(thresholdspeccategorycode, 0) = 0 OR thresholdspeccategorycode IN (SELECT datacode FROM qutl_get_gentable_itemtype_filtering(616, @v_newprojectitemtype, @v_newprojectusageclass)))
			THEN NULL 
			ELSE thresholdspeccategorycode
		END as thresholdspeccategorycode,
		CASE
			WHEN (COALESCE(thresholdspecitemcode, 0) = 0 OR itemcode IN (SELECT datasubcode FROM qutl_get_gentable_itemtype_filtering(616, @v_newprojectitemtype, @v_newprojectusageclass) WHERE datacode = thresholdspeccategorycode))
			THEN NULL 
