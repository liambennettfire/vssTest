  ...
  -- Copy product numbers for @i_copy_projectkey and current taqelementkey
	...
	begin
		...
		where commentkey = @tobecopiedkey AND
	     (COALESCE(commenttypecode, 0) = 0 OR commenttypecode IN (SELECT datacode FROM qutl_get_gentable_itemtype_filtering(534, @v_newprojectitemtype, @v_newprojectusageclass))) AND
	     (COALESCE(commenttypesubcode, 0) = 0 OR commenttypesubcode IN (SELECT datasubcode FROM qutl_get_gentable_itemtype_filtering(534, @v_newprojectitemtype, @v_newprojectusageclass) WHERE datacode = commenttypecode)) 		
