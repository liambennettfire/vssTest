...
END
	...
	begin
        ...
        FETCH NEXT FROM taqprojectcontactrole_iteration_cur INTO @v_rolecode, @v_activeind, @v_authortypecode, @v_primaryind, @v_ratetypecode, @v_workrate, @v_quantity, @v_shippingmethodcode, @v_globalcontactrelationshipkey, @v_indicator
		...
		ORDER BY itemtypesubcode DESC				 		 
		 ...
		 IF @v_indicator1 <> 1 OR NOT EXISTS(SELECT * FROM taqprojectcontactrole WHERE taqprojectkey = @i_new_projectkey AND rolecode = @v_rolecode) BEGIN                 
				...
				insert into taqprojectcontactrole
						...
						shippingmethodcode, globalcontactrelationshipkey, indicator)        
				values(@newkey2, @newkey, @i_new_projectkey, @v_rolecode, @v_activeind,
					   ...
					   @v_shippingmethodcode, @v_globalcontactrelationshipkey, @v_indicator)
...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
        ...
        DECLARE taqprojectcontactrole_iteration_cur CURSOR FOR																   
			  SELECT r.rolecode, r.activeind,
				r.authortypecode, r.primaryind, r.ratetypecode, r.workrate, r.quantity, r.shippingmethodcode, r.globalcontactrelationshipkey, r.indicator
			  from taqprojectcontactrole r
			  where taqprojectkey = @i_copy2_projectkey
					and taqprojectcontactrolekey = @tobecopiedkey2
					and taqprojectcontactkey = @tobecopiedkey AND 
...
--			IF @v_indicator1 <> 1 OR NOT EXISTS(SELECT * FROM taqprojectcontactrole WHERE taqprojectkey = @i_new_projectkey AND rolecode = @v_rolecode) BEGIN 		 	                
			IF NOT EXISTS(SELECT * FROM taqprojectcontactrole WHERE taqprojectkey = @i_new_projectkey AND rolecode = @v_rolecode AND taqprojectcontactkey = @newkey) BEGIN 		 	                
				exec get_next_key @i_userid, @newkey2 output
				insert into taqprojectcontactrole
						(taqprojectcontactrolekey, taqprojectcontactkey, taqprojectkey, rolecode, activeind,
						authortypecode, primaryind, lastuserid, lastmaintdate, ratetypecode, workrate, quantity, 
