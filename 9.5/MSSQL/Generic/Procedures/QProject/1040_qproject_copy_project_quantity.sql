...
WHERE taqprojectkey = @i_copy_projectkey AND
   qtyoutletcode IN (SELECT datacode FROM qutl_get_gentable_itemtype_filtering(527, @v_newprojectitemtype, @v_newprojectusageclass)) AND
  (COALESCE(qtyoutletsubcode, 0) = 0 OR qtyoutletsubcode IN (SELECT datasubcode FROM qutl_get_gentable_itemtype_filtering(527, @v_newprojectitemtype, @v_newprojectusageclass) WHERE datacode = qtyoutletcode))	  
  ...
  WHERE q1.taqprojectkey = @i_copy2_projectkey AND
    NOT EXISTS (SELECT * FROM taqprojectqtybreakdown q2
                WHERE q1.qtyoutletcode = q2.qtyoutletcode AND
                  ...
                  q2.taqprojectkey = @i_copy_projectkey) AND
   qtyoutletcode IN (SELECT datacode FROM qutl_get_gentable_itemtype_filtering(527, @v_newprojectitemtype, @v_newprojectusageclass)) AND
  (COALESCE(qtyoutletsubcode, 0) = 0 OR qtyoutletsubcode IN (SELECT datasubcode FROM qutl_get_gentable_itemtype_filtering(527, @v_newprojectitemtype, @v_newprojectusageclass) WHERE datacode = qtyoutletcode))	              
