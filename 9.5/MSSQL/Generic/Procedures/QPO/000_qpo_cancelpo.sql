...
BEGIN
  ...
  IF @v_qsicode_project <> 41 AND @v_qsicode_project <> 51 BEGIN
    SET @o_error_code = -1
  ...
  IF @v_error <> 0 BEGIN
    SET @o_error_code = -1
    SET @o_error_desc = 'Error returning Cancelled status from gentable 522 qsicode 12'
    RETURN  
  END   
  ...
  -- Change status on every ‘Pending’ PO report to ‘Cancelled’  
	DECLARE taqproject_cur CURSOR FOR
		...
		WHERE v.taqprojectkey = p.taqprojectkey
		    AND v.relatedprojectkey = c.projectkey 
    ...
    WHILE @@fetch_status = 0 BEGIN
		UPDATE taqproject 
		SET taqprojectstatuscode = @v_cancelled_status, lastuserid = @lastuserid_var, lastmaintdate = GETDATE()
