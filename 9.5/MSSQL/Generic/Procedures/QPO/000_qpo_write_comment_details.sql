...
DECLARE
  ...
  @v_sequence INT,
  @v_spacepos INT,
  @v_string VARCHAR(244),
  @v_substring VARCHAR(255),
  @v_trimmednormalreversestring VARCHAR(255),
  @v_trimmedreversestring VARCHAR(500)
  
BEGIN

  SET @v_sequence = 0
  SET @v_commenttext = @i_commenttext

  IF LEN(CONVERT(VARCHAR(244), @v_commenttext)) > 0
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
        ...
        BREAK
      END
      ELSE BEGIN  --LEN(@v_commenttext) > 244
  ...
  END --IF LEN(CONVERT(VARCHAR(240), @v_commenttext)) > 0
