...
**
**    Modified: Kusum   Date:02/11/2014 Case 31547
**
*******************************************************************************/
...
BEGIN
  ...
  IF (@i_rolecode =   @v_shipping_location_rolecode) OR (@i_rolecode =   @v_shipping_location_rarely_used_rolecode) BEGIN
	SELECT @v_count = COUNT(*) 
	     FROM taqprojectcontact t , taqprojectcontactrole r
	 WHERE t.taqprojectkey = @i_projectkey
	        AND t.taqprojectcontactkey = r.taqprojectcontactkey
	        AND t.globalcontactkey = @i_globalcontactkey
	        AND r.globalcontactrelationshipkey = @i_globalcontactrelationshipkey
	        AND r.shippingmethodcode = @i_shippingmethodcode
	        
	  IF @v_count = 1 BEGIN
			SELECT TOP(1) @v_taqprojectcontactkey = t.taqprojectcontactkey 
				 FROM taqprojectcontact t , taqprojectcontactrole r
		     WHERE t.taqprojectkey = @i_projectkey
			     	AND t.taqprojectcontactkey = r.taqprojectcontactkey
				    AND t.globalcontactkey = @i_globalcontactkey
			...
			END     	 
			 ...
			 IF NOT EXISTS (SELECT * FROM taqprojectcontactrole WHERE taqprojectcontactkey = @v_taqprojectcontactkey AND 
				   taqprojectkey = @i_projectkey AND rolecode = @i_rolecode AND globalcontactrelationshipkey = @i_globalcontactrelationshipkey
				   AND shippingmethodcode = @i_shippingmethodcode) BEGIN  
				  ...
				  INSERT INTO taqprojectcontactrole
					...
					quantity, shippingmethodcode, globalcontactrelationshipkey, indicator, lastuserid, lastmaintdate)
				  VALUES(@v_taqprojectcontactrolekey, @v_taqprojectcontactkey, @i_projectkey, @i_rolecode, 1,
					@i_quantity, @i_shippingmethodcode, @i_globalcontactrelationshipkey, @i_indicator, @i_userid, GETDATE())	
   ...
   IF @i_rolecode <>   @v_shipping_location_rolecode BEGIN
	  IF EXISTS(SELECT * FROM taqprojectcontact WHERE taqprojectkey = @i_projectkey AND globalcontactkey = @i_globalcontactkey) BEGIN
	  ...
	  IF NOT EXISTS (SELECT * FROM taqprojectcontactrole WHERE taqprojectcontactkey = @v_taqprojectcontactkey AND 
				   taqprojectkey = @i_projectkey AND rolecode = @i_rolecode) BEGIN  
		  EXEC get_next_key @i_userid, @v_taqprojectcontactrolekey OUTPUT
