IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = Object_id('dbo.specs_copy_write_documentationspecs') AND (type = 'P' OR type = 'RF'))
BEGIN
  DROP PROC dbo.specs_copy_write_documentationspecs
END
GO

CREATE PROCEDURE specs_copy_write_documentationspecs 	(
  @i_from_bookkey     INT,
...
BEGIN
   ...
   BEGIN  
		...
		INSERT INTO documentationspecs(bookkey, printingkey, vendorkey,artwork,artworkdesc,flatsize,finalsize,stock,numcolors,
          printmethod,numsides,numpages,coating,
          perfed,scored,folded,collated,trim,drilled,shrinkwrapped,bindin,blowin,dies,diedescription,
        ...
        VALUES (@i_to_bookkey,@i_to_printingkey,@v_vendorkey,@v_artwork,@v_artworkdesc,@v_flatsize,@v_finalsize,@v_stock,@v_numcolors,
            @v_printmethod,@v_numsides,@v_numpages,@v_coating,
            @v_perfed,@v_scored,@v_folded,@v_collated,@v_trim,@v_drilled,@v_shrinkwrapped,
				@v_bindin,@v_blowin,@v_dies,@v_diedescription,@i_userid,getdate())
	END

	-- Copy all notes associated with documentationspecs
   EXEC specs_Copy_write_notes @i_from_bookkey,@i_from_printingkey,@i_to_bookkey, @i_to_printingkey,30,@i_userid,@o_error_code OUTPUT,@o_error_desc OUTPUT
