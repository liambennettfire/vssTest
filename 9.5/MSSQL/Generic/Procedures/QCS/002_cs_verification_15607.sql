...
IF @v_customerprocedurename <> '' BEGIN
	...
	IF @o_error_code = -1 BEGIN
	    ...
	    SELECT @v_msgtype_error = datacode FROM gentables WHERE tableid = 539 AND qsicode = 2 --Error
	    SELECT @v_messagecategoryqsicode = 32
		SET @o_error_code = 0
		EXEC bookverificationmessage_insert @i_bookkey, @v_verificationtypecode, @v_msgtype_error, @v_msg, @i_username, @o_error_code output, @o_error_desc output, @v_messagecategoryqsicode
		IF @o_error_code = -1
			RETURN
	...
	ELSE IF @o_error_code > 0 BEGIN
		...
		IF @o_error_code = 1 BEGIN-- Passed
			SELECT @v_msgtype_error = datacode FROM gentables WHERE tableid = 539 AND qsicode = 4 --Information
		END
		ELSE IF @o_error_code = 2 BEGIN-- Warning
			SELECT @v_msgtype_error = datacode FROM gentables WHERE tableid = 539 AND qsicode = 3 --Warning
			SET @v_warnings = 1
		END
