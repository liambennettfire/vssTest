/******************************************************************************
**  Name: imp_300024011001
**  Desc: IKE  Globalcontact first name
**  Auth: Bennett     
**  Date: 5/9/2016
*******************************************************************************
**  Change History
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[imp_300024011001]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[imp_300024011001]
GO

CREATE PROCEDURE dbo.imp_300024011001 
...
BEGIN
  ...
  if coalesce(@v_elementval,'')<>coalesce(@v_old_val,'')
    begin
      ...
      set @v_errmsg='Globalcontact first name updated'
    end
  else
    begin
      set @v_errmsg='Globalcontact first name unchanged'
    end
...
GRANT EXECUTE ON dbo.[imp_300024011001] to PUBLIC 
GO
