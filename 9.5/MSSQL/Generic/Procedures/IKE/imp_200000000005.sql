/******************************************************************************
**  Name: imp_200000000005
**  Desc: IKE Valid Date Warnings
**  Auth: Bennett     
**  Date: 5/9/2016
*******************************************************************************
**  Change History
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[imp_200000000005]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[imp_200000000005]
GO

CREATE PROCEDURE dbo.imp_200000000005 
...
BEGIN
	...
	IF dbo.resolve_date(@v_elementval) is NULL
    		BEGIN
      			SET @v_errlevel = 2
      			SET @v_msg = 'WARNING (INVALID DATE): '
      			SET @v_msg = @v_msg + coalesce(@v_elementdesc, 'n/a') +': '
      			SET @v_msg = @v_msg + coalesce(@v_elementval, 'n/a') + ' will not be updated'
    		END
...
GRANT EXECUTE ON dbo.[imp_200000000005] to PUBLIC 
GO
