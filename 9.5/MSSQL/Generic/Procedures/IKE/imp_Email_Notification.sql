...
BEGIN
	...
	SET HTML='<tr><td width = 5%>' 
		...
		+ '</td></tr>'
	INSERT INTO imp_Notifications SELECT -1, NULL,NULL, NULL, NULL,'<HTML><BR />'+COALESCE(@szEmailHeaderName + ': ','')+'BATCH# '+CAST(@iMaxBatchKey as VARCHAR(MAX))+'<TABLE border="1" width=100%>'
	INSERT INTO imp_Notifications SELECT 100000000, NULL,NULL, NULL, NULL,'</TABLE></HTML>'
