...
--print '@v_variableamount= ' + cast(@v_variableamount as varchar)
  ...
  IF @v_fixedamount > 0 BEGIN
     IF @v_fixedchargecode IS NOT NULL BEGIN
		...
		SELECT @v_bucket_cost=cost FROM #scalecostbucket_table WHERE internalcode=@v_fixedchargecode --AND taqversionspecitemkey=@i_taqversionspecitemkey
					
		IF @v_bucket_cost IS NULL BEGIN --insert value to costbucket
			SET @v_bucket_cost = @v_fixedamount
			INSERT INTO #scalecostbucket_table (formatkey,internalcode,cost,validforprtgs,calccostcode)
  ...
  IF @i_specialprocess = 'SPECUNIT' BEGIN
    ...
    IF @v_variablecost > 0 BEGIN
      ...
      --insert costs into bucket
      SELECT @v_bucket_cost = cost FROM #scalecostbucket_table WHERE internalcode = @v_chargecode --AND taqversionspecitemkey=@i_taqversionspecitemkey
...
--				  	VALUES (@i_taqversionspecitemkey, @v_chargecode, @v_acctgcatcode,@v_bucket_cost,@v_bucket_calccost)
   ...
   ELSE IF @i_percentcalcind IS NOT NULL AND @i_percentcalcind <> 0 BEGIN
    ...
    WHILE (@@FETCH_STATUS=0) BEGIN
        SELECT @v_bucket_cost=cost FROM #scalecostbucket_table WHERE internalcode=@v_code2 --AND taqversionspecitemkey=@i_taqversionspecitemkey
