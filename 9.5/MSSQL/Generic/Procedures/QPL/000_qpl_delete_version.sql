...
**	--------  ------  -----------
**	01/08/15  JR      Case #29966
**  05/19/16  Kate    Case 35972 - Background recalc - delete from taqversionrecalcneeded table
**********************************************************************************************************************/
...
BEGIN
 ...
 -- TAQVERSIONMARKETCHANNELYEAR
  ...
  -- TAQVERSIONRECALCNEEDED
  DELETE FROM taqversionrecalcneeded
  WHERE taqprojectkey = @i_projectkey AND plstagecode = @i_plstage AND taqversionkey = @i_plversion
  
  SELECT @v_error = @@ERROR
  IF @v_error <> 0 BEGIN
    SET @o_error_desc = 'Error deleting from taqversionrecalcneeded table (A-Error ' + cast(@v_error AS VARCHAR) + ').'
    GOTO RETURN_ERROR
  END  
  ...
  -- If there are no versions left for this project/stage after deleting this version, remove all recalc needed rows for this project/stage
  ...
  BEGIN
    ...
    SELECT @v_error = @@ERROR
    IF @v_error <> 0 BEGIN
      SET @o_error_desc = 'Error deleting from taqversionrecalcneeded table (B-Error ' + cast(@v_error AS VARCHAR) + ').'
      GOTO RETURN_ERROR
    END    
  END
