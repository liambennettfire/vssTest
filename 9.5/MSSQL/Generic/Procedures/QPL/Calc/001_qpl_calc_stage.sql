if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_calc_stage') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_calc_stage
GO

CREATE PROCEDURE qpl_calc_stage (  
  @i_projectkey INT,
  @i_plstage    INT,
...
/**********************************************************************************************
**  Name: qpl_calc_stage
**  Desc: Generic Stage calculation which sums up costs or income records for all chargecodes
...
BEGIN
  ...
  ELSE
    BEGIN
      ...
      EXEC qpl_calc_version @i_projectkey, @i_plstage, @v_selected_versionkey, 
        @i_placctgcategory, @i_placctgsubcategory, @i_incomeind, @o_result OUTPUT
    END
...
GRANT EXEC ON qpl_calc_stage TO PUBLIC
GO
