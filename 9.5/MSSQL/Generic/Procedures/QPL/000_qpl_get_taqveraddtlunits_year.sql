if exists (select * from dbo.sysobjects where id = object_id(N'dbo.qpl_get_taqveraddtlunits_year') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
  drop procedure dbo.qpl_get_taqveraddtlunits_year
GO

CREATE PROCEDURE qpl_get_taqveraddtlunits_year (  
...
/***************************************************************************************************
**  Name: qpl_get_taqveraddtlunits_year
**  Desc: This stored procedure returns taqversionaddtlunitsyear records for given version/format.
**
...
BEGIN
  ...
  IF @i_excludeqsicode = 1 BEGIN
    SELECT y.*
    FROM taqversionaddtlunits u LEFT OUTER JOIN taqversionaddtlunitsyear y ON u.addtlunitskey = y.addtlunitskey
...
GRANT EXEC ON qpl_get_taqveraddtlunits_year TO PUBLIC
GO
