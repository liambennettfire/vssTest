...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[deletetitle_delete_book]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[deletetitle_delete_book]
GO
...
/*              : Kusum Basra                                                                                                                                                 */
/*                                                                                                                                                                                    */
/*    Note: IF this procedure is called from other than the desktop application                                                                         */
/*             a) IF primary title is deleted, will change all subordinate titles to primary titles and all title propagation will be deleted*/
/*             b) IF subordinate title is deleted, will remove all title propagation                                                                            */ 
/********************************************************************************************/ 
...
BEGIN
  ...
  WHILE (@@FETCH_STATUS = 0 ) BEGIN
    ...
    IF @v_clientoption = 0 BEGIN
      ...
      WHILE (@@FETCH_STATUS = 0) BEGIN
        ...
        IF @v_set_budgetprice = 0.00 AND @v_set_finalprice = 0.00 BEGIN
        ...
        ELSE BEGIN       
            ...
            IF @v_orig_set_budgetprice <> @v_set_budgetprice BEGIN
			...
			IF @v_orig_set_finalprice <> @v_set_finalprice BEGIN
  ...
  DELETE FROM bookfamily WHERE bookfamily.childbookkey = @delete_title_bookkey

  DELETE FROM bookfamily WHERE bookfamily.parentbookkey = @delete_title_bookkey
  ...
  -- ***** END 5/3/07 - KW - Modified for Case 4712. *****/
	...
	/* delete from book  */
	DELETE FROM book WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
	...
	DELETE FROM bookauthor WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
	...
	DELETE FROM bookdetail WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
	...
	DELETE FROM bookaudience WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
	...
	DELETE FROM bookorgentry WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
	...
	/* delete from bookprice  */
	DELETE FROM bookprice WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
	...
	DELETE FROM citation WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
	...
	DELETE FROM bookproductdetail WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 
	begin
	...
	/* delete from keyword  */
	DELETE FROM keyword WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
	...
	DELETE FROM bookkeywords WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
		select @err_msg = 'Error deleting from bookkeywords for bookkey' + convert(char(10),@delete_title_bookkey) 
        print @err_msg
        SELECT @error_code = -1
		SELECT @error_desc = @err_msg
		return 
	...
	/* delete from isbn  */
	DELETE FROM isbn WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
	...
	DELETE FROM bookmisc WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
	...
	DELETE FROM associatedtitles WHERE bookkey = @delete_title_bookkey OR associatetitlebookkey = @delete_title_bookkey;
	if @@error != 0 begin
	...
	DELETE FROM rankings WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
		select @err_msg = 'Error deleting from rankings for bookkey' + convert(char(10),@delete_title_bookkey) 
        print @err_msg
        SELECT @error_code = -1
		SELECT @error_desc = @err_msg
		return 
	...
	DELETE FROM discoveryquestions WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
		select @err_msg = 'Error deleting from discoveryquestions for bookkey' + convert(char(10),@delete_title_bookkey) 
        print @err_msg
        SELECT @error_code = -1
		SELECT @error_desc = @err_msg
		return 
	...
	DELETE FROM bookcustom WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 
	begin
		select @err_msg = 'Error deleting from bookcustom for bookkey' + convert(char(10),@delete_title_bookkey) 
        print @err_msg
        SELECT @error_code = -1
		SELECT @error_desc = @err_msg
		return 
	...
	DELETE FROM booksubjectcategory WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
		select @err_msg = 'Error deleting from booksubjectcategory for bookkey' + convert(char(10),@delete_title_bookkey) 
        print @err_msg
        SELECT @error_code = -1
		SELECT @error_desc = @err_msg
		return 
	...
	DELETE FROM bookverificationmessage WHERE bookkey = @delete_title_bookkey;
	if @@error != 0 begin
		select @err_msg = 'Error deleting from bookverificationmessage for bookkey' + convert(char(10),@delete_title_bookkey) 
        print @err_msg
        SELECT @error_code = -1
		SELECT @error_desc = @err_msg
		return 
	...
	IF @v_count > 0 BEGIN
		...
		DELETE FROM coretitleinfo
			WHERE bookkey = @delete_title_bookkey AND printingkey = 0 ;
		IF @@error != 0 BEGIN
