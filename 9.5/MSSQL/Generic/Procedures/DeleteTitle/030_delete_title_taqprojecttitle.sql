...
*   --------     --------    ------------------------------------------------------------------
**  03/04/2016   UK          Case 36810
**  06/29/2016   UK		     Case 38904
***********************************************************************************************/
...
IF @v_count2 > 0 BEGIN
	SELECT @v_titleacq_datacode = datacode, @v_titleacq_datasubcode = datasubcode
	  FROM subgentables
     WHERE tableid = 550 AND datacode = 3 AND  qsicode = 1
END
...
--print @v_searchitemcode
     IF @v_searchitemcode = @v_titleacq_datacode AND @v_usageclasscode = @v_titleacq_datasubcode BEGIN
...
--print @v_active_status
				UPDATE taqproject SET taqprojectstatuscode = @v_active_status, lastmaintdate = GETDATE() WHERE taqprojectkey = @v_taqprojectkey
			 END    --@v_count2 = 0
		END            --@v_count = 0
